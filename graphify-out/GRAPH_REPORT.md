# Graph Report - anidachi-LP-monorepo  (2026-09-06)

## Corpus Check
- 11 files · ~1,155,063 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 11177 nodes · 24928 edges · 664 communities (390 shown, 248 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 420 edges (avg confidence: 0.85)
- Token cost: current Codex semantic usage unavailable; code extraction was local AST.

## Graph Freshness
- Built from commit: `b28f8dc` (scoped catalog API update).
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `pnpm graph:update:code` after meaningful code changes; use the Codex Graphify skill for semantic inputs.

## Community Hubs (Navigation)
- .next/**
- getGuideLinks
- getResolvedSiteOrigin
- seo-guide-blocks.tsx
- primary-checkout-cta.tsx
- react
- overlay-app.tsx
- auth-client.ts
- room-invite-notifications.ts
- protocol/src/index.ts
- room-session.ts
- room-session-storage.ts
- social.ts
- room-client.ts
- pricing-copy.ts
- popup-app.tsx
- watch-history-v3-routes.ts
- next/server
- watch-history.ts
- account.ts
- src/types.ts
- api/src/index.ts
- logDebug
- device-push.ts
- VideoAdapter
- PlaybackSyncController
- popup-watch-history.tsx
- [slug]/page.tsx
- crm-client.tsx
- social-client.ts
- verifyKreatliCrmSession
- RoomDurableObject
- p2p-media.ts
- oauth-transaction.ts
- watch-library.ts
- overlay-interface-settings.tsx
- crunchyroll.content.ts
- crunchyroll/progress.ts
- watch_history_v3_disposable_target.mjs
- overlay-room-rail.tsx
- watch-library-client.tsx
- room-departure-retry.ts
- private-integration-blob.ts
- RoomState
- room-media-defaults.ts
- popup-people-model.ts
- plan-entitlements.ts
- chrome-extension-demo-async-overlay.tsx
- video/prepare/route.ts
- privileged-overlay-intent.ts
- watch-history-v3.ts
- survey-lead.ts
- watch-history-client.ts
- playback-sync-controller.ts
- extension-session.ts
- /graphify
- room-source.ts
- room-persistence.ts
- reaction-shortcuts.ts
- ghost-cam.ts
- account-inbox.ts
- p2p-media-harness.mjs
- watch-history-storage.ts
- portfolio-audit.ts
- room-departure.ts
- createWatchHistoryClient
- store.ts
- participant-disconnect.ts
- room-hibernation-runtime.ts
- watch-history-controller.ts
- extension-codes.ts
- overlay-layout-editor.tsx
- vitest
- blob-reconciliation.ts
- db.ts
- WatchSourceDescriptor
- youtube/player-chrome.ts
- youtube/storage.ts
- watch-library-routes.test.ts
- middleware.ts
- Watch History Crunchyroll Catalog Progress Implementation Plan
- Harness
- p2p-media.test.ts
- sync-checkout-session/route.ts
- Task 1 Shared Session and Departure Contracts
- room-socket-attachment.ts
- bridge-client.ts
- react-dom/client
- account-inbox-cache.ts
- YouTubeVideoAdapter
- session.ts
- connect/page.tsx
- pseo-new-guides.tsx
- api/src/room-lifecycle.ts
- diagnostic-log.ts
- scripts
- room-source-persistence.ts
- devDependencies
- getSession
- invites-client.tsx
- AniDachi New Chat Project Context
- best-anime-movies-to-watch-with-friends/page.tsx
- Account Data Watch History Social Inbox Foundation Design
- send-connection-requests.mjs
- src/account-inbox-client.ts
- overlay-layout-model.ts
- getUserById
- [roomId]/page.tsx
- overlay-layout-engine.ts
- crunchyroll/player-chrome.ts
- dependencies
- popup-watch-history.test.tsx
- youtube/api.ts
- post/status/route.ts
- migrate-private-integration-blobs.ts
- node:assert/strict
- AniDachi Core Foundation to UI/UX Handoff Plan
- ice-servers.ts
- instagram/storage.ts
- anidachi-auth/watch-history-browse.ts
- debug-log.ts
- feature-requests.ts
- anidachi-seo-aeo-pages.md
- rules
- Anidachi Auth Integration Implementation Plan
- Social Rooms Subscriptions Execution Plan
- protocol/src/watch-history-browse.ts
- voice-audio-preferences.ts
- catalog.ts
- privileged-overlay-wiring.test.tsx
- Watch Drawer Browse Local Verification
- Social Rooms, Friends, Groups, And Subscriptions Execution Plan
- Watch History v3 Local Verification
- Watch History Catalog And Progress Design
- tasks
- hotkeys.ts
- reaction-pop.tsx
- node:fs
- tiktok/storage.ts
- Main Repository Monorepo Migration Implementation Plan
- AniDachi Pre-release Security and Reliability Readiness Plan
- .acquireMicrophoneTrack
- artwork-select.ts
- sitemap-discovery.ts
- Watch History Room Authority Threat Model
- include
- user-identity.ts
- p2p-ice.ts
- contact-messages.ts
- Commercial Room, P2P, and Watch Progress Architecture Implementation Plan
- Production Room, Realtime, and P2P Hardening Roadmap
- Source Adapter Architecture Implementation Plan
- Canonical Runtime Flow
- use-camera-interaction-lock.ts
- voice-activity.ts
- study.ts
- scripts
- compilerOptions
- Anidachi Architecture and Stack Notes
- overlay-layout-runtime.ts
- extension/test/watch-history-browse.test.ts
- watch-anime-together/page.tsx
- Watch History Local Read Implementation Plan
- Overlay Layout Engine V2 Design
- room-signaling-harness.mjs
- RecentP2PSignalBuffer
- Kreatli CRM Data Schema
- Current Development State
- Watch History v3 Staging Verification
- Watch History v2 Clean MVP Implementation Plan
- Participant
- popup-watch-browse.test.tsx
- background-privileged-room-route.test.ts
- youtube/progress.ts
- subscribe-interest-route.ts
- friends-client.tsx
- public.browse_watch_history_v3
- Anidachi Project Operating Manual
- Development Flow Quality System Plan
- LinkedIn Sales Navigator Connection Requests
- Project Planes
- dependencies
- Anidachi Project Architecture And Development
- AniDachi Extension Icon 48px
- room-invite-target-status.ts
- public-media-blob.ts
- protocol/package.json
- watch_history_v3_catalog_read_states_contract.mjs
- Development Workflow Hardening Implementation Plan
- Plan Code Canonicalization And Billing Cleanup Implementation Plan
- linkedin/package.json
- api/package.json
- overlay-mount.ts
- YouTube Playback Synchronization Hardening Implementation Plan
- dev-check.mjs
- release-channel-build.test.ts
- components.json
- watch-history-v3-routes.test.ts
- client.ts
- Survey → Subscription Conversion (Planner Notes)
- Crunchyroll Adapter Notes
- Task 2: view-local owner-validated extension browse client
- Anidachi Development Environments
- Invitation Delivery Reliability Implementation Plan
- OpenClaw: YouTube Shorts posting
- compilerOptions
- ghost-cam-size.ts
- overlay-layout-editor.test.tsx
- Chrome Web Store Listing — AniDachi Extension
- public.get_account_inbox_page
- Real-WebRTC Two-browser P2P Harness
- generate-extension-icons.mjs
- demo/package.json
- social-snapshot-cache.ts
- ControlledWebSocket
- package.json
- Environment And Secrets Matrix
- watch-history-v3.catalog-read-states.test.ts
- 20260814010000_watch_history_v2_foundation.sql
- One canonical personal episode progress row in Supabase
- Core Contract
- Voice Controls and Participant Audio Implementation Plan
- Interface Visibility Settings Design
- pull_request_template.md
- wxt.config.ts
- privacy-sanitizer.ts
- tiktok/api.ts
- best-anime-to-watch-with-friends/page.tsx
- best-short-anime-to-watch-with-friends/page.tsx
- AniDachi SEO Content Guidelines
- seo-cta-cleanup.py
- auth_artifact_cleanup_plan_contract.mjs
- YouTube SEO conversion polish + agent upgrades
- Handoff For AI Working On AniDachi Site Pages
- Technical production baseline
- Edge Cases
- File Map
- Two-phase Production Promotion
- Waitlist And CRM Durable Storage Recovery Implementation Plan
- AGENTS.md
- demo/tsconfig.json
- extension/package.json
- popup-styles.ts
- overlay-voice-session.ts
- watch_history_v2_migration_order_contract.mjs
- scratchpad.md
- Codex-Hosted Semantic Extraction
- Global Constraints
- V1 Product Decisions (Historical)
- Shared Pure Visibility Policy
- validate-extension-artifact.mjs
- room-metering.ts
- overlay-layout-engine.test.ts
- MemoryStorageArea
- public.get_account_inbox_page_v2
- watch_history_v3.test.sql
- Project Knowledge Map
- Popup People And Social Directory Implementation Plan
- Rollout Phases
- smoke-staging-web.mjs
- Incremental Re-extraction
- MAX_PARTICIPANT_ID_CHARS
- createWatchHistoryCatalogCoordinator
- Download on the App Store Badge
- 20260816090000_watch_history_v2_bounded_read.sql
- public.active_room_sessions
- README.md
- Resources Progress Menu Implementation Plan
- Task 7 Complete
- p2p-scorecard.mjs
- room-invite-notification-runtime.test.ts
- SEO Trust & Authority plan (2026-07-28)
- AniDachi Logo Asset
- 20260819133849_auth_channel_rotation.sql
- Notification Delivery
- Rollout Order
- Completed Public-Product Recovery Record
- Development Quality Gates
- panel-account-title.tsx
- overlay-layout-ghost-preview.tsx
- overlay-media-session.ts
- extension/src/watch-history-browse.ts
- Conversion metrics (GA4)
- Host Avatar Asset
- public.apply_watch_progress_v2
- graphify reference: extra exports and benchmark
- Executor's Feedback or Assistance Requests
- Global Constraints
- MemoryStorage
- room-quota-display.ts
- room-tab-lock.ts
- include
- about/page.tsx
- Haruto Avatar Image
- Natsuki Avatar Asset
- extension_auth_pkce_concurrency_contract.mjs
- public.room_invite_actions
- public.consume_extension_auth_code_v1
- Staging Acceptance Checklist
- Global Constraints
- Global Constraints
- Approved Voice UX Simplification V2
- Production Acceptance Snapshot: 685 Survey Leads
- Watch Drawer Browse And Presentation
- YouTube Adapter Notes
- findKatamariPlayerFromReactNode
- overlay-layout.ts
- AniDachi Apple Touch Icon
- AniDachi Web Logo Asset
- public.watch_sessions
- Operating contract (mandatory)
- Project Status Board
- Site, Extension, Auth, and Database Integration Notes
- Voice UX Simplification V2
- Participant Audio Controls
- September 4 Microphone-Independent Main Control Correction
- Watch History Catalog And Progress Implementation Plan
- sync.ts
- current-resource-panel.tsx
- appendDiagnosticEntry
- diagnostic-log.test.ts
- background-invite-notification-wiring.test.ts
- AniDachi App Icon
- skills.ts
- public.room_invites
- public.list_recent_people_evidence
- public.claim_active_room_session_v1
- anidachi_private.tick_inbox_push_scheduler
- watch_history_v3_catalog.test.sql
- watch_history_v3_episode_previews.test.sql
- watch_history_v3_resource_bounds.test.sql
- graphify reference: query, path, explain
- YouTube keyword bank + templates (Keyword Planner US)
- High-level Task Breakdown (historical)
- Sitewide CTA → Plan-Picker Survey (Planner Notes)
- Control plane: apps/web and Supabase
- AniDachi Project Knowledge Map
- Тарифы AniDachi для MVP
- Extension UX
- Always-Visible Compact Participant Pills
- Final Staging Runtime Acceptance
- Exact Assignment Release
- Testing Strategy
- Five-Object CRM Reconciliation
- smoke-worker.mjs
- apps/api Agent Instructions
- apps/extension Agent Instructions
- apps/web Agent Instructions
- account/feature-requests/page.tsx
- CTA and conversion map (internal)
- privacy.ts
- watch_history_v2_migration_concurrency_contract.mjs
- 20260525_anidachi_auth.sql
- 20260620_billing_entitlements.sql
- 20260621_social_profiles_friends_recent.sql
- public.friend_groups
- public.list_recent_people_evidence_v2
- auth_artifact_cleanup.test.sql
- Participant Audio Control Surface Handoff
- V2 Staging Acceptance Matrix
- Architectural Boundaries
- Failure Behavior
- Q: Оценить отзыв о Wave 2 Watch History v2
- Q: Trace Watch History v2 extension background ownership, outbox/storage, provider progress adapters, overlay and popup writers for Wave 3.
- Q: так что ты сделал и зачем и что еще осталось, хорошо подумай и тщательно пройдить по пройденым шагам и по будущим
- Q: Trace website and extension JWT verification, refresh rotation, session caching, logout, and database authority for Task 7.
- Q: Trace Watch History v2 from extension local playback projection and outbox through HTTP API and bounded title RPC to website and Popup; identify whether a visible title returns all episode rows.
- waitlist-stats/route.ts
- Q: How should per-participant volume be moved into a side pill for video bubbles while preventing player hover interference?
- Video-Bubble Side Pill
- Stale Active Room Assignment After Guest Leave
- Reaction Misclassified as Control Traffic
- packages/protocol Agent Instructions
- inbox_push_scheduler.test.sql
- PageSessionStorage
- 20260602_extension_auth.sql
- 20260612_room_lifecycle_quota.sql
- 20260818131602_oauth_login_transactions.sql
- public.watch_catalog_read_v3
- watch_history_v3_browse.test.sql
- vercel.json
- graphify reference: add a URL and watch a folder
- graphify reference: commit hook and native CLAUDE.md integration
- graphify reference: incremental update and cluster-only
- Experimental Features
- Current Reality Check
- Privacy Security And Abuse Controls
- Audio Speech Activity Classification
- Voice Activity and Flow Model
- Target Contract And Storage Map
- Production Closeout Complete
- Surface Behavior
- launch-chrome.sh
- vitest/config
- main.ts
- env.d.ts
- public.friend_invite_links
- inbox_push_outbox.test.sql
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
- Staging Smoke Workflow
- Current Integration Foundation
- Single Active Room Global Constraints
- Persistent Exact Operation Generation
- Panel Access and Focus Overrides
- Build Extension Workflow
- Camera Interaction Shell
- Local Video Demo Page
- vite-env.d.ts
- postcss.config.mjs
- web/README.md
- auth_channel_rotation.test.sql
- extraction-spec.md
- Watch History v2 Clean MVP Scope
- Task 10: Full Automated Gates And Staging Acceptance
- Task 4: Add The Authenticated V2 Web Service And Routes
- Authoritative Room Departure Implementation Plan
- Fail-Closed CRM Runtime
- Unbounded History Enrichment
- Extension Background Owner
- Bounded Title Projection
- build-extension-public.sh
- build-extension-staging.sh
- kreatli email crm crm client copyfilteredsurveyleadstsv
- kreatli email crm crm client copyrendered
- kreatli email crm crm client runexport
- kreatli email crm crm client runexportsurveyleads
- kreatli email crm crm client runpreview
- Checkout Events
- CTA Click Events
- GA4 Conversion Funnel
- CTA Surface Map
- Page Template IDs
- Pricing Checkout Surface
- Internal Linking Protocol
- Jikan Content Source
- JSON LD Schema
- SEO Content Engine
- Sitemap Protocol
- Template A Anime Pages
- Template B Comparison Pages
- Template D Listicles
- Table Of Contents Component
- Web App Scope
- Web Commands
- public.watch_session_participants
- public.rooms
- public.rooms
- public.rooms
- public.rooms
- public.users
- public.watch_progress_checkpoints
- public.watch_sessions
- public.devices
- public.watch_progress_checkpoints
- public.extension_auth_codes
- public.refresh_tokens
- public.rooms
- cache
- Graphify File Watcher
- Graphify URL Ingest
- Graphify Extra Export Formats
- Token Reduction Benchmark
- Extraction Confidence Rubric
- Deep Mode Inference Rules
- Extraction JSON Schema
- Extraction Node ID Contract
- Graphify GitHub Clone Flow
- Cross Repo Graph Merge
- Monorepo Graph Merge Flow
- Native Claude Graphify Integration
- Graphify Post Commit Hook
- Graphify BFS DFS Query Modes
- NetworkX Query Fallback
- Path and Explain Query Flows
- Graphify Query Feedback Save
- Whisper Transcription Flow
- config
- SEO Conversion CTA Rules
- Genre Hub Page Pattern
- SEO Internal Linking Rules
- SEO Skill Invocation Flow
- JSON LD Initial HTML Requirement
- New URL Checklist
- Programmatic Watch Page Guardrails
- Robots Crawl Scope Rules
- SEO AEO Skill Scope
- Sitemap Discovery Rules
- SEO Voice and Claims Rules
- GSC SEO Optimization Batch
- Homepage CRO Rework
- Hub and Listicle Backlinks
- MAL ID and Jikan Cache Work
- Manual Spot Check Requests
- Plan Picker Survey
- Programmatic Watch Page Expansion
- Sitewide CTA Survey Modal
- Survey Email CRM Integration
- Safe Site Paths
- Sensitive Paths
- Site Development Handoff Brief
- Cloudflare TURN Endpoint
- Crunchyroll Adapter
- Host Authoritative Sync
- P2P Media Current Default
- Room Creation Auth Flow
- Environment Separation
- Extension Build Environments
- ICE Server Strategy
- P2P Signaling Replay
- WebSocket Keepalive
- PR Evidence
- Room P2P Acceptance
- Site Auth Checks
- Staging Noindex Gate
- Crunchyroll Progress Identity
- Future Backend Progress Upgrade
- Local Watch Progress Store
- Overlay Progress Recorder
- Resources Panel UI
- Resources Progress Plan
- Auth Integration Plan
- Extension Auth Bridge
- Invite Extension Handoff
- Production Auth Hardening
- Room Token Model
- Website Supabase Identity
- Worker Room Token Verification
- Commercial Architecture Plan
- Durable Watch Progress
- Observability Debug Export
- P2P Signal Replay
- Commercial Release Gates
- Source Switching Contract
- Ultra Light P2P Contract
- Branch Protection Workflow
- Build Output Policy
- Cloudflare Worker Environments
- GitHub CI Deploy Workflows
- Monorepo Layout
- Monorepo Migration Plan
- Production Release Flow
- Source Of Truth Repo
- Staging Acceptance Test
- Vercel Monorepo Deploy
- Canonical Staging Origin
- Everyday Development Loop
- Manual Dispatch Safety
- Narrow Bearer Bypass
- Staging Noindex After Password
- Staging Smoke Tests
- Store Safe Extension Builds
- Workflow Hardening Plan
- Definition Of Done For The Whole Roadmap
- How To Use This Plan
- Non-Negotiable Order
- Product Constraints
- Progress Log
- Task 0: Baseline And Progress Tracking
- Task 10: Observability, QA, And Release Gates
- Task 1: Confirm Current Staging Baseline
- Task 2: Durable Room Lifecycle And Idempotent Create
- Task 3: Protocol Contract Cleanup
- Task 4: Reconnect, Resume, And Session Identity
- Task 5: Source Descriptor, Source Generation, And Source Switching Contract
- Task 6: Cloudflare WebSocket Hibernation Migration
- Task 7: Ultra-Light P2P Reliability Polish
- Task 8: First-Class Source Switching
- Task 9: Durable Shared Watch Progress
- Block 1 — Measurement Foundation And Two-Browser Harness
- Block 2 — Room Lifecycle And Idempotent Create (roadmap Task 2)
- Block 3 — Invite Flow Without Dead Ends
- Block 4 — Sessions, Reconnect Identity, Tab Lifecycle (roadmap Tasks 3–4)
- Block 5 — P2P Media Engine Polish (roadmap Task 7, client-only)
- Block 6 — Source Switching, Hibernation, Room-End Alarms (roadmap Tasks 5–6)
- Block 7 — Network, Security, Cost Guardrails
- Block 8 — Release Gates And Rollback
- Definition Of Done (whole plan)
- Development-Flow Gate Before Block 6
- Execution Order
- Network, Security, and Cost Guardrails
- Non-Negotiable Rules
- Product Decisions Fixed By This Plan
- Progress Log
- Verified Current Reality (code audit 2026-06-12)
- Canonical Agent Entry Point
- CodeRabbit Contextual Review
- Dev Check Profiles
- Development Quality Plan
- Environment Secret Hygiene
- Local Project Knowledge Graph
- PR Template Review Checklists
- Product Planning Integration
- Rollback Runbooks
- Staging Acceptance Release Gate
- Task 9 Closeout Documentation
- Conditional CRM Rollback Anchors
- Historical Execution Plan Index
- Extension-local Web Lock
- Single Active Room Session Design
- Staging Acceptance Evidence
- Waitlist CRM Storage Incident
- Emergency Active-Room Recovery
- eslintrc
- PR AI Contribution Notes
- google
- google ads tokens mergegoogleadstokens
- Task 6 Progress Report
- kreatli crm gmail tokens cleargmailtokens
- kreatli crm gmail tokens isgmailconnected
- lib founder discord founder discord username
- lib gtag ga measurement id
- lib room quota min session start seconds
- lib social account limits max instagram accounts
- lib social account limits max tiktok accounts
- node http
- node zlib
- og
- Allowed Native Build Dependencies
- Minimum Release Age Exclusion
- public.subscriptions
- public.user_tracked_titles
- script
- src constants anidachi build id
- src constants api http base
- src constants api ws base
- src crunchyroll control crunchyroll control result source
- src crunchyroll control crunchyroll control source
- src index p2psignal
- src message composer events anidachi composer open attr
- src message composer events anidachi message composer shortcut event
- src message composer events anidachi message composer submit event
- unified
- apps demo src styles css
- apps web app globals css
- apps web lib google ads tokens.ts
- apps web lib kreatli crm gmail tokens.ts

## God Nodes (most connected - your core abstractions)
1. `.next/**` - 174 edges
2. `next/link` - 171 edges
3. `OverlayApp()` - 155 edges
4. `SeoPageLayout()` - 146 edges
5. `TocHeading` - 143 edges
6. `vitest` - 143 edges
7. `getGuideLinks()` - 122 edges
8. `logDebug()` - 120 edges
9. `P2PMediaController` - 117 edges
10. `getResolvedSiteOrigin()` - 116 edges

## Surprising Connections (you probably didn't know these)
- `Project Planes` --semantically_similar_to--> `Extension Review Scope`  [INFERRED] [semantically similar]
  AGENTS.md → .coderabbit.yaml
- `Project Planes` --semantically_similar_to--> `Protocol Review Scope`  [INFERRED] [semantically similar]
  AGENTS.md → .coderabbit.yaml
- `Project Planes` --semantically_similar_to--> `Web Review Scope`  [INFERRED] [semantically similar]
  AGENTS.md → .coderabbit.yaml
- `Three-Plane Architecture` --semantically_similar_to--> `Project Planes`  [INFERRED] [semantically similar]
  docs/new-chat-project-context.md → AGENTS.md
- `Project Planes` --semantically_similar_to--> `API Review Scope`  [INFERRED] [semantically similar]
  AGENTS.md → .coderabbit.yaml

## Import Cycles
- 1-file cycle: `apps/web/app/about/page.tsx -> apps/web/app/about/page.tsx`
- 1-file cycle: `apps/extension/src/current-resource-panel.tsx -> apps/extension/src/current-resource-panel.tsx`
- 1-file cycle: `apps/extension/entrypoints/background.ts -> apps/extension/entrypoints/background.ts`
- 1-file cycle: `apps/api/test/auth.test.ts -> apps/api/test/auth.test.ts`
- 1-file cycle: `apps/extension/src/active-adapter-playback.ts -> apps/extension/src/active-adapter-playback.ts`
- 1-file cycle: `apps/web/app/account/account-nav.tsx -> apps/web/app/account/account-nav.tsx`
- 1-file cycle: `apps/web/app/account/watch-library/watch-library-client.test.tsx -> apps/web/app/account/watch-library/watch-library-client.test.tsx`
- 1-file cycle: `scripts/linkedin/send-connection-requests.mjs -> scripts/linkedin/send-connection-requests.mjs`
- 1-file cycle: `apps/extension/entrypoints/content.tsx -> apps/extension/entrypoints/content.tsx`
- 1-file cycle: `apps/extension/test/release-channel-build.test.ts -> apps/extension/test/release-channel-build.test.ts`
- 1-file cycle: `apps/extension/test/privileged-overlay-wiring.test.tsx -> apps/extension/test/privileged-overlay-wiring.test.tsx`
- 1-file cycle: `apps/web/app/api/account/inbox/route.ts -> apps/web/app/api/account/inbox/route.ts`
- 1-file cycle: `apps/web/app/api/kreatli-crm/gmail/send/route.ts -> apps/web/app/api/kreatli-crm/gmail/send/route.ts`
- 1-file cycle: `apps/web/app/opengraph-image.tsx -> apps/web/app/opengraph-image.tsx`
- 1-file cycle: `apps/web/app/layout.tsx -> apps/web/app/layout.tsx`
- 1-file cycle: `apps/web/lib/youtube/api.ts -> apps/web/lib/youtube/api.ts`
- 1-file cycle: `apps/web/app/kreatli-email-crm/actions.ts -> apps/web/app/kreatli-email-crm/actions.ts`
- 1-file cycle: `apps/extension/src/extension-channel-identity.ts -> apps/extension/src/extension-channel-identity.ts`
- 1-file cycle: `apps/web/lib/pre-release-security-boundaries.test.ts -> apps/web/lib/pre-release-security-boundaries.test.ts`
- 1-file cycle: `apps/web/app/api/blou/publish/carousel/route.ts -> apps/web/app/api/blou/publish/carousel/route.ts`

## Hyperedges (group relationships)
- **Account Owned Cross Surface State** — docs_superpowers_specs_2026_08_06_account_data_history_social_inbox_design_account_isolation, docs_superpowers_specs_2026_08_06_account_data_history_social_inbox_design_shared_contracts, docs_superpowers_specs_2026_08_06_account_data_history_social_inbox_design_popup_surface_behavior, docs_superpowers_specs_2026_08_06_account_data_history_social_inbox_design_web_dashboard_behavior [EXTRACTED 1.00]
- **Watch History delivery preserves canonical ownership and separate authenticated acceptance** — docs_current_development_state_watch_history_v3, docs_current_development_state_exact_query_browse, docs_current_development_state_local_read_followup, docs_current_development_state_watch_history_ownership, docs_current_development_state_database_first_rollout, docs_current_development_state_artifact_and_authenticated_acceptance [EXTRACTED 1.00]
- **Three runtime planes coordinated through shared contracts** — docs_project_architecture_and_development_control_plane, docs_project_architecture_and_development_live_plane, docs_project_architecture_and_development_client_plane, docs_project_architecture_and_development_shared_protocol [EXTRACTED 1.00]
- **Code Refresh, Semantic Refresh, and Freshness Policy** — docs_project_knowledge_map_normalized_code_graph_refresh, docs_project_knowledge_map_codex_hosted_semantic_extraction, docs_project_knowledge_map_incremental_freshness_policy [EXTRACTED 1.00]
- **One Policy Drives Launcher, Rail, and Preview** — docs_superpowers_plans_2026_07_30_interface_visibility_settings_pure_visibility_policy, docs_superpowers_plans_2026_07_30_interface_visibility_settings_main_control_integration, docs_superpowers_plans_2026_07_30_interface_visibility_settings_persistent_participant_rail, docs_superpowers_plans_2026_07_30_interface_visibility_settings_finite_policy_preview [EXTRACTED 1.00]
- **Server contract, isolated client, drawer presentation, and local integration** — docs_superpowers_plans_2026_09_05_watch_drawer_browse_durable_provenance_server_boundary, docs_superpowers_plans_2026_09_05_watch_drawer_browse_watch_history_browse_contract, docs_superpowers_plans_2026_09_05_watch_drawer_browse_query_isolated_extension_client, docs_superpowers_plans_2026_09_05_watch_drawer_browse_drawer_layout_settings, docs_superpowers_plans_2026_09_05_watch_drawer_browse_integration_evidence_handoff [EXTRACTED 1.00]
- **Fast saved-history display with bounded server refresh** — docs_superpowers_plans_2026_09_05_watch_history_local_read_exact_query_episode_previews, docs_superpowers_plans_2026_09_05_watch_history_local_read_persistent_read_cache, docs_superpowers_plans_2026_09_05_watch_history_local_read_drawer_integration, docs_superpowers_plans_2026_09_05_watch_history_local_read_hard_read_fences [EXTRACTED 1.00]
- **Local evidence establishes a candidate while authenticated rollout remains pending** — docs_watch_drawer_browse_local_verification_automated_source_evidence, docs_watch_drawer_browse_local_verification_exact_source_database_evidence, docs_watch_drawer_browse_local_verification_isolated_narrow_artifact, docs_watch_drawer_browse_local_verification_real_component_visual_evidence, docs_watch_drawer_browse_local_verification_review_gate_exceptions, docs_watch_drawer_browse_local_verification_authorized_rollout_sequence [EXTRACTED 1.00]
- **Independent layers of local candidate verification** — docs_watch_history_local_read_verification_source_sql_evidence, docs_watch_history_local_read_verification_installed_mv3_evidence, docs_watch_history_local_read_verification_build_isolation, docs_watch_history_local_read_verification_review_fixes [EXTRACTED 1.00]
- **Durable Invitation Delivery Architecture** — docs_current_development_state_invitation_notification_direction, docs_environment_and_secrets_matrix_invitation_outbox_recovery, docs_superpowers_plans_2026_09_04_invitation_delivery_reliability_transactional_delivery_outbox, docs_superpowers_specs_2026_08_06_account_data_history_social_inbox_design_durable_account_inbox, docs_superpowers_specs_2026_08_06_account_data_history_social_inbox_design_web_push_invalidation, docs_superpowers_specs_2026_08_06_account_data_history_social_inbox_design_supabase_scheduled_recovery [EXTRACTED 1.00]
- **Wave 1 High-severity Boundaries** — docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_wave_1_immediate_high_severity_boundaries, docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_nextjs_15_5_23_security_patch, docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_public_and_private_blob_isolation [EXTRACTED 1.00]
- **Wave 2 Auth Security Chain** — docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_browser_oauth_transaction_binding, docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_extension_client_binding_and_pkce, docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_refresh_family_channel_rotation [EXTRACTED 1.00]
- **Wave 3 Room and Extension Isolation** — docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_room_history_authority_lifetime, docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_pre_join_websocket_admission, docs_superpowers_plans_2026_08_18_pre_release_security_reliability_readiness_plan_privileged_overlay_and_diagnostics_isolation [EXTRACTED 1.00]
- **Core Foundation Cross Plane Contracts** — docs_superpowers_plans_2026_08_21_core_foundation_ui_handoff_plan_monotonic_source_persistence, docs_superpowers_plans_2026_08_21_core_foundation_ui_handoff_plan_lifecycle_invite_actionability [EXTRACTED 1.00]
- **CRM Data Model** — apps_web_crm_data_schema_crm_data_layout, apps_web_crm_data_schema_contact_record, apps_web_crm_data_schema_touch_event, apps_web_crm_data_schema_outreach_queue_eligibility [EXTRACTED 1.00]
- **Graphify Extraction Pipeline** — _codex_skills_graphify_skill_file_detection, _codex_skills_graphify_skill_ast_structural_extraction, _codex_skills_graphify_skill_semantic_extraction, _codex_skills_graphify_skill_ast_semantic_merge, _codex_skills_graphify_skill_graph_build_pipeline [EXTRACTED 1.00]
- **Graph Integrity and Persistence** — _codex_skills_graphify_skill_graph_shrink_guard, _codex_skills_graphify_skill_graph_health_check, _codex_skills_graphify_skill_manifest_and_cost_tracking, _codex_skills_graphify_skill_incremental_update [EXTRACTED 1.00]
- **Graph Navigation Commands** — _codex_skills_graphify_skill_graph_query_navigation, _codex_skills_graphify_skill_graph_path_navigation, _codex_skills_graphify_skill_graph_node_explanation [EXTRACTED 1.00]
- **Incremental Graph Consistency Controls** — _codex_skills_graphify_references_update_incremental_detection, _codex_skills_graphify_references_update_semantic_cache_invalidation, _codex_skills_graphify_references_update_changed_file_replacement, _codex_skills_graphify_references_update_portable_manifest, _codex_skills_graphify_references_update_semantic_output_stamp_gate [EXTRACTED 1.00]
- **Participant Session Identity Flow** — docs_superpowers_plans_2026_08_23_single_active_room_session_implementation_plan_prepared_session_candidate, docs_superpowers_plans_2026_08_23_single_active_room_session_implementation_plan_participant_session_id, docs_superpowers_plans_2026_08_23_single_active_room_session_implementation_plan_room_token_session_binding, docs_superpowers_plans_2026_08_23_single_active_room_session_implementation_plan_pending_disconnect_records [EXTRACTED 1.00]
- **Repaired Waitlist Read Path** — graphify_out_memory_query_20260823_160857_trace_the_repaired_hero_waitlist_stats_crm_hero, graphify_out_memory_query_20260823_160857_trace_the_repaired_hero_waitlist_stats_crm_waitlist_stats_route, graphify_out_memory_query_20260823_160857_trace_the_repaired_hero_waitlist_stats_crm_store, graphify_out_memory_query_20260823_160857_trace_the_repaired_hero_waitlist_stats_crm_read_blob_text [EXTRACTED 1.00]
- **Voice Mode Publication Speech and Flow Separation** — docs_superpowers_plans_2026_07_27_voice_controls_and_participant_audio_plan_room_scoped_voice_state, docs_superpowers_plans_2026_07_27_voice_controls_and_participant_audio_plan_microphone_publication_state, docs_superpowers_plans_2026_07_27_voice_controls_and_participant_audio_plan_audio_speech_activity, docs_superpowers_plans_2026_07_27_voice_controls_and_participant_audio_plan_audio_transport_flow [EXTRACTED 1.00]
- **Waitlist CRM Final Staging Acceptance** — docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_final_staging_runtime_acceptance, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_three_observed_failed_survey_submissions_recovered, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_staging_survey_lead_count_685, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_fresh_preview_deployment_dpl_anazpf8xthuccrymz19tdkq2y3rq [EXTRACTED 1.00]
- **Waitlist CRM Production Closeout** — docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_task_7_complete, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_production_main_promotion_pr_240, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_fresh_production_redeploy_dpl_dct6ocjbbej848rfac38w5bhbdyg, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_production_crm_runtime_logs_clean, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_distinct_live_signup_2026_08_23t18_13_49_881z, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_post_signup_private_authority_688_contacts_686_survey_leads, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_final_docs_triggered_production_deployment_dpl_hcqvvanf9v4enshjyekrpmkfqeuy, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_live_product_counts_are_dynamic [EXTRACTED 1.00]
- **Waitlist CRM Runtime Durability Chain** — docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_crm_runtime_authority, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_optimistic_blob_mutation, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_conflict_safe_crm_mutations, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_truthful_public_submission_responses [EXTRACTED 1.00]
- **Watch History v2 Canonical Pipeline** — docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_provider_owned_eligibility, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_meaningful_progress_controller, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_extension_background_single_writer, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_authenticated_v2_web_service, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_transactional_progress_writer, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_canonical_v2_read_model, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_popup_website_read_model_cutover [EXTRACTED 1.00]
- **Watch History v2 Consistency Fences** — docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_account_history_generation_fencing, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_idempotency_first_conflict_ordering, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_deletion_fences, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_bounded_shape_outbox [EXTRACTED 1.00]
- **Watch History v2 Release Sequence** — docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_versioned_protocol_contract, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_authenticated_v2_web_service, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_extension_background_single_writer, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_logical_pre_release_cutover, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_staging_acceptance_matrix, docs_superpowers_plans_2026_08_14_watch_history_v2_clean_mvp_implementation_production_deployment_ordering [EXTRACTED 1.00]
- **Extension Release Safety** — github_workflows_build_extension_release_ref_validation, github_workflows_build_extension_extension_channel [INFERRED 0.85]
- **Room Reliability Verification Stack** — docs_superpowers_plans_2026_06_12_room_flow_p2p_flawless_execution_plan_service_level_objectives, docs_superpowers_plans_2026_06_12_room_flow_p2p_flawless_execution_plan_four_layer_p2p_acceptance_matrix, docs_superpowers_plans_2026_06_12_room_flow_p2p_flawless_execution_plan_two_browser_harness, docs_superpowers_plans_2026_06_12_room_flow_p2p_flawless_execution_plan_release_gates_and_rollback [INFERRED 0.85]
- **Canonical Catalog Progress Model** — docs_superpowers_plans_2026_09_05_watch_history_crunchyroll_catalog_progress_plan_canonical_progress_model, docs_superpowers_specs_2026_08_13_watch_history_catalog_progress_design_canonical_durable_data_model, docs_watch_history_v3_local_verification_canonical_schema_3_storage [INFERRED 0.95]
- **Cross-Plane Architecture and Contract Governance** — agents_project_planes, agents_cross_plane_contract_first, docs_new_chat_project_context_three_plane_architecture [INFERRED 0.95]
- **Derived reads preserve server authority and confirmed provenance** — docs_current_development_state_watch_history_v3, docs_current_development_state_group_provenance, docs_watch_history_local_read_verification_bounded_cache_policy, docs_watch_history_local_read_verification_preview_continuation, docs_watch_history_local_read_verification_pending_shared_observations [INFERRED 0.95]
- **Exact Departure Identity Fence** — docs_superpowers_plans_2026_08_31_authoritative_room_departure_persistent_exact_operation_generation, docs_superpowers_plans_2026_08_31_authoritative_room_departure_authoritative_room_snapshot_handoff, docs_superpowers_specs_2026_08_31_authoritative_room_departure_design_exact_session_invariants, docs_superpowers_specs_2026_08_31_authoritative_room_departure_design_late_admission_compensation [INFERRED 0.95]
- **Final History Boundary Fixes** — docs_superpowers_plans_2026_09_05_watch_history_crunchyroll_catalog_progress_plan_final_local_implementation_closeout, docs_watch_history_v3_local_verification_final_review_fixes [INFERRED 0.95]
- **Human-gated Outreach Flow** — apps_web_crm_data_agents_human_controlled_sending, apps_web_crm_data_workflow_gmail_send_step, apps_web_crm_data_workflow_touch_logging_step, apps_web_crm_data_workflow_next_action_scheduling [INFERRED 0.95]
- **Listener-Local Participant Volume Surfaces** — graphify_out_memory_query_20260828_013040_how_should_per_participant_volume_be_moved_into_a_video_bubble_side_pill [INFERRED 0.95]
- **Risk-Gated Staging Release System** — agents_git_release_flow, readme_development_workflow, docs_development_quality_gates_gate_matrix, docs_new_chat_project_context_git_and_release_flow [INFERRED 0.95]
- **Safe CRM Cutover** — docs_superpowers_specs_2026_08_23_waitlist_crm_durable_storage_design_five_object_reconciliation, docs_superpowers_specs_2026_08_23_waitlist_crm_durable_storage_design_deployment_order, docs_superpowers_specs_2026_08_23_waitlist_crm_durable_storage_design_rollback_strategy, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_staging_acceptance, docs_superpowers_plans_2026_08_23_waitlist_crm_durable_storage_recovery_production_cutover [INFERRED 0.95]
- **Schema 3 Staging Activation** — docs_current_development_state_watch_history_v3_staging_activation, docs_superpowers_plans_2026_09_05_watch_history_crunchyroll_catalog_progress_plan_staging_activation_closeout, docs_superpowers_specs_2026_08_13_watch_history_catalog_progress_design_staging_activation_completion, docs_watch_history_v3_staging_verification_ordered_staging_activation [INFERRED 0.95]
- **Single Active Room Cross-plane Authority** — docs_superpowers_specs_2026_08_23_single_active_room_session_design_durable_cross_room_assignment, docs_superpowers_specs_2026_08_23_single_active_room_session_design_live_room_state, docs_superpowers_specs_2026_08_23_single_active_room_session_design_session_binding [INFERRED 0.95]
- **Three-Plane Architecture Across Room Documents** — docs_superpowers_plans_2026_06_07_production_room_p2p_hardening_roadmap_three_plane_architecture [INFERRED 0.95]

## Communities (663 total, 248 thin omitted)

### Community 0 - ".next/**"
Cohesion: 0.01
Nodes (236): faq, itemList, metadata, tocHeadings, faq, headings, metadata, SITE_URL (+228 more)

### Community 1 - "getGuideLinks"
Cohesion: 0.02
Nodes (136): AniDachiVsHyperbeamPage(), faq, headings, metadata, SITE_URL, AniDachiVsMetastreamPage(), faq, headings (+128 more)

### Community 2 - "getResolvedSiteOrigin"
Cohesion: 0.02
Nodes (127): escapeXml(), GET(), faq, headings, itemList, metadata, SITE_URL, faq (+119 more)

### Community 3 - "seo-guide-blocks.tsx"
Cohesion: 0.03
Nodes (123): faq, headings, metadata, SITE_URL, faq, headings, metadata, SITE_URL (+115 more)

### Community 4 - "primary-checkout-cta.tsx"
Cohesion: 0.03
Nodes (109): geistMono, geistSans, metadata, shouldNoindex, viewport, AnalyticsEvents(), onScroll(), ConditionalFooter() (+101 more)

### Community 5 - "react"
Cohesion: 0.03
Nodes (84): AccountNav(), isActive(), ITEMS, dynamic, metadata, BlouLoginForm(), safeNextParam(), dynamic (+76 more)

### Community 6 - "overlay-app.tsx"
Cohesion: 0.03
Nodes (103): HOLD_FIRE_SUPER_REACTION_EXPERIMENT, normalizeExperimentFlag(), useGhostCam(), attachAndPlayVideoElement(), activeRoomConflictMessage(), appendVisibleReaction(), buildCurrentSourceUrlForInvite(), CameraBubble() (+95 more)

### Community 7 - "auth-client.ts"
Cohesion: 0.04
Nodes (91): assertExtensionLogoutRedirect(), attemptWebsiteLogoutFlow(), AuthCommand, AuthMessage, AuthMessageResponse, buildExtensionConnectUrl(), buildExtensionLogoutUrl(), buildWebUrl() (+83 more)

### Community 8 - "room-invite-notifications.ts"
Cohesion: 0.04
Nodes (94): getCurrentExtensionSession(), withInvitationHttpDeadline(), beginNotificationRetry(), claimNotificationRetry(), clearNotificationRetryAccount(), completeNotificationRetry(), expired(), Lane (+86 more)

### Community 9 - "protocol/src/index.ts"
Cohesion: 0.04
Nodes (45): cleanClassName(), controlsDebugSnapshot(), elementDebugSnapshot(), finiteOrNull(), inputDebugSnapshot(), readBuffered(), rectSnapshot(), videoDebugSnapshot() (+37 more)

### Community 10 - "room-session.ts"
Cohesion: 0.04
Nodes (79): lifecycleApi, dynamic, POST(), dynamic, POST(), dynamic, POST(), dynamic (+71 more)

### Community 11 - "room-session-storage.ts"
Cohesion: 0.07
Nodes (86): assertTabId(), clearRoomSessionForClosedTab(), clearRoomSessionForDepartureIfMatch(), clearRoomSessionForDepartureIfMatchNow(), clearRoomSessionIfMatch(), clearRoomSessionIfMatchForTabNow(), clearRoomSessionRecoveryHint(), confirmRoomSessionForTab() (+78 more)

### Community 12 - "social.ts"
Cohesion: 0.06
Nodes (87): AccountOverviewPage(), db(), acceptFriendInviteLink(), acceptFriendRequest(), acceptFriendshipFromInvite(), acceptRoomInvite(), addFriendGroupMember(), archiveFriendGroup() (+79 more)

### Community 13 - "room-client.ts"
Cohesion: 0.04
Nodes (72): BackgroundPushEvent, dispatchPrivilegedRoomRuntimeMessage(), dispatchRoomDepartureRuntimeMessage(), PrivilegedRoomRuntimeDependencies, RemovedRoomTabDependencies, ROOM_ADMISSION_DEPARTURE_SETTLE_TIMEOUT_MS, waitForAdmissionCompletion(), getStoredAuthTokens() (+64 more)

### Community 14 - "pricing-copy.ts"
Cohesion: 0.03
Nodes (65): faq, itemList, metadata, tocHeadings, faq, headings, metadata, SITE_URL (+57 more)

### Community 15 - "popup-app.tsx"
Cohesion: 0.04
Nodes (61): accountErrorState(), accountIdentityChanged(), accountLoadingState(), AccountOwnedState, accountReadyState(), AccountRequestGate, AccountRequestToken, AccountScopeToken (+53 more)

### Community 16 - "watch-history-v3-routes.ts"
Cohesion: 0.04
Nodes (60): AccountWatchLibraryPage(), dynamic, metadata, dynamic, POST, dynamic, POST, dynamic (+52 more)

### Community 17 - "next/server"
Cohesion: 0.07
Nodes (55): dynamic, POST(), dynamic, POST(), dynamic, POST(), dynamic, POST() (+47 more)

### Community 18 - "watch-history.ts"
Cohesion: 0.03
Nodes (72): Dependencies, WatchHistoryCatalogAcknowledgement, WatchItemKindSchema, WatchLibraryResponseSchema, AccountGenerationSchema, addAggregateIssues(), CatalogRequestBaseSchema, DisplayTitleSchema (+64 more)

### Community 19 - "account.ts"
Cohesion: 0.03
Nodes (72): AcceptedRoomInviteResponse, AcceptedRoomInviteResponseSchema, ACCOUNT_RESPONSE_SCHEMA_VERSION, AccountInboxActiveRoomInviteItemSchema, AccountInboxCountsSchema, AccountInboxFriendRequestItemSchema, AccountInboxItemSchema, AccountInboxMissedRoomInviteItemSchema (+64 more)

### Community 20 - "src/types.ts"
Cohesion: 0.05
Nodes (60): getSecret(), isBoundedId(), isBoundedUrl(), isPositiveInteger(), RoomHistoryAttestationClaims, signRoomHistoryAttestation(), signRoomTokenForTest(), verifyRoomToken() (+52 more)

### Community 21 - "api/src/index.ts"
Cohesion: 0.04
Nodes (39): app, closeInvalidRoomFrame(), closeRoomRateLimitedSocket(), consumeParsedRoomEventBoundary(), consumeRoomFrameBoundary(), departureResponse(), detachResponse(), Env (+31 more)

### Community 22 - "logDebug"
Cohesion: 0.10
Nodes (13): logDebug(), RoomSendDisposition, countMatches(), createP2PRtcConfiguration(), createVideoElement(), formatCameraErrorMessage(), mediaElementUsesTrack(), P2PMediaController (+5 more)

### Community 23 - "device-push.ts"
Cohesion: 0.05
Nodes (56): DELETE(), dynamic, dynamic, GET(), maxDuration, POST(), AccountInboxPushResult, defaultDevicePushRepository (+48 more)

### Community 24 - "VideoAdapter"
Cohesion: 0.05
Nodes (34): ContentLifecycleDependencies, ContentLifecycleRuntime, createReactOverlayRenderer(), detectLifecycleResult(), ensurePageStyles(), installMessageComposerKeyboardGuard(), LOCAL_CONTENT_SCRIPT_MATCHES, main() (+26 more)

### Community 25 - "PlaybackSyncController"
Cohesion: 0.10
Nodes (6): isMediaSettling(), PlaybackSyncController, createHarness(), getSyncCorrection(), normalizeRemotePlaybackState(), PlaybackState

### Community 26 - "popup-watch-history.tsx"
Cohesion: 0.06
Nodes (61): HistoryConsent(), PopupHistorySettings(), BrowseMessage, Parser, PopupWatchBrowseRecovery, usePopupWatchBrowse(), detailCursor(), detailMeta() (+53 more)

### Community 27 - "[slug]/page.tsx"
Cohesion: 0.06
Nodes (59): AnimeWithFriendsPage(), buildTitleTag(), buildToc(), dynamic, generateMetadata(), getAnimeBySlug(), getPageLastModified(), Props (+51 more)

### Community 28 - "crm-client.tsx"
Cohesion: 0.06
Nodes (62): addContactAction(), applyImportAction(), CrmActionState, deleteContactAction(), exportCsvDataAction(), exportSurveyLeadsCsvAction(), guard(), ImportPreviewResult (+54 more)

### Community 29 - "social-client.ts"
Cohesion: 0.10
Nodes (63): createWebsiteRoomHeaders(), acceptFriendRequest(), acceptFriendRequestFromApi(), acceptFriendRequestHttpMessage(), acceptInviteHttpMessage(), acceptRoomInvite(), acceptRoomInviteFromApi(), addFriendGroupMember() (+55 more)

### Community 30 - "verifyKreatliCrmSession"
Cohesion: 0.07
Nodes (55): dynamic, failRedirect(), GET(), dynamic, GET(), dynamic, errorToShortString(), failRedirect() (+47 more)

### Community 31 - "RoomDurableObject"
Cohesion: 0.09
Nodes (13): encode(), handleRoomWebSocketMessageBoundary(), RoomDurableObject, roomEndedEvent(), sendAndCloseEndedRoomSockets(), endedRoomTombstone, clearStoredRoomLifecycleAndAlarm(), readEndedRoomTombstone() (+5 more)

### Community 32 - "p2p-media.ts"
Cohesion: 0.04
Nodes (61): addOptionalNumbers(), applyP2PCodecPreferences(), configureSender(), copyDefinedStat(), createP2PMediaSignalDedupeKey(), DEFAULT_STUN_SERVERS, enableP2POpusDtxAndInbandFec(), findMediaTransceiver() (+53 more)

### Community 33 - "oauth-transaction.ts"
Cohesion: 0.07
Nodes (51): dynamic, GET(), dynamic, GET(), dynamic, GET(), dynamic, GET() (+43 more)

### Community 34 - "watch-library.ts"
Cohesion: 0.06
Nodes (61): listRoomMembers(), ProfileRow, PublicProfile, archiveOldestTrackedTitlesOverLimit(), buildWatchLibraryItems(), CHECKPOINT_KINDS, clampProgress(), clampSeconds() (+53 more)

### Community 35 - "overlay-interface-settings.tsx"
Cohesion: 0.05
Nodes (32): AnidachiLogoMark(), AnidachiLogoMarkProps, getDefaultInterfacePreferences(), INTERFACE_PREFERENCES_STORAGE_KEY, INTERFACE_PREFERENCES_VERSION, InterfacePreferencesPatch, InterfacePreferencesV1, isRecord() (+24 more)

### Community 36 - "crunchyroll.content.ts"
Cohesion: 0.07
Nodes (63): BitmovinLikePlayer, BitmovinNamespace, BitmovinPlayerConstructor, BitmovinPlayerMethod, clampMediaTime(), clickElement(), CrunchyrollKatamariPlayer, delay() (+55 more)

### Community 37 - "crunchyroll/progress.ts"
Cohesion: 0.07
Nodes (59): cleanCrunchyrollTitle(), cleanImageUrl(), collectJsonLdSeasonCandidates(), collectJsonLdSeriesCandidates(), CrunchyrollCurrentObjectIdentity, crunchyrollHistoryPolicy, CrunchyrollProgressInput, CrunchyrollSeasonCandidate (+51 more)

### Community 38 - "watch_history_v3_disposable_target.mjs"
Cohesion: 0.06
Nodes (47): cleanup(), measurementsOutput, pageOutput, requireAcceptedCatalog(), sql(), target, sql(), assertAcceptedCatalogMeasurement() (+39 more)

### Community 39 - "overlay-room-rail.tsx"
Cohesion: 0.06
Nodes (38): MainControlVisibility, ParticipantPillVisibility, MainControlPresentation, MainControlRevealPhase, ParticipantPillPresentation, ParticipantRailPresentation, resolveMainControlPresentation(), resolveParticipantPillPresentation() (+30 more)

### Community 40 - "watch-library-client.tsx"
Cohesion: 0.06
Nodes (48): bindWatchHistoryPageRefresh(), buildLaunchUrl(), clampProgress(), deleteConfirmation(), deleteScopeKey(), EpisodeRow(), errorMessage(), formatClock() (+40 more)

### Community 41 - "room-departure-retry.ts"
Cohesion: 0.06
Nodes (39): acknowledgeRoomDepartureAdmissionHandoff(), claimRoomDepartureAdmissionHandoffCleanup(), createRoomDepartureRetryCoordinator(), defaultRoomDepartureRetryCoordinator, departPersistedRoomSession(), drainRoomDepartureRetries(), earliestAttempt(), exactIdentity() (+31 more)

### Community 42 - "private-integration-blob.ts"
Cohesion: 0.06
Nodes (47): dynamic, POST(), clearGoogleAdsTokens(), GOOGLE_ADS_TOKENS_BLOB_PATH, GoogleAdsStoredTokens, mergeGoogleAdsTokens(), readGoogleAdsTokens(), tokenPath() (+39 more)

### Community 43 - "RoomState"
Cohesion: 0.05
Nodes (14): RoomSocketVerifiedIdentity, HostStateUpdateErrorCode, HostStateUpdateResult, LEGACY_ROOM_CAPABILITIES, matchesCanonicalFingerprint(), MediaSeatChangeCode, MediaSeatChangeResult, normalizeRoomSourceUpdate() (+6 more)

### Community 44 - "room-media-defaults.ts"
Cohesion: 0.07
Nodes (36): CAMERA_OPTIONS, MICROPHONE_OPTIONS, resolveOptionIndex(), RoomDefaultControl(), RoomDefaultsSettingsPanel(), RoomDefaultsSettingsPanelProps, CAMERA_ENABLED_PREFERENCE_VERSION, CameraEnabledPreferenceRecord (+28 more)

### Community 45 - "popup-people-model.ts"
Cohesion: 0.05
Nodes (36): AccountInboxItem, buildPopupInboxModel(), buildPopupPeopleModel(), cloneFriend(), cloneGroup(), cloneInboxItem(), cloneProfile(), cloneRecentPerson() (+28 more)

### Community 46 - "plan-entitlements.ts"
Cohesion: 0.08
Nodes (47): CheckoutTier, getOrCreateStripeCustomer(), loginUrlForRequest(), POST(), getBillingCustomerByUserId(), getUserIdByStripeCustomerId(), listSubscriptionsForUser(), updateUserPlan() (+39 more)

### Community 47 - "chrome-extension-demo-async-overlay.tsx"
Cohesion: 0.05
Nodes (34): ActiveWatcher, ASYNC_STEP_LABELS, AsyncDemoOverlayKeyframes(), AsyncDemoOverlayLayer(), BEAT_CAPTIONS, BEAT_TITLES, DemoMode, FRIEND (+26 more)

### Community 48 - "video/prepare/route.ts"
Cohesion: 0.11
Nodes (48): AccountResult, blobUrlToProxyUrl(), POST(), prepareTikTokImages(), publishToIgAccount(), AccountResult, blobUrlToProxyUrl(), maxDuration (+40 more)

### Community 49 - "privileged-overlay-intent.ts"
Cohesion: 0.09
Nodes (45): isAuthMessage(), activeRoomAuthorityClaimExecutionsByTab, assertIntentResponse(), authorityStorageKey(), authorityStorageMutationQueuesByTab, clearPrivilegedOverlayContextForTab(), clearRoomAuthorityClaimIfOwned(), createClaimOwnerId() (+37 more)

### Community 50 - "watch-history-v3.ts"
Cohesion: 0.12
Nodes (53): buildHostAuthoritativeWatchHistoryRoomSource(), buildWatchHistoryTitleEpisodesV3Response(), buildWatchHistoryV3Response(), compareEpisodeRows(), compareObservationDescending(), databaseRowKey(), hasOnlyKeys(), invalidDatabaseResponse() (+45 more)

### Community 51 - "survey-lead.ts"
Cohesion: 0.10
Nodes (36): GET(), GET(), AccountWaitlistStatus, buildResult(), buildSurveyNote(), buildSurveySegments(), creditReferrer(), getAccountWaitlistStatus() (+28 more)

### Community 52 - "watch-history-client.ts"
Cohesion: 0.06
Nodes (41): BrowseCommand, BrowseInput, cancelWatchHistoryCatalogForTab(), defaultWatchHistorySession(), emptyPartition(), flushWatchHistoryInBackground(), handleWatchHistoryHttpMessage(), hasExactKeys() (+33 more)

### Community 53 - "playback-sync-controller.ts"
Cohesion: 0.07
Nodes (31): ActiveAdapterPlaybackOptions, useActiveAdapterPlayback(), isMediaTimeBuffered(), MediaReadyReason, READY_EVENTS, RemoteSeekAttempt, shouldDeferHostStateSeek(), shouldSeekForHostState() (+23 more)

### Community 54 - "extension-session.ts"
Cohesion: 0.09
Nodes (43): createRefreshTokenFamily(), databaseResultOrThrow(), generateRefreshToken(), hashToken(), RefreshChannel, resolveRefreshTokenFamily(), revokeAllRefreshTokenFamiliesForUser(), revokeRefreshTokenFamily() (+35 more)

### Community 55 - "/graphify"
Cohesion: 0.04
Nodes (48): AST and Semantic Merge, AST Structural Extraction, Community Detection, Community Labeling, Corpus Size Guard, Existing Graph Fast Path, Corpus File Detection, GitHub and Multi-Path Merge (+40 more)

### Community 56 - "room-source.ts"
Cohesion: 0.08
Nodes (41): normalizeCreateRoomInput(), dynamic, POST(), persistRoomSource(), assignCleanString(), handleRoomCreateRequestBody(), invalidRequest(), parseRoomCreateRequestBody() (+33 more)

### Community 57 - "room-persistence.ts"
Cohesion: 0.06
Nodes (36): activeRoomLifecycle, emptyRoomLifecycle, emptyRoomRetryAt(), ROOM_LIFECYCLE_STORAGE_KEY, RoomMeterState, activateStoredRoomLifecycle(), claimStoredRoomEndAttempt(), createStoredP2PReplayMetadata() (+28 more)

### Community 58 - "reaction-shortcuts.ts"
Cohesion: 0.07
Nodes (23): ANIDACHI_EMOJI_CATALOG, DockScaleStyle, ReactionShortcutEditor(), ReactionShortcutEditorProps, assignReactionShortcut(), getDefaultReactionShortcutPreferences(), isReactionEmoji(), isRecord() (+15 more)

### Community 59 - "ghost-cam.ts"
Cohesion: 0.08
Nodes (29): GhostCamOptions, GhostCamSession, IncomingP2PSignalSender, p2pSignalMatchesActiveGeneration(), p2pSignalMetadata(), replayPendingP2PSignals(), syncRemoteVoiceParticipant(), useP2PGhostCam() (+21 more)

### Community 60 - "account-inbox.ts"
Cohesion: 0.09
Nodes (38): dynamic, GET(), dynamic, POST(), AccountInboxApiError, AccountInboxCountRow, accountInboxCountsFromRow(), accountInboxDatabaseError() (+30 more)

### Community 61 - "p2p-media-harness.mjs"
Cohesion: 0.09
Nodes (45): node:http, API_DIR, appendWorkerVar(), b64url(), buildWorkerArgs(), bundleHarness(), cleanupHarness(), closeHarnessServer() (+37 more)

### Community 62 - "watch-history-storage.ts"
Cohesion: 0.08
Nodes (38): bindWatchHistoryPreferenceListener(), isRecord(), LocalPreferenceAuthority, preferenceSignature(), StorageChange, StorageChangedEventLike, StorageChangeListener, activeGenerationsFromPartitions() (+30 more)

### Community 63 - "portfolio-audit.ts"
Cohesion: 0.10
Nodes (40): getGoogleMarketingAuthClient(), fetchGa4LandingByChannel(), fetchGa4LandingConversions(), fetchGa4TopPages(), Ga4LandingChannelRow, Ga4LandingConversionRow, Ga4PageRow, resolveGa4PropertyName() (+32 more)

### Community 64 - "room-departure.ts"
Cohesion: 0.09
Nodes (37): handleRemovedRoomTab(), WEB_HTTP_BASE, ConfirmedRoomDepartureOutcome, confirmedRuntimeDeparture(), confirmExplicitRoomDeparture(), ConfirmExplicitRoomDepartureDependencies, departActiveWebsiteRoomFromApi(), departWebsiteRoomFromApi() (+29 more)

### Community 65 - "createWatchHistoryClient"
Cohesion: 0.16
Nodes (45): bestEffortFlushWatchHistoryBeforeSignOut(), browseHardRevision(), browsePath(), browseReadRevision(), browseResponseMeta(), capturePauseKey(), createWatchHistoryClient(), authenticatedRequest() (+37 more)

### Community 66 - "store.ts"
Cohesion: 0.14
Nodes (43): isContactDue(), appendTouch(), assertLocalCrmRuntime(), blobReadText(), blobUpdateText(), ContactMutation, ContactMutationResult, contactsFilePath() (+35 more)

### Community 67 - "participant-disconnect.ts"
Cohesion: 0.13
Nodes (39): acknowledgeParticipantDisconnect(), acknowledgeStoredParticipantDisconnect(), cancelParticipantDisconnectForJoin(), cancelStoredParticipantDisconnectForJoin(), claimDueParticipantDisconnects(), claimDueStoredParticipantDisconnects(), claimParticipantDisconnect(), claimStoredParticipantDisconnect() (+31 more)

### Community 68 - "room-hibernation-runtime.ts"
Cohesion: 0.06
Nodes (16): ConnectParams, connectRoomClient(), makeEmptyAlarmDue(), openRoomSocket(), participant(), readRoomRuntime(), RoomRuntimeSnapshot, roomToken() (+8 more)

### Community 69 - "watch-history-controller.ts"
Cohesion: 0.12
Nodes (39): WatchHistoryCaptureResult, createWatchHistoryController(), applyLocalPreferences(), capture(), dispose(), emitCurrent(), emitRetainedSourceChange(), ensureSessionKey() (+31 more)

### Community 70 - "extension-codes.ts"
Cohesion: 0.08
Nodes (36): dynamic, POST(), dynamic, GET(), consumeExtensionAuthCode(), ConsumeExtensionAuthCodeInput, createExtensionAuthCode(), CreateExtensionAuthCodeInput (+28 more)

### Community 71 - "overlay-layout-editor.tsx"
Cohesion: 0.10
Nodes (39): CHAT_TEXT_SCALE_OPTIONS, createPreviewContext(), FALLBACK_PREVIEW_CONTEXT, finitePositive(), formatChatWidth(), getArrowDelta(), getChatPointerSteps(), getObjectDragPointer() (+31 more)

### Community 72 - "vitest"
Cohesion: 0.08
Nodes (18): ensureGenericSource(), ensureSourceForProvider(), SourceAdapterDefinition, findBestVideo(), findPlayerContainer(), findVideosDeep(), isUsableVideo(), scoreVideo() (+10 more)

### Community 73 - "blob-reconciliation.ts"
Cohesion: 0.09
Nodes (38): assertContact(), BlobHeadResult, BlobReadResult, CONTACT_STATUSES, contentTypeFor(), CrmMeta, digest(), isObject() (+30 more)

### Community 74 - "db.ts"
Cohesion: 0.09
Nodes (38): ACTIVE_ROOM_CONFLICT_MESSAGE, ActiveRoomAssignment, ActiveRoomClaimDatabaseResult, activeRoomConflictResponseInput(), ActiveRoomCreateDatabaseResult, ActiveRoomReleaseDatabaseResult, ActiveRoomSessionDatabaseError, ActiveRoomSummary (+30 more)

### Community 75 - "WatchSourceDescriptor"
Cohesion: 0.09
Nodes (24): NormalizedRoomSourceUpdate, NormalizedWatchSourceDescriptor, AuthoritativeRoomSource, PendingSourceNavigation, PlaybackSyncControllerOptions, CanonicalSourceNavigationResult, DefinitionLookup, navigationRejection() (+16 more)

### Community 76 - "youtube/player-chrome.ts"
Cohesion: 0.10
Nodes (34): AvailableElementRect, childListMutationMayChangeChromeRoots(), childListMutationTouchesChromeRoots(), clampNumber(), getActiveFallbackChromeElements(), getAvailableElementRects(), getAvailableRect(), getAvailableRects() (+26 more)

### Community 77 - "youtube/storage.ts"
Cohesion: 0.09
Nodes (31): dynamic, GET(), OPTIONAL, dynamic, GET(), INSTAGRAM_SCOPES, dynamic, GET() (+23 more)

### Community 78 - "watch-library-routes.test.ts"
Cohesion: 0.10
Nodes (29): dynamic, POST, dynamic, GET, PATCH, dynamic, POST, dynamic (+21 more)

### Community 79 - "middleware.ts"
Cohesion: 0.12
Nodes (31): AUTH_REFRESH_PATH, isStaticOrInternalAssetPath(), shouldAutoRefreshWebsiteSession(), INTERNAL_TOOL_APP_SEGMENTS, INTERNAL_TOOL_ROBOTS_DISALLOW, isInternalToolPath(), isPublicMarketingPath(), needsSessionMiddleware() (+23 more)

### Community 80 - "Watch History Crunchyroll Catalog Progress Implementation Plan"
Cohesion: 0.09
Nodes (40): Adapted Summary Projections, Canonical Deletion Safety, Canonical Crunchyroll Identity, Canonical Logical Identity, Canonical Progress, Canonical Progress Model, Canonical Progress Semantics, Canonical Reads And Deletion (+32 more)

### Community 81 - "Harness"
Cohesion: 0.10
Nodes (11): P2PMediaSessionInput, createRoomConnectionId(), isOlderHistoryBoundary(), RoomClient, RoomConnectionStatus, sameHistoryBoundary(), Harness, HarnessState (+3 more)

### Community 82 - "p2p-media.test.ts"
Cohesion: 0.06
Nodes (19): classifyRemoteVideoActivity(), decideP2PIceRestart(), decideP2PSignalConnection(), getP2PAudioTransceiverDirection(), isPoliteP2PPeer(), p2pAudioTrackSwapNeedsNegotiation(), planP2PVideoSenderSync(), reconcilePeerAction() (+11 more)

### Community 83 - "sync-checkout-session/route.ts"
Cohesion: 0.11
Nodes (30): checkoutSessionUserId(), dynamic, POST(), POST(), sanitizeDiscordHandle(), POST(), beginStripeEventProcessing(), markStripeEventFailed() (+22 more)

### Community 84 - "Task 1 Shared Session and Departure Contracts"
Cohesion: 0.06
Nodes (39): Active Room Conflict Response, Active Room Sessions Table, Atomic Active-room RPCs, Single Active Room Cross-plane Architecture, Room Disconnect Grace Constant, Durable Object Disconnect Authority, Extension Web Locks Local Guard, Graphify Semantic Update (+31 more)

### Community 85 - "room-socket-attachment.ts"
Cohesion: 0.09
Nodes (20): VerifiedRoomToken, ROOM_ADMISSION_JOIN_DEADLINE_MS, ROOM_ADMISSION_PENDING_PER_SUBJECT_LIMIT, RoomAdmission, RoomAdmissionJoinResult, RoomAdmissionResult, RoomAdmissionSocket, attachmentToVerifiedRoomToken() (+12 more)

### Community 86 - "bridge-client.ts"
Cohesion: 0.10
Nodes (27): isControlRequest(), checkHistoryAbort(), cmsPath(), collectCrunchyrollHistoryCatalog(), HistoryJsonLoader, historyRows(), isCrunchyrollControlResult(), resolveCrunchyrollHistoryMetadata() (+19 more)

### Community 87 - "react-dom/client"
Cohesion: 0.07
Nodes (12): rootElement, isWithinOverlayHotkeyBoundary(), OVERLAY_HOTKEY_BOUNDARY_ATTRIBUTE, overlayHotkeyBoundaryProps, OverlayInteractionBoundaryProps, OverlayUnmountCleanupOptions, useOverlayUnmountCleanup(), ISOLATED_EVENT_HANDLERS (+4 more)

### Community 88 - "account-inbox-cache.ts"
Cohesion: 0.11
Nodes (30): ACCOUNT_INBOX_CACHE_VERSION, accountInboxCacheKeyForUser(), accountInboxItemInstanceKey(), CachedAccountInbox, clearCachedAccountInboxForUser(), getCachedAccountInboxForUser(), inboxStructure(), isCanonicalUtcTimestamp() (+22 more)

### Community 89 - "YouTubeVideoAdapter"
Cohesion: 0.08
Nodes (16): AdapterPlaybackSnapshot, clampVolumePercent(), YouTubeVideoAdapter, AD_CONTAINER_SELECTOR, finiteNonNegative(), isVisibleMarker(), MEDIA_EVENTS, positiveFinite() (+8 more)

### Community 90 - "session.ts"
Cohesion: 0.10
Nodes (28): dynamic, POST(), dynamic, GET(), loginRedirectUrl(), POST(), refreshSessionFromCookie(), dynamic (+20 more)

### Community 91 - "connect/page.tsx"
Cohesion: 0.10
Nodes (28): ExtensionConnectMobileConfirm(), dynamic, ExtensionConnectPage(), metadata, Props, dynamic, ERROR_MESSAGES, LoginPage() (+20 more)

### Community 92 - "pseo-new-guides.tsx"
Cohesion: 0.08
Nodes (21): metadata, metadata, metadata, metadata, metadata, metadata, metadata, metadata (+13 more)

### Community 93 - "api/src/room-lifecycle.ts"
Cohesion: 0.08
Nodes (30): boundedInternalWebCallbackTimeout(), fetchAndReadJsonWithBoundedTimeout(), INTERNAL_WEB_CALLBACK_TIMEOUT_MS, internalWebCallbackConfig(), InternalWebLifecycleEnv, isLoopbackHostname(), notifyWebParticipantDeparted(), notifyWebRoomEnded() (+22 more)

### Community 94 - "diagnostic-log.ts"
Cohesion: 0.09
Nodes (35): ANIDACHI_BUILD_ID, API_HTTP_BASE, API_WS_BASE, VOICE_KEYWORD_EMOJI, WXT_VAPID_PUBLIC_KEY, clearDiagnosticsFromPage(), compactDiagnosticPageData(), compactPageDebug() (+27 more)

### Community 95 - "scripts"
Cohesion: 0.05
Nodes (37): scripts, build, build:extension:icons, build:extension:public, build:extension:staging, build:extension:staging:local-broad, check, check:extension:icons (+29 more)

### Community 96 - "room-source-persistence.ts"
Cohesion: 0.10
Nodes (26): PARTICIPANT_DISCONNECT_STORAGE_KEY, ParticipantDisconnectState, RoomLifecycleState, acknowledgeStoredRoomSourceAttempt(), claimStoredRoomSourceAttempt(), enqueueStoredRoomSource(), ensureStoredRoomSourcePending(), hasExactKeys() (+18 more)

### Community 97 - "devDependencies"
Cohesion: 0.06
Nodes (36): devDependencies, happy-dom, @types/chrome, @types/react, @types/react-dom, typescript, vitest, wxt (+28 more)

### Community 98 - "getSession"
Cohesion: 0.09
Nodes (27): AccountFriendsPage(), dynamic, metadata, AccountLayout(), dynamic, metadata, dynamic, GET() (+19 more)

### Community 99 - "invites-client.tsx"
Cohesion: 0.10
Nodes (28): AcceptInviteResponse, AccountInboxItem, acknowledgeInboxPageSeen(), ActiveRoomInvite, Avatar(), formatDate(), FriendRequestRow(), InboxFriendRequest (+20 more)

### Community 100 - "AniDachi New Chat Project Context"
Cohesion: 0.07
Nodes (34): AniDachi Contributor Startup Contract, Feature to Staging to Production Git Flow, Graphify Development Workflow, Layered Contributor Instructions, Repository Security Boundaries, Account-Scoped Local-First Extension Cache, AniDachi New Chat Project Context, Current Product Focus (+26 more)

### Community 101 - "best-anime-movies-to-watch-with-friends/page.tsx"
Cohesion: 0.05
Nodes (25): faq, headings, itemList, metadata, SITE_URL, faq, headings, itemList (+17 more)

### Community 102 - "Account Data Watch History Social Inbox Foundation Design"
Cohesion: 0.08
Nodes (34): Room Invites, Acceptance Criteria, Account Data Watch History Social Inbox Foundation Design, Account Data, Watch History, Social, And Inbox Foundation Design, Compatibility With The Invite Runtime, Durable Account Inbox, Existing Foundation To Reuse, Fixed Product Decisions (+26 more)

### Community 103 - "send-connection-requests.mjs"
Cohesion: 0.14
Nodes (34): node:process, node:readline/promises, batchLabel(), BLOCKER_PATTERNS, BLOCKER_SCOPED_SELECTORS, clickActionsButton(), closeOpenOverlays(), confirmBatch() (+26 more)

### Community 104 - "src/account-inbox-client.ts"
Cohesion: 0.12
Nodes (28): accountInboxFromBridge(), accountInboxHttpError(), AccountInboxHttpMessage, AccountInboxHttpMessageResponse, AccountInboxUnauthorizedError, decodeAccountInboxResponse(), handleAccountInboxHttpMessage(), isAccountInboxHttpMessage() (+20 more)

### Community 105 - "overlay-layout-model.ts"
Cohesion: 0.10
Nodes (31): GhostCamSizeStep, ResolvedChatLayout, ResolvedVideoLayout, clampInteger(), DEFAULT_LAYOUT, getDefaultOverlayLayoutPreferencesV2(), isRecord(), normalizeCameraSizeStep() (+23 more)

### Community 106 - "getUserById"
Cohesion: 0.18
Nodes (29): dynamic, maxDuration, POST(), dynamic, POST(), activeRoomConflictResponse(), claimActiveRoomSession(), createRoomWithActiveSession() (+21 more)

### Community 107 - "[roomId]/page.tsx"
Cohesion: 0.11
Nodes (24): dynamic, POST(), wantsJson(), dynamic, GET(), ExtensionCheck(), dynamic, Props (+16 more)

### Community 108 - "overlay-layout-engine.ts"
Cohesion: 0.14
Nodes (32): CHAT_TEXT_METRICS, clampAxisPosition(), clampInteger(), clampRectToSafeRect(), ContactAxis, createCameraPriorityFallback(), createChatLayout(), createMinimumFallback() (+24 more)

### Community 109 - "crunchyroll/player-chrome.ts"
Cohesion: 0.10
Nodes (27): areCrunchyrollPlayerChromeStatesEqual(), clampNumber(), CRUNCHYROLL_CONTROL_SELECTOR, CRUNCHYROLL_CONTROL_SELECTORS, CrunchyrollPlayerChromeState, DEFAULT_CAM_STACK_BOTTOM_PX, DEFAULT_CRUNCHYROLL_PLAYER_CHROME_STATE, DEFAULT_MINI_PANEL_RIGHT_PX (+19 more)

### Community 110 - "dependencies"
Cohesion: 0.06
Nodes (32): @amplitude/unified, dependencies, @amplitude/unified, class-variance-authority, clsx, google-auth-library, googleapis, jose (+24 more)

### Community 111 - "popup-watch-history.test.tsx"
Cohesion: 0.11
Nodes (24): subscribeToPopupWatchHistorySnapshot(), createListWatchHistoryMessage(), clientFixture(), findButton(), findInput(), fixtureBrowseDetail(), fixtureBrowseEpisodes(), fixtureBrowseTitles() (+16 more)

### Community 112 - "youtube/api.ts"
Cohesion: 0.13
Nodes (26): clearStateCookie(), dynamic, GET(), getOrigin(), dynamic, GET(), ensureAllCredentials(), ensureAuthClient() (+18 more)

### Community 113 - "post/status/route.ts"
Cohesion: 0.17
Nodes (27): dynamic, GET(), safeRefreshTiktok(), GET(), processInstagramAccount(), processTikTokAccount(), GET(), processInstagramReel() (+19 more)

### Community 114 - "migrate-private-integration-blobs.ts"
Cohesion: 0.10
Nodes (26): assertPrivatePath(), isPrivateIntegrationBlobPath(), readResult(), stream(), streamingReadResult(), assertReadableResult(), authFromEnvironment(), BlobAuth (+18 more)

### Community 116 - "node:assert/strict"
Cohesion: 0.10
Nodes (12): alt, contentType, runtime, size, files, user, ModuleLoader, node:assert/strict (+4 more)

### Community 117 - "AniDachi Core Foundation to UI/UX Handoff Plan"
Cohesion: 0.09
Nodes (31): Atomic Invite Response, Bounded Watch History, Bounded Watch History Read Contract, Canonical Durable Room Source, Canonical Room Source Contract, Coalesced Source Retry, Completed Staging Foundation, AniDachi Core Foundation to UI/UX Handoff Plan (+23 more)

### Community 118 - "ice-servers.ts"
Cohesion: 0.11
Nodes (26): CachedCloudflareIceServers, clearIceServersCacheForTest(), cloneIceServersPayload(), CloudflareIceServersResponse, createIceServersPayload(), dropEmptyIceServers(), FALLBACK_ICE_SERVERS, filterBrowserBlockedTurnUrls() (+18 more)

### Community 119 - "instagram/storage.ts"
Cohesion: 0.12
Nodes (24): clearStateCookie(), dynamic, GET(), getOrigin(), ShortLivedTokenResponse, POST(), GET(), ensureCredentials() (+16 more)

### Community 120 - "anidachi-auth/watch-history-browse.ts"
Cohesion: 0.11
Nodes (22): dynamic, GET, dynamic, GET, dynamic, GET, dynamic, GET (+14 more)

### Community 121 - "debug-log.ts"
Cohesion: 0.14
Nodes (25): clearDebugLog(), compactDebugData(), compactInputData(), compactVideoData(), copyDebugFields(), DebugEntry, entries, getCompactDebugLogText() (+17 more)

### Community 122 - "feature-requests.ts"
Cohesion: 0.14
Nodes (22): POST(), CATEGORY_LABELS, buildEmail(), clientIp(), FeatureRequestPostDependencies, handleFeatureRequestPost(), rateBucket, rateLimited() (+14 more)

### Community 123 - "anidachi-seo-aeo-pages.md"
Cohesion: 0.07
Nodes (26): Addy Osmani / technical SEO gate, AEO (answer engines), Conversion and CTAs, Crunchyroll anti-cannibalization map (owned queries), Crunchyroll conversion checklist (required), Crunchyroll conversion stack (required), Crunchyroll Keyword Planner gate (before locking URLs), Crunchyroll measurement note (+18 more)

### Community 124 - "rules"
Cohesion: 0.07
Nodes (28): noLabelWithoutControl, useButtonType, useKeyWithClickEvents, useSemanticElements, useValidAnchor, useExhaustiveDependencies, files, ignoreUnknown (+20 more)

### Community 125 - "Anidachi Auth Integration Implementation Plan"
Cohesion: 0.07
Nodes (28): Anidachi Auth Integration Implementation Plan, Authenticated Room Flow, Branch, Backup, and Commit Strategy, Current Research Snapshot, Database Migration Design, Extension Files to Add, Extension Files to Modify, Extension Login Flow (+20 more)

### Community 126 - "Social Rooms Subscriptions Execution Plan"
Cohesion: 0.09
Nodes (29): API Surface, Billing, Billing Tables, Core Product Rules, Dashboard Aggregates, Database Model, Devices, Devices And Web Push Subscriptions (+21 more)

### Community 127 - "protocol/src/watch-history-browse.ts"
Cohesion: 0.07
Nodes (27): filters, GroupContext, Key, Provider, Timestamp, WatchHistoryBrowseDatabaseOptionsSchema, WatchHistoryBrowseDatabasePageSchema, WatchHistoryBrowseGroupSchema (+19 more)

### Community 128 - "voice-audio-preferences.ts"
Cohesion: 0.15
Nodes (25): MicrophoneIntent, shouldPublishMicrophone(), ParticipantAudioContourControl(), ParticipantAudioControlProps, ParticipantAudioInlineControl(), ParticipantMuteButton(), applyParticipantAudioSliderValue(), clampParticipantAudioVolume() (+17 more)

### Community 129 - "catalog.ts"
Cohesion: 0.15
Nodes (25): array(), Availability, boundedString(), classifyAvailability(), compareOrder(), compatibleSeason(), CrunchyrollCatalogInput, CrunchyrollCatalogPartialReason (+17 more)

### Community 130 - "privileged-overlay-wiring.test.tsx"
Cohesion: 0.11
Nodes (19): confirmedRoomSession(), createAdapter(), extensionStorage, flushRoomActionWork(), guestParticipant(), hostParticipant(), installActiveHostRoomRuntime(), installOverlayRuntime() (+11 more)

### Community 131 - "Watch Drawer Browse Local Verification"
Cohesion: 0.08
Nodes (24): Durable Model And Convergence, Historical v2 Evidence, Historical v2 Read Boundary, Local Watch Drawer Browse Candidate, Product Surface, Providers, Rollout And Rollback, Runtime Ownership (+16 more)

### Community 132 - "Social Rooms, Friends, Groups, And Subscriptions Execution Plan"
Cohesion: 0.07
Nodes (27): Access Rules, API Tests, Billing And Entitlements, Checkout Requirements, Current Gaps To Close, Dashboard Shell, Devices And Notifications, Done Means (+19 more)

### Community 133 - "Watch History v3 Local Verification"
Cohesion: 0.12
Nodes (28): Catalog Snapshots And Aliases, Localized Provider Labels, Accepted Large Catalog Benchmark, Canonical Schema 3 Storage, Controlled Small Catalog Benchmark, Coordinated Activation Order, Disposable Target Guards, Extension Storage Transition (+20 more)

### Community 134 - "Watch History Catalog And Progress Design"
Cohesion: 0.12
Nodes (28): Clean Start Test History Decision, Atomic Deletion Semantics, Bounded Catalog Snapshot, Canonical Durable Data Model, Clean Start Amendment, Consistent History Product Goal, Crunchyroll Provider Policy, Expanded Season (+20 more)

### Community 135 - "tasks"
Cohesion: 0.08
Nodes (27): ^build, dist/**, !.next/cache/**, .output/**, dependsOn, outputs, dependsOn, outputs (+19 more)

### Community 136 - "hotkeys.ts"
Cohesion: 0.14
Nodes (22): getEmojiHotkey(), getHotkeyAction(), hasBlockedModifier(), HotkeyAction, HotkeyEventLike, HotkeyState, isEditableElement(), isEditableEventTarget() (+14 more)

### Community 137 - "reaction-pop.tsx"
Cohesion: 0.10
Nodes (22): LiveChatMessage, VisibleReaction, clamp(), findReactionAnchor(), REACTION_IDENTITY_CUE_DURATION_MS, REACTION_MOTION_PROFILES, REACTION_VISIBLE_DURATION_MS, ReactionAnchor (+14 more)

### Community 138 - "node:fs"
Cohesion: 0.08
Nodes (19): providerDirectories, sharedOverlayRuntimeFiles, sourceAdaptersDirectory, sourceDirectory, AUTHORITY_EXPIRY_MIGRATION_URL, CUTOVER_MIGRATION_URL, functionDefinition(), MIGRATION_URL (+11 more)

### Community 139 - "tiktok/storage.ts"
Cohesion: 0.14
Nodes (22): clearCookies(), dynamic, GET(), getOrigin(), POST(), GET(), compat, __dirname (+14 more)

### Community 140 - "Main Repository Monorepo Migration Implementation Plan"
Cohesion: 0.07
Nodes (26): Commit Strategy, Current Status, Definition Of Done, Environment Model, Execution Order, Known Migration Risks And Mitigations, Main Repository Monorepo Migration Implementation Plan, Non-Negotiable Rules (+18 more)

### Community 141 - "AniDachi Pre-release Security and Reliability Readiness Plan"
Cohesion: 0.13
Nodes (27): Active Foundation Scope Transfer, Browser OAuth Transaction Binding, Closure Without Full Completion, Database Implementation Contract, Deferred Pre-public Hardening, Execution Drift Gate, Existing System Planes, Extension Client Binding and PKCE (+19 more)

### Community 142 - ".acquireMicrophoneTrack"
Cohesion: 0.16
Nodes (5): createLocalAudioLevelMeter(), LocalAudioLevelMeter, classifyMicrophoneTerminalFailure(), formatMicrophoneErrorMessage(), microphoneErrorName()

### Community 143 - "artwork-select.ts"
Cohesion: 0.16
Nodes (23): getCrunchyrollApiUrl(), getCrunchyrollContentToken(), getCrunchyrollLocale(), getCrunchyrollOrigin(), isCrunchyrollPage(), loadCrunchyrollCmsObject(), loadCrunchyrollPosterArtwork(), loadCrunchyrollPosterFromMainWorld() (+15 more)

### Community 144 - "sitemap-discovery.ts"
Cohesion: 0.13
Nodes (23): sitemap(), guideLinks, APP_DIR, discoverStaticSitemapUrlPaths(), EXCLUDED_TOP_LEVEL, EXCLUDED_URL_PATHS, inferSitemapMeta(), isDynamicSegment() (+15 more)

### Community 145 - "Watch History Room Authority Threat Model"
Cohesion: 0.08
Nodes (25): 1. Overview, 2. Persisted Lifecycle Audit, 3. Approved Attestation Contract, 4. Verification Order, 5. Threats, Mitigations, and Residual Risk, 6. Severity Calibration and Gate Decision, Account fences and self-only writes, Actors (+17 more)

### Community 146 - "include"
Cohesion: 0.08
Nodes (23): @cloudflare/workers-types, compilerOptions, types, extends, include, src, test, compilerOptions (+15 more)

### Community 147 - "user-identity.ts"
Cohesion: 0.15
Nodes (20): createAuthMessage(), sendAuthCommand(), adoptWebsiteSessionWithRetry(), DEFAULT_SILENT_ADOPTION_DELAYS_MS, isTerminalIdentityResult(), report(), SilentSessionAdoptionOptions, SilentSessionAdoptionReason (+12 more)

### Community 148 - "p2p-ice.ts"
Cohesion: 0.17
Nodes (23): buildIceServersRequest(), CachedIceServers, cachedIceServersByScope, clearP2PIceServersCacheForTest(), cloneIceServers(), dedupeIceServers(), getIceServerUrls(), hasTurnServer() (+15 more)

### Community 149 - "contact-messages.ts"
Cohesion: 0.15
Nodes (19): POST(), CATEGORY_LABELS, ContactForm(), CONTACT_CATEGORIES, CONTACT_MESSAGE_SEGMENT, ContactCategory, ContactMessageRecord, isContactCategory() (+11 more)

### Community 150 - "Commercial Room, P2P, and Watch Progress Architecture Implementation Plan"
Cohesion: 0.08
Nodes (24): Architecture Boundaries, Cloudflare Worker + Durable Object / Live Plane, Commercial Room, P2P, and Watch Progress Architecture Implementation Plan, Commit Strategy, Current Status, Definition Of Done, Durable Room Lifecycle, Execution Order (+16 more)

### Community 151 - "Production Room, Realtime, and P2P Hardening Roadmap"
Cohesion: 0.12
Nodes (25): Durable Room Lifecycle and Idempotent Create, Durable Shared Watch Progress, Explicit Room Protocol Contract, Manual Staging Release Gate, Ordered Hardening Program, Participant Session Identity, Production Room, Realtime, and P2P Hardening Roadmap, Short-Lived Room Token Authority (+17 more)

### Community 152 - "Source Adapter Architecture Implementation Plan"
Cohesion: 0.08
Nodes (24): Core Contracts, Definition Of Done, Global Constraints, Non-Goals, PR 1: Behavior-Preserving Provider Extraction, PR 2: Lifecycle, Capabilities, And First-Class YouTube, Provider And Room Isolation Invariants, Registry Rules (+16 more)

### Community 153 - "Canonical Runtime Flow"
Cohesion: 0.10
Nodes (25): Account History Generation Fencing, Account-Scoped Local Cache, Authenticated Watch History v2 Web Service, Bounded-Shape Watch History Outbox, Canonical Runtime Flow, Canonical Watch History v2 Read Model, Cursor-Bounded Title Projection, Durable History Deletion Fences (+17 more)

### Community 154 - "use-camera-interaction-lock.ts"
Cohesion: 0.13
Nodes (16): PixelRect, OverlayLayoutRuntimeContextInput, PlayerOverlayInsets, CAMERA_INTERACTION_APPROACH_TIMEOUT_MS, CAMERA_INTERACTION_RELEASE_DELAY_MS, CameraInteractionLockState, CameraInteractionPlayerGeometry, equalInsets() (+8 more)

### Community 155 - "voice-activity.ts"
Cohesion: 0.13
Nodes (13): audioActivityStatsChanged(), classifyPeerHealth(), getAudioTrackFromElement(), summarizeStats(), AUDIO_QUIET_SAMPLES_BEFORE_CLEAR, AUDIO_SPEECH_LEVEL_THRESHOLD, AudioActivityStats, AudioReceiverTrackState (+5 more)

### Community 156 - "study.ts"
Cohesion: 0.17
Nodes (22): cleanClassName(), elementSnapshot(), EVENT_NAMES, finite(), finiteOrNull(), getCrunchyrollStudySnapshot(), getElementPath(), getRect() (+14 more)

### Community 157 - "scripts"
Cohesion: 0.08
Nodes (23): name, private, scripts, blob:migrate:private, build, cache:jikan, check, crm (+15 more)

### Community 158 - "compilerOptions"
Cohesion: 0.08
Nodes (23): compilerOptions, allowJs, esModuleInterop, incremental, isolatedModules, jsx, module, moduleResolution (+15 more)

### Community 159 - "Anidachi Architecture and Stack Notes"
Cohesion: 0.08
Nodes (23): Anidachi Architecture and Stack Notes, Cloudflare Deploy, Current Stack, Extension, Fullscreen Overlay Decision, Ghost Cam And Audio, Hotkeys, Identity and Invite Flow (+15 more)

### Community 160 - "overlay-layout-runtime.ts"
Cohesion: 0.23
Nodes (21): CAMERA_INTERACTION_CORRIDOR_PADDING_PX, clamp(), createOverlayLayoutRuntimeContext(), finiteNonNegative(), getBubbleGapPx(), getCameraInteractionCorridor(), getOverlayLayoutCameraSlotCount(), getOverlayLayoutReservedRects() (+13 more)

### Community 161 - "extension/test/watch-history-browse.test.ts"
Cohesion: 0.13
Nodes (18): BrowseCacheStorage, browserStorage, createWatchHistoryBrowseCache(), initialize(), prune(), Entry, WATCH_BROWSE_FRESH_MS, WATCH_BROWSE_MAX_AGE_MS (+10 more)

### Community 162 - "watch-anime-together/page.tsx"
Cohesion: 0.11
Nodes (19): AnimeWatchPartyPage(), faq, metadata, pillarItemList, SITE_URL, tocHeadings, AnimeWatchPartyToolkitPage(), faq (+11 more)

### Community 163 - "Watch History Local Read Implementation Plan"
Cohesion: 0.11
Nodes (23): Feature PR to staging and tested production promotion, Graphify knowledge graph maintenance, Real-network P2P and TURN acceptance boundary, Development quality gates, Single active room session invariant, Safe site-only automatic promotion, Stable drawer read integration, Exact-query initial episode previews (+15 more)

### Community 164 - "Overlay Layout Engine V2 Design"
Cohesion: 0.09
Nodes (22): Camera Group, Chat, Clean Version 2 Start, Code Boundaries, Component Tests, Delivery Stages, Draft And Apply, Failure Handling (+14 more)

### Community 165 - "room-signaling-harness.mjs"
Cohesion: 0.17
Nodes (15): API_DIR, b64url(), Client, __dirname, main(), playbackStateFor(), PORT, rawConnect() (+7 more)

### Community 166 - "RecentP2PSignalBuffer"
Cohesion: 0.16
Nodes (8): addP2PSignalForDispatch(), AddP2PSignalResult, BufferedP2PSignalEvent, getP2PSignalDedupeKey(), matchesGenerationScope(), P2PSignalReplayScope, RecentP2PSignalBuffer, StoredP2PReplayMetadata

### Community 167 - "Kreatli CRM Data Schema"
Cohesion: 0.13
Nodes (22): Kreatli CRM Agent Instructions, Conservative Outreach Governance, CRM CLI Workflow, Human-controlled Sending, Personal Outreach Source of Truth, UTC Queue Interpretation, Contact Import Modes, Contact Record (+14 more)

### Community 168 - "Current Development State"
Cohesion: 0.09
Nodes (22): Account Read Contracts And Popup Isolation, Branches And Protection, Core Foundation UI/UX Handoff, Current Development State, Current Product Behavior, Development Quality System, Documentation Map, Extension Channels (+14 more)

### Community 169 - "Watch History v3 Staging Verification"
Cohesion: 0.14
Nodes (22): Extension Rollback, History Only Forward Fix Rollback, OAuth And Environment Rollback, Release And Rollback Runbook, Release Incident Runbook, Supabase Rollback, Watch History v3 Coordinated Transition, Web Rollback (+14 more)

### Community 170 - "Watch History v2 Clean MVP Implementation Plan"
Cohesion: 0.09
Nodes (22): Corrected Decision Ledger, Current External Constraints Rechecked For This Plan, Data And Conflict Rules, Deferred Catalog Evidence Gate (Not Part Of Core MVP Execution), Execution Waves And Mandatory Stops, Failure And Account-Switch Behavior, Final MVP Boundary, Global Constraints (+14 more)

### Community 171 - "Participant"
Cohesion: 0.15
Nodes (15): getMediaAction(), MediaActionInput, orderRoomParticipants(), PanelCameraControl(), PanelCameraControlProps, participantInitials(), participantMediaStatus(), participantOrderRank() (+7 more)

### Community 172 - "popup-watch-browse.test.tsx"
Cohesion: 0.13
Nodes (18): PopupWatchHistoryPanel(), isNewerThanCanonicalHistory(), loadConfirmedPopupWatchHistorySnapshot(), normalizeCachedWatchHistoryResponse(), selectConfirmedPopupWatchHistorySnapshot(), aggregate, browse(), button() (+10 more)

### Community 173 - "background-privileged-room-route.test.ts"
Cohesion: 0.13
Nodes (14): cleanupRoomAdmissionHandoffForTab(), connectRoomHttpMessage(), RoomAdmissionHandoff, captureRoomSessionIdentity(), createDepartureRetryScheduler(), createDepartureRetryStorage(), createSessionStorage(), deferred() (+6 more)

### Community 174 - "youtube/progress.ts"
Cohesion: 0.16
Nodes (15): HistoryPolicyInput, isValidHistoryMedia(), normalizeHistoryUrl(), ProviderPlaybackMetadata, SourceAdapterHistoryPolicy, canonicalSourceUrl(), canonicalYouTubeHistoryUrl(), cleanYouTubeProgressVideoId() (+7 more)

### Community 175 - "subscribe-interest-route.ts"
Cohesion: 0.15
Nodes (16): POST(), dynamic, KreatliCrmPage(), listContactMessages(), parseContactMessages(), listFeatureRequests(), parseFeatureRequests(), isGmailConfigured() (+8 more)

### Community 176 - "friends-client.tsx"
Cohesion: 0.12
Nodes (18): addOptimisticMember(), Avatar(), CurrentUser, EMPTY_FRIENDS, formatRecentMeta(), FriendGroup, FriendInviteLinkResponse, FriendListItem (+10 more)

### Community 177 - "public.browse_watch_history_v3"
Cohesion: 0.10
Nodes (16): public.apply_watch_progress_v3(), public.browse_watch_history_v3(), public.profiles, public.user_watch_settings, public.users, public.watch_episode_progress, public.watch_session_participants, public.watch_sessions (+8 more)

### Community 178 - "Anidachi Project Operating Manual"
Cohesion: 0.10
Nodes (21): Anidachi Project Operating Manual, Development Flow, Development Startup Checklist, Documents To Read First, Everyday Development Loop, Extension Channels, How Future Watch Progress Should Fit, How Joining Works (+13 more)

### Community 179 - "Development Flow Quality System Plan"
Cohesion: 0.10
Nodes (20): Block 0 - Stabilize The Working Baseline, Block 10 - Integrate The Flow Into Product Planning, Block 1 - Add One Canonical Agent Entry Point, Block 2 - Configure CodeRabbit As A Contextual Reviewer, Block 3 - Build A Local Project Knowledge Graph, Block 4 - Formalize PR Templates And Review Checklists, Block 5 - Make Local Verification Easy And Standard, Block 6 - Strengthen CI Without Blocking Fast Iteration (+12 more)

### Community 180 - "LinkedIn Sales Navigator Connection Requests"
Cohesion: 0.10
Nodes (20): Architecture, CLI flags, Common skip reasons, Exit codes, Files, `launch-chrome.sh`, LinkedIn Sales Navigator Connection Requests, Outcomes (+12 more)

### Community 181 - "Project Planes"
Cohesion: 0.12
Nodes (20): Contract First for Cross-Plane Changes, Project Planes, API Review Scope, Extension Review Scope, GitHub Actions Review Scope, CodeRabbit Path Filters, Plan Review Scope, Protocol Review Scope (+12 more)

### Community 182 - "dependencies"
Cohesion: 0.10
Nodes (20): dependencies, @anidachi/protocol, hono, jose, jose, dependencies, @anidachi/protocol, lucide-react (+12 more)

### Community 183 - "Anidachi Project Architecture And Development"
Cohesion: 0.10
Nodes (20): Extension Popup Shell, Anidachi Project Architecture And Development, Branch Model, Current Stack, Documentation Maintenance, Environments, Extension Runtime, High-Level System (+12 more)

### Community 184 - "AniDachi Extension Icon 48px"
Cohesion: 0.13
Nodes (20): AniDachi Extension Icon, Blue Purple Gradient Badge, Browser Extension Brand Identity, Dark Rounded Square Canvas, Stylized A Mark, AniDachi Extension Icon 16px, White Stylized A Brand Mark, Extension Toolbar Icon (+12 more)

### Community 185 - "room-invite-target-status.ts"
Cohesion: 0.17
Nodes (16): finalizeStatuses(), GROUP_STATUS_ORDER, isNewerRecipientEntry(), mergeInviteRecipients(), mergeRecipientEntry(), mergeRoomInviteTargetStatus(), mergeTargetRecipients(), RecipientStatusEntry (+8 more)

### Community 186 - "public-media-blob.ts"
Cohesion: 0.15
Nodes (18): GET(), Blob200, Blob304, contentTypesForExtension(), discardStream(), hasUnsafePathEncoding(), isCanonicalDate(), JPEG_CONTENT_TYPES (+10 more)

### Community 187 - "protocol/package.json"
Cohesion: 0.10
Nodes (19): typescript, typescript, dependencies, zod, devDependencies, typescript, vitest, exports (+11 more)

### Community 188 - "watch_history_v3_catalog_read_states_contract.mjs"
Cohesion: 0.11
Nodes (13): begin(), cases, commit(), failed, first, foreign, omitted, outputPath (+5 more)

### Community 189 - "Development Workflow Hardening Implementation Plan"
Cohesion: 0.10
Nodes (19): Acceptance Criteria, Development Workflow Hardening Implementation Plan, Evidence Snapshot, File Map, Recommended Execution Order, Target Operating Model, Task 0: Prepare A Safe Implementation Branch, Task 10: Update Active Documentation As Source Of Truth (+11 more)

### Community 190 - "Plan Code Canonicalization And Billing Cleanup Implementation Plan"
Cohesion: 0.10
Nodes (19): Completion Criteria, Current Problems, Evidence Reviewed, File Structure, Hard Rules, Plan Code Canonicalization And Billing Cleanup Implementation Plan, Rollback Plan, Task 0: Branch And Baseline Hygiene (+11 more)

### Community 191 - "linkedin/package.json"
Cohesion: 0.10
Nodes (18): esbuild, playwright, dependencies, playwright, name, private, scripts, chrome (+10 more)

### Community 192 - "api/package.json"
Cohesion: 0.11
Nodes (18): devDependencies, @cloudflare/vitest-pool-workers, typescript, vitest, wrangler, vitest, name, private (+10 more)

### Community 193 - "overlay-mount.ts"
Cohesion: 0.22
Nodes (15): getOverlayMountDecision(), getOverlayPageDecision(), isOverlayAllowedOnPage(), mutationsAffectVideo(), nodeContainsVideo(), OverlayMountDecision, OverlayPageDecision, shouldRefreshSameVideoAdapter() (+7 more)

### Community 194 - "YouTube Playback Synchronization Hardening Implementation Plan"
Cohesion: 0.11
Nodes (18): Current Verified Baseline, Definition Of Done, External Constraints Verified On 2026-07-23, Global Constraints, Product Decisions, Provisional Interfaces And Dependency Direction, Rollback Boundaries, Status And Relationship To Existing Plans (+10 more)

### Community 195 - "dev-check.mjs"
Cohesion: 0.15
Nodes (17): args, changedFiles(), classify(), commands, dedupe(), fail(), files, isDocs() (+9 more)

### Community 196 - "release-channel-build.test.ts"
Cohesion: 0.12
Nodes (11): artifactText(), broadPatterns, expectCanonicalRuntime(), hostileEnvironment, localHostPermissions, Manifest, productionHostPermissions, repoRoot (+3 more)

### Community 197 - "components.json"
Cohesion: 0.11
Nodes (17): aliases, components, hooks, lib, ui, utils, iconLibrary, rsc (+9 more)

### Community 198 - "watch-history-v3-routes.test.ts"
Cohesion: 0.15
Nodes (11): encodeWatchHistoryCursor(), deletion, dependencies(), unexpected(), createWatchHistoryV3RouteHandlers(), catalogBeginBody(), catalogCommitBody(), catalogContext() (+3 more)

### Community 199 - "client.ts"
Cohesion: 0.19
Nodes (15): buildSeed(), generateKeywordIdeas(), GenerateKeywordIdeasInput, getAccessToken(), getCustomerId(), getDeveloperToken(), KeywordIdeaResult, normalizeCustomerId() (+7 more)

### Community 200 - "Survey → Subscription Conversion (Planner Notes)"
Cohesion: 0.11
Nodes (18): Async Mode Demo — landing page (2026-07-07), Converting mechanism goals (subscription purchase), Current Status / Progress Tracking (2026-05-12), Force-Index Sitemap + Noindex Cleanup (2026-07-17), Hero extension demo overlay restyle (2026-07-26), High-converting SEO batch (2026-07-19) — Keyword Planner validated, High-converting SEO batch 2 (2026-07-22) — Keyword Planner validated, High-impact survey improvements (ideas) (+10 more)

### Community 201 - "Crunchyroll Adapter Notes"
Cohesion: 0.14
Nodes (18): Active Player And Detail Page States, Active Player Waiting State, Bounded MAIN World Metadata Bridge, Catalog Completeness Evidence, Catalog Revision And Context Fences, Crunchyroll Control Fallbacks, Crunchyroll Adapter Notes, Crunchyroll Fullscreen Parent (+10 more)

### Community 202 - "Task 2: view-local owner-validated extension browse client"
Cohesion: 0.14
Nodes (18): Extension Session Ownership, Task 1: durable invitation provenance and bounded server browse, Task 4: local integration evidence and controller handoff, Task 2: view-local owner-validated extension browse client, WatchHistoryBrowseQuery and WatchHistoryBrowseResponse, My groups association belongs only to the group owner, Authenticated group-invitation provenance with immutable name snapshot, Local calendar ranges with exclusive next-midnight UTC end (+10 more)

### Community 203 - "Anidachi Development Environments"
Cohesion: 0.11
Nodes (18): Anidachi Development Environments, API Environments, Branch Model, Extension Builds, Fast Local Extension Development, Local Toolchain, OAuth Redirects, P2P Scorecard (+10 more)

### Community 204 - "Invitation Delivery Reliability Implementation Plan"
Cohesion: 0.17
Nodes (18): Invitation Outbox Recovery, Automatic Staging Recovery Evidence, Independent Worker Recovery Drain, Invitation Delivery Reliability Implementation Plan, Open Popup Inbox Convergence, Ordered Staging Cutover, Outer Statement Timeout, Pending Request Suppression (+10 more)

### Community 205 - "OpenClaw: YouTube Shorts posting"
Cohesion: 0.11
Nodes (17): Authentication, Caption rules (YouTube), Error codes, Examples, Form fields, Human setup (one-time, not via OpenClaw), OpenClaw: YouTube Shorts posting, Operational notes (+9 more)

### Community 206 - "compilerOptions"
Cohesion: 0.11
Nodes (17): compilerOptions, allowSyntheticDefaultImports, exactOptionalPropertyTypes, forceConsistentCasingInFileNames, isolatedModules, module, moduleResolution, noEmit (+9 more)

### Community 207 - "ghost-cam-size.ts"
Cohesion: 0.21
Nodes (15): clamp01(), DEFAULT_GHOST_CAM_SIZE_STEP, getAdaptiveGhostCamMaxPx(), getCameraStackWidthShare(), getGhostCamGapPx(), getGhostCamSizeLabel(), getGhostCamSizePx(), getResponsiveGhostCamSizePx() (+7 more)

### Community 209 - "Chrome Web Store Listing — AniDachi Extension"
Cohesion: 0.12
Nodes (16): Category, Chrome Web Store Listing — AniDachi Extension, Detailed Description, Extension Name, Keywords (for internal reference, not shown on store), Language, Minimal Push Payload Privacy Model, Notification Permission Disclosure (+8 more)

### Community 210 - "public.get_account_inbox_page"
Cohesion: 0.13
Nodes (13): prepare_friendship_inbox_state, prepare_room_invite_recipient_inbox_state, public.get_account_inbox_page(), public.reconcile_account_inbox(), public.friend_groups, public.friendships, public.profiles, public.room_invite_recipients (+5 more)

### Community 211 - "Real-WebRTC Two-browser P2P Harness"
Cohesion: 0.12
Nodes (16): Disconnect Grace, Durable Cross-room Assignment, Exact Departure, Invite And Membership Semantics, Live Room State, Migration-first Rollout, No New Distributed Authority For MVP, Selected: Supabase Assignment Plus Existing Room Durable Object (+8 more)

### Community 212 - "generate-extension-icons.mjs"
Cohesion: 0.15
Nodes (16): node:zlib, CHECK_ONLY, chunk(), crc32(), encodePng(), OUTPUT_DIR, paethPredictor(), PNG_SIGNATURE (+8 more)

### Community 213 - "demo/package.json"
Cohesion: 0.12
Nodes (15): devDependencies, typescript, vite, vitest, vitest, name, private, scripts (+7 more)

### Community 214 - "social-snapshot-cache.ts"
Cohesion: 0.21
Nodes (14): CachedSocialSnapshot, clearCachedSocialSnapshotForUser(), getCachedSocialSnapshotForUser(), isCanonicalUtcTimestamp(), isRecord(), isSocialSnapshotCacheFresh(), parseCachedSocialSnapshot(), setCachedSocialSnapshotForUser() (+6 more)

### Community 216 - "package.json"
Cohesion: 0.12
Nodes (15): @biomejs/biome, devDependencies, @biomejs/biome, turbo, typescript, vitest, engines, node (+7 more)

### Community 217 - "Environment And Secrets Matrix"
Cohesion: 0.12
Nodes (15): ANIDACHI_NOTIFICATION_DRAIN_SECRET, Change Checklist, Cloudflare, Cloudflare Configuration, Environment And Secrets Matrix, Environment Separation, GitHub Actions, GitHub Actions Configuration (+7 more)

### Community 218 - "watch-history-v3.catalog-read-states.test.ts"
Cohesion: 0.19
Nodes (11): EvidenceCase, GENERATED_AT, enabled, exactRecord(), parseResourceDetailPage(), parseResourceEpisodeCursor(), parseResourceTitlePage(), RecordValue (+3 more)

### Community 219 - "20260814010000_watch_history_v2_foundation.sql"
Cohesion: 0.25
Nodes (14): public.apply_watch_progress_v2(), public.delete_watch_history_v2(), public.recent_people_evidence, public.set_watch_preferences_v2(), public.user_watch_settings, public.watch_episode_progress, public.watch_history_deletions, public.watch_history_receipts (+6 more)

### Community 220 - "One canonical personal episode progress row in Supabase"
Cohesion: 0.16
Nodes (15): Background-owned account and generation scoped cache/outbox, Bounded canonical title and episode reads with honest continuation, Browse rollback restores prior v3 consumers and retains additive history data, One canonical personal episode progress row in Supabase, Owner-private group provenance requires invited overlapping viewing, Schema-3 logical episode identity and bounded catalog evidence, Transactional idempotency, server order, generation, authority, and deletion fences, Browser-local explicit YouTube tracking consent (+7 more)

### Community 221 - "Core Contract"
Cohesion: 0.13
Nodes (14): Core Contract, Done Means, File Structure, Global Constraints, Prerequisites, Provider Player Overlay Geometry Implementation Plan, Rollback, Task 1: Add The Provider-Neutral Geometry Contract (+6 more)

### Community 222 - "Voice Controls and Participant Audio Implementation Plan"
Cohesion: 0.13
Nodes (14): Final Definition of Done, Implementation Status, Preference Model, Pull Request and Promotion, Runtime State Model, Task 1: Add the Voice Domain and Preference Codec, Task 2: Separate Microphone Publication, Speech, and Audio Flow, Task 3: Generalize the P2P Microphone Lifecycle (+6 more)

### Community 223 - "Interface Visibility Settings Design"
Cohesion: 0.13
Nodes (14): Accessibility, Architecture, Goals, Information Architecture, Interface Visibility Settings Design, Main Control, Non-Goals, Participant Pills (+6 more)

### Community 224 - "pull_request_template.md"
Cohesion: 0.13
Nodes (14): Affected Planes, AI Contribution Notes, Changed Areas, Docs / Graphify, Goal, Quality Gate, Risk Class, Rollback (+6 more)

### Community 225 - "wxt.config.ts"
Cohesion: 0.14
Nodes (11): resolveExtensionChannel(), apiHttpHostPermission, apiWsHostPermission, buildId, extensionChannel, extensionIcons, extensionManifestKey, LOCAL_HOST_PERMISSIONS (+3 more)

### Community 226 - "privacy-sanitizer.ts"
Cohesion: 0.18
Nodes (13): HASH_IDENTIFIER_ARRAY_FIELDS, HASH_IDENTIFIER_FIELDS, isHostOrSubdomain(), looksLikeRawMediaPayload(), MEDIA_IDENTIFIER_FIELDS, RAW_FRAME_FIELDS, RAW_MEDIA_FIELDS, redactNetworkAddress() (+5 more)

### Community 227 - "tiktok/api.ts"
Cohesion: 0.33
Nodes (12): publishToTtAccount(), publishToTtAccount(), apiError(), ensureAllCredentials(), ensureCredentials(), fetchPublishStatus(), initInboxPhotoPost(), initInboxVideoPost() (+4 more)

### Community 228 - "best-anime-to-watch-with-friends/page.tsx"
Cohesion: 0.16
Nodes (9): faq, headings, itemList, metadata, faq, metadata, tocHeadings, DataTableColumn (+1 more)

### Community 229 - "best-short-anime-to-watch-with-friends/page.tsx"
Cohesion: 0.13
Nodes (10): faq, headings, itemList, metadata, SITE_URL, faq, headings, itemList (+2 more)

### Community 230 - "AniDachi SEO Content Guidelines"
Cohesion: 0.14
Nodes (14): AniDachi SEO Content Guidelines, Canonical anime URL (programmatic), Guides / compare / listicles, Hard safety rules (ranking / indexation), Information architecture, Measurement, Page checklist (new or substantial edit), Pre-publish checklist (+6 more)

### Community 231 - "seo-cta-cleanup.py"
Cohesion: 0.20
Nodes (12): bg_like(), main(), collections, math, pathlib, pil, re (Python standard library), main() (+4 more)

### Community 232 - "auth_artifact_cleanup_plan_contract.mjs"
Cohesion: 0.20
Nodes (11): actualVisits(), assertPlan(), explain(), familyDelete, findSubplan(), flattenPlan(), lineageDelete, migration (+3 more)

### Community 233 - "YouTube SEO conversion polish + agent upgrades"
Cohesion: 0.14
Nodes (13): Execution order, Goal, Light pass (P2 guides), Out of scope, Part A — Page enrichment (conversion-first), Part B — SEO agent upgrades, Pillar-specific (`watch-youtube-together/page.tsx`), Priority pages (edit these; light pass on the rest) (+5 more)

### Community 234 - "Handoff For AI Working On AniDachi Site Pages"
Cohesion: 0.14
Nodes (13): Branch Model, Checks To Run, Current Expected Workflow Summary, Handoff For AI Working On AniDachi Site Pages, How Site Changes Reach Main, Paths That Are Safe For Site-Only Auto-Promotion, Paths That Block Auto-Promotion, Production Environment (+5 more)

### Community 235 - "Technical production baseline"
Cohesion: 0.22
Nodes (14): Owner-bound account read contracts and Popup isolation, Artifact and authenticated acceptance boundaries, Channel-bound OAuth and refresh-token security, Waitlist and public-form CRM durable storage recovery, Database-first runtime rollout, Exact-query bounded Watch History browsing, Fast reopen staging delivery, Owner-private My groups provenance (+6 more)

### Community 236 - "Edge Cases"
Cohesion: 0.14
Nodes (14): Blocked User, Dashboard And Popup Drift, Edge Cases, Free Guest In Paid Room, Friend Removed After Invite, Group Changed After Invite, Local Progress Reconcile Conflict, Multi-Device User (+6 more)

### Community 237 - "File Map"
Cohesion: 0.14
Nodes (13): Account Contracts And Popup Isolation Implementation Plan, Completion Boundary, Existing files to modify, File Map, Global Constraints, New files, Task 1: Shared Account Read Contracts, Task 2: Versioned Web Read Responses (+5 more)

### Community 238 - "Two-phase Production Promotion"
Cohesion: 0.16
Nodes (14): Fresh Approval Before Every Merge, Frozen Staging Candidate, Staging and Main Git Convergence, Migration-first Phase, Technical Baseline Is Not a Public Launch, PR 174 Superseded Combined Promotion, PR 227 Migration-only Promotion, PR 228 Runtime Promotion (+6 more)

### Community 239 - "Waitlist And CRM Durable Storage Recovery Implementation Plan"
Cohesion: 0.23
Nodes (14): Conflict-Safe CRM Mutations, CRM-Specific Blob Runtime Authority, Canonical Docs Environment Contract And Graphify Closeout, Recovery Global Constraints, Waitlist And CRM Durable Storage Recovery Implementation Plan, Interim Staging Data Acceptance, Legacy Public CRM Rollback Source, Lossless CRM Reconciliation Tool (+6 more)

### Community 240 - "AGENTS.md"
Cohesion: 0.15
Nodes (12): Commands, Done Means, Git Flow, graphify, Instruction Layers, Knowledge Graph, Quality Gates, Read First (+4 more)

### Community 241 - "demo/tsconfig.json"
Cohesion: 0.15
Nodes (12): compilerOptions, lib, extends, include, src, lib, dom, dom (+4 more)

### Community 242 - "extension/package.json"
Cohesion: 0.15
Nodes (12): name, private, scripts, build, check, dev, dev:local, dev:staging (+4 more)

### Community 243 - "popup-styles.ts"
Cohesion: 0.28
Nodes (6): extensionThemeTokens, popupStyles, popupWatchHistoryStyles, overlayStyles, getNumericProperty(), getRule()

### Community 244 - "overlay-voice-session.ts"
Cohesion: 0.27
Nodes (11): createVoiceSessionState(), getVoiceIndicatorParticipantIds(), isVoiceSessionPublishing(), MicrophoneRelease, reduceVoiceSession(), shouldResetPersistedOpenMicAfterMediaSeatLoss(), stopVoiceSessionImmediately(), VoiceSessionAction (+3 more)

### Community 245 - "watch_history_v2_migration_order_contract.mjs"
Cohesion: 0.15
Nodes (12): initializationPosition, initializationSql, lockTimeoutPosition, migrationUrl, rpcPosition, sessionTriggerPosition, sourceLockPosition, transactionPosition (+4 more)

### Community 246 - "scratchpad.md"
Cohesion: 0.17
Nodes (12): AniDachi SEO audit (Planner → Executor 2026-08-11), Background and Motivation, Background and Motivation (historical), Current Status / Progress Tracking, Full SEO Analysis (Executor 2026-08-02), GSC SEO Optimisation Batch (2026-06-08), High-level Task Breakdown, Homepage CRO Rework (Execution Summary) (+4 more)

### Community 247 - "Codex-Hosted Semantic Extraction"
Cohesion: 0.17
Nodes (13): Codex-Hosted Semantic Extraction, Graphify Union Merge Driver, Graphify 0.9.53 Release, Headless Graphify Semantic Backend, Incremental Freshness and Upgrade Policy, Per-Machine Setup and Opt-In Git Hooks, Normalized AST-Only Code Graph Refresh, September 4 Graphify Installation Audit (+5 more)

### Community 248 - "Global Constraints"
Cohesion: 0.15
Nodes (12): Done Means, Global Constraints, Rollout And Rollback, Room And P2P Release Hardening Implementation Plan, Task 1: Fail-Closed Microphone And Capture Ownership, Task 2: Bounded Protocol And Worker Room Boundary, Task 3: Terminal Room Lifecycle Across Web And Durable Object, Task 4: Empty-Room Alarm And Idempotent Web Callback (+4 more)

### Community 249 - "V1 Product Decisions (Historical)"
Cohesion: 0.17
Nodes (13): Microphone Errors, Open Mic, Participant Mix, Per-Participant Playback, Player and Dictation, Player Audio, Privacy and Lifecycle, Push to Talk (+5 more)

### Community 250 - "Shared Pure Visibility Policy"
Cohesion: 0.21
Nodes (13): Accessible Interface Settings View, Finite Policy-Driven Preview, Main-Control Preference Integration, Versioned Interface Preference Model, Shared Pure Visibility Policy, Serialized Preference Persistence, Immediate Apply with Persistence Rollback, Interface Accessibility Contract (+5 more)

### Community 251 - "validate-extension-artifact.mjs"
Cohesion: 0.15
Nodes (8): args, broadPatterns, channel, contentMatches, expectedByChannel, manifest, manifestPath, videoHosts

### Community 252 - "room-metering.ts"
Cohesion: 0.36
Nodes (10): createRoomMeterState(), elapsedMs(), isRecord(), isSafeTimestamp(), parseRoomMeterState(), reconcileRoomMeter(), roomUsageSummary(), safeTimestamp() (+2 more)

### Community 253 - "overlay-layout-engine.test.ts"
Cohesion: 0.23
Nodes (11): cloneDefinition(), getDefaultOverlayLayoutDefinition(), layoutAt(), blockChatGridCell(), blockChatGridSpan(), blockChatRow(), chatTopForSelectionRow(), createCompactChatDefinition() (+3 more)

### Community 254 - "MemoryStorageArea"
Cohesion: 0.17
Nodes (4): BackgroundDependencies, DelayedRemoveStorageArea, MemoryStorageArea, StorageAreaLike

### Community 255 - "public.get_account_inbox_page_v2"
Cohesion: 0.23
Nodes (10): public.get_account_inbox_page_v2(), public.reconcile_account_inbox_v2(), public.respond_room_invite_v2(), public.friend_groups, public.friendships, public.profiles, public.room_invite_recipients, public.room_invites (+2 more)

### Community 256 - "watch_history_v3.test.sql"
Cohesion: 0.17
Nodes (6): public.recent_people_evidence, watch_v3_force_receipt_failure, watch_v3_recent_people_before_expired, watch_v3_recent_people_before_post_end, watch_v3_recent_people_pair, pg_temp.watch_v3_force_receipt_failure

### Community 257 - "Project Knowledge Map"
Cohesion: 0.17
Nodes (11): Baseline Graph, Current Anidachi Queries To Run Before P2P Block 6, Current Status, Isolated Code Update Regression Test, Keeping The Graph Current, Output Policy, Project Knowledge Map, Pull Request Rule (+3 more)

### Community 258 - "Popup People And Social Directory Implementation Plan"
Cohesion: 0.17
Nodes (11): Final Self-Review Checklist, Popup People And Social Directory Implementation Plan, Pull Request And Rollback, Scope Guardrails, Task 1: Add Shared Recent-People And Social-Directory Contracts, Task 2: Make Recent People Canonical And Duplicate-Free, Task 3: Add The MV3 Social-Directory And Friend-Request Bridge, Task 4: Migrate The Account-Owned Popup Social Snapshot (+3 more)

### Community 259 - "Rollout Phases"
Cohesion: 0.17
Nodes (12): Rollout Phases, Task 10: Automated Gates, Staging Deployment, And Real Acceptance, Task 11: Separate V1 Cutover And Checkpoint Cleanup, Task 1: Crunchyroll Catalog Source Preflight And Sanitized Fixtures, Task 2: Additive Watch History V2 Protocol, Task 3: Additive Supabase Foundation And Transactional RPCs, Task 4: V2 Web Service, Routes, Pagination, And Server Validation, Task 5: Account-Scoped Compact Outbox And V2 Extension Client (+4 more)

### Community 260 - "smoke-staging-web.mjs"
Cohesion: 0.29
Nodes (10): assertHeaderIncludes(), assertStatus(), assertTextIncludes(), baseUrl, extractCookie(), fetchManual(), formBody(), getSetCookies() (+2 more)

### Community 261 - "Incremental Re-extraction"
Cohesion: 0.18
Nodes (11): Changed-file Replacement, Cluster-only Refresh, Code-only Fast Path, Deleted-source Pruning, Graph Update Diff, Incremental File Detection, Incremental Re-extraction, Incremental Update Runbook (+3 more)

### Community 262 - "MAX_PARTICIPANT_ID_CHARS"
Cohesion: 0.25
Nodes (8): loadVoiceModePreference(), parseVoiceModePreference(), persistVoiceModePreference(), StorageAreaLike, VOICE_MODE_PREFERENCE_VERSION, VoiceModePreferenceRecord, voiceModePreferenceStorageKeyForUser(), MAX_PARTICIPANT_ID_CHARS

### Community 263 - "createWatchHistoryCatalogCoordinator"
Cohesion: 0.24
Nodes (8): contextKey(), createWatchHistoryCatalogCoordinator(), begin(), commit(), createWatchHistoryPageResolver(), abortCatalogs(), resolve(), matches()

### Community 264 - "Download on the App Store Badge"
Cohesion: 0.31
Nodes (11): Download on the App Store Badge, App Store Conversion CTA, Apple Logo Glyph, Black Rounded Badge Container, Download on the App Store Lettering, Gray Badge Outline, iOS App Distribution Trust Signal, Mobile App Download Funnel (+3 more)

### Community 265 - "20260816090000_watch_history_v2_bounded_read.sql"
Cohesion: 0.18
Nodes (5): public.list_watch_history_v2_page(), public.user_watch_settings, public.watch_episode_progress, sync_watch_history_session_summaries_v2, public.sync_watch_history_session_summaries_v2

### Community 266 - "public.active_room_sessions"
Cohesion: 0.29
Nodes (8): public.active_room_sessions, public.claim_active_room_session_v1(), public.create_room_with_active_session_v1(), public.finalize_room_usage(), public, public.room_members, public.rooms, public.users

### Community 267 - "README.md"
Cohesion: 0.18
Nodes (7): Workspace Package Globs, AniDachi — Watch Anime Together, Environment Variables, Local Development, Project Structure, SEO / Content, Stack

### Community 268 - "Resources Progress Menu Implementation Plan"
Cohesion: 0.18
Nodes (10): Acceptance Criteria, File Structure, Future Backend Upgrade, Resources Progress Menu Implementation Plan, Task 1: Local Watch Progress Store, Task 2: Crunchyroll Progress Identity, Task 3: Resources Panel Component, Task 4: Overlay Integration and Recorder (+2 more)

### Community 269 - "Task 7 Complete"
Cohesion: 0.20
Nodes (11): Explicit Production Approval Gate Satisfied, Acceptance Snapshot Private Authority: 687 Contacts And 685 Survey Leads, Final Production CRM Rollback Anchors, Pre-Cutover Deployment Rollback Anchor, Production Controlled Public-Form Acceptance, Production CRM Runtime Logs Clean, Production Cutover And Closeout, Production Narrow Credential Boundary (+3 more)

### Community 270 - "p2p-scorecard.mjs"
Cohesion: 0.29
Nodes (10): analyzeFile(), fail(), get(), median(), ms(), pairKind(), parseExport(), paths (+2 more)

### Community 271 - "room-invite-notification-runtime.test.ts"
Cohesion: 0.22
Nodes (5): alarms, diagnostic, json(), registered(), state

### Community 272 - "SEO Trust & Authority plan (2026-07-28)"
Cohesion: 0.20
Nodes (10): Agent / guidelines, Attribution, Freeze exit criteria, Hard safety constraint, Run the portfolio audit, SEO Trust & Authority plan (2026-07-28), URL stability (ranked pages), What it does not replace (+2 more)

### Community 273 - "AniDachi Logo Asset"
Cohesion: 0.33
Nodes (10): AniDachi Logo Asset, Anime Mascot Face, App Icon Readability, Circular Badge Shape, Closed Smile Expression, Dark Curved Eyes, Friendly Brand Tone, Hair Flame Shape (+2 more)

### Community 274 - "20260819133849_auth_channel_rotation.sql"
Cohesion: 0.38
Nodes (8): public.create_refresh_token_family_v1(), public.refresh_token_families, public.refresh_token_lineage, public.resolve_refresh_token_family_v1(), public.revoke_refresh_token_family_v1(), public.rotate_refresh_token_family_v1(), public.devices, public.users

### Community 275 - "Notification Delivery"
Cohesion: 0.24
Nodes (10): pg_net Platform Permission Boundary, Durable Client Recovery, Account Isolation, Browser Notifications, Client Recovery Intents, MVP Channel, Notification Delivery, pg_net Transport Limits (+2 more)

### Community 276 - "Rollout Order"
Cohesion: 0.20
Nodes (10): Phase 0 - Contract And Plan, Phase 1 - Billing Entitlements Foundation, Phase 2 - Profiles, Friends, And Recent People, Phase 3 - Personal Groups, Phase 4.5 - Product Surface Reframe, Phase 4 - Room Capabilities And Media Seats, Phase 5 - Invites, Inbox, And Push Delivery, Phase 6 - Watch Library, History, And Continue Together (+2 more)

### Community 277 - "Completed Public-Product Recovery Record"
Cohesion: 0.20
Nodes (9): Distinct Live Signup 2026-08-23T18:13:49.881Z Created Position 686, Completed Public-Product Recovery Record, Distinct Live Signup Advanced Production To 686, Fresh-Deployment Acceptance, Live Production Survey Lead Count: 686, Plans, Production And Main Promotion Complete, Staging And Production CRM Recovery Complete (+1 more)

### Community 278 - "Development Quality Gates"
Cohesion: 0.28
Nodes (8): Plane-Specific Definition of Done, Baseline For Every Change, Development Quality Gates, Verification Evidence Capture, Evidence To Capture, Exceptions, Gate Matrix, Risk-Proportional Gate Exceptions

### Community 279 - "panel-account-title.tsx"
Cohesion: 0.39
Nodes (7): AuthenticatedUserPlan, calculatePlanGlyphOffset(), getLastVisibleGrapheme(), measureGlyphAscent(), PanelAccountTitle(), PanelAccountTitleProps, PLAN_LABELS

### Community 280 - "overlay-layout-ghost-preview.tsx"
Cohesion: 0.33
Nodes (6): CHAT_PREVIEW_MESSAGES, OverlayLayoutChatPreview(), ResolvedOverlayLayout, getPixelRectStyle(), OverlayLayoutGhostPreview(), OverlayLayoutGhostPreviewProps

### Community 281 - "overlay-media-session.ts"
Cohesion: 0.33
Nodes (7): CameraEnabledForRoomConnectionInput, DEFAULT_LOCAL_CAMERA_ENABLED, getCameraEnabledForRoomConnection(), getP2PMediaSessionState(), P2PMediaSessionState, persistRoomSessionForCurrentJoin(), PersistRoomSessionForCurrentJoinInput

### Community 282 - "extension/src/watch-history-browse.ts"
Cohesion: 0.42
Nodes (7): createWatchHistoryDateRange(), isoRange(), localDateStart(), moveLocalCalendarDate(), parseLocalDate(), WatchHistoryDateRange, WatchHistoryDateRangeResult

### Community 283 - "Conversion metrics (GA4)"
Cohesion: 0.22
Nodes (8): Conversion metrics (GA4), Event names (funnel), Next test hypotheses, Parameters (all string-friendly for GA4), QA checklist (post-deploy), SEO portfolio audit, Stripe checkout metadata, Web vitals (field data)

### Community 284 - "Host Avatar Asset"
Cohesion: 0.28
Nodes (9): Calm Neutral Expression, Cool Blue Purple Lighting, Demo Avatar Profile, Frontal Headshot Composition, Host Avatar Asset, Host Persona, Minimal Dark Wardrobe, Square Avatar Crop (+1 more)

### Community 285 - "public.apply_watch_progress_v2"
Cohesion: 0.22
Nodes (8): public.apply_watch_progress_v2(), public.profiles, public.user_watch_settings, public.users, public.watch_episode_progress, public.watch_session_participants, public.watch_sessions, public.watch_history_receipts

### Community 286 - "graphify reference: extra exports and benchmark"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 287 - "Executor's Feedback or Assistance Requests"
Cohesion: 0.22
Nodes (9): Additional batch (10 more watch pages — 2026-05-12), Additional batch (10 more watch pages — 2026-05-14), Additional batch (10 more watch pages — 2026-05-15), Additional batch (10 more watch pages — 2026-05-19), Additional batch slugs (implemented 2026-05-12), Additional batch slugs (implemented 2026-05-14), Executor's Feedback or Assistance Requests, Proposed next 10 `/watch/[slug]-with-friends` pages (+1 more)

### Community 288 - "Global Constraints"
Cohesion: 0.22
Nodes (8): Global Constraints, Interface Visibility Settings Implementation Plan, Task 1: Define The Versioned Interface Preference Model, Task 2: Add One Pure Visibility Policy For Runtime And Preview, Task 3: Load And Persist Preferences With Ordered Writes, Task 4: Build The Interface Settings View And Truthful Preview, Task 5: Connect Main-Control Preferences Without Regressing Edge Intent, Task 6: Add Persistent Compact Participant Pills

### Community 290 - "room-quota-display.ts"
Cohesion: 0.43
Nodes (6): applyRoomUsageSnapshot(), isNewerRoomUsage(), nonnegative(), quotaDayFromResetAt(), roomQuotaRemainingSeconds(), RoomUsageDisplayAnchor

### Community 291 - "room-tab-lock.ts"
Cohesion: 0.46
Nodes (5): acquireRoomTabLock(), getLockManager(), isRoomTabLockSupported(), LockManagerLike, releaseRoomTabLock()

### Community 292 - "include"
Cohesion: 0.25
Nodes (7): extends, include, src, test, entrypoints, wxt.config.ts, ./.wxt/tsconfig.json

### Community 293 - "about/page.tsx"
Cohesion: 0.32
Nodes (4): metadata, DISCORD_SERVER_CTA_LABEL, DISCORD_SERVER_INVITE_URL, PUBLIC_SOCIAL_LINKS

### Community 294 - "Haruto Avatar Image"
Cohesion: 0.39
Nodes (8): Approachable Demo Identity, Haruto Avatar Image, Direct Eye Contact, Friendly Smiling Portrait, Haruto Persona, Neutral Profile Background, Profile Placeholder Asset, Watch Party Avatar

### Community 295 - "Natsuki Avatar Asset"
Cohesion: 0.43
Nodes (8): Approachable Social Presence, Demo Watch-Party Persona, Natsuki Avatar Asset, Red Turtleneck Styling, Smiling Portrait Subject, Soft Blurred Background, Square Avatar Composition, Warm Expressive Smile

### Community 296 - "extension_auth_pkce_concurrency_contract.mjs"
Cohesion: 0.29
Nodes (6): challenge, codeHash, psqlArgs, query(), stateHash, waitForActivity()

### Community 297 - "public.room_invite_actions"
Cohesion: 0.25
Nodes (6): public.room_invite_actions, public, public.friend_groups, public.room_invites, public.rooms, public.users

### Community 298 - "public.consume_extension_auth_code_v1"
Cohesion: 0.25
Nodes (6): public.consume_extension_auth_code_v1(), public.extension_auth_codes, public.refresh_tokens, public.oauth_login_transactions, public.refresh_token_families, public.refresh_token_lineage

### Community 299 - "Staging Acceptance Checklist"
Cohesion: 0.25
Nodes (7): Always Check, Evidence To Attach To PR, Extension, Promotion Rule, Room / P2P, Site / Auth, Staging Acceptance Checklist

### Community 300 - "Global Constraints"
Cohesion: 0.25
Nodes (7): Execution Note, Global Constraints, Overlay Layout Engine V2 Core Implementation Plan, Task 1: Version 2 Layout Model And Defaults, Task 2: Deterministic Camera Slot Geometry, Task 3: Chat Geometry, Collision Search, And Core Resolver, Task 4: Core Acceptance And Knowledge Graph Refresh

### Community 301 - "Global Constraints"
Cohesion: 0.25
Nodes (7): Global Constraints, Overlay Layout Runtime And Editor V2 Implementation Plan, Task 1: V2 Storage Contract And Runtime Style Adapter, Task 2: Live Runtime Parity, Task 3: Draft-Based V2 Layout Editor, Task 4: Delete V1 And Finish Visual Styling, Task 5: Acceptance, Documentation, And Staging Delivery

### Community 302 - "Approved Voice UX Simplification V2"
Cohesion: 0.25
Nodes (8): Approved Voice UX Simplification V2, Product Contract, Runtime Contract, Task 10: Add Room-Scoped Voice Restore (Complete), Task 11: Simplify Voice UI and Remove Dictation (Complete), Task 12: Verify and Hand Off (Automated Complete, Manual Pending), Task 9: Replace the V1 Voice Interaction Contract (Complete), V2 Implementation Tasks

### Community 303 - "Production Acceptance Snapshot: 685 Survey Leads"
Cohesion: 0.29
Nodes (8): Final Docs-Triggered Production Deployment dpl_HcqvVAnF9V4EnSHjYekrpmKfQEUY, Fresh Production Redeploy dpl_DCt6ocJBbEJ848rfaC38W5bhbdyg, Full-Redeploy Durability Proof, Live Product Counts Are Dynamic, Post-Signup Private Authority: 688 Contacts And 686 Survey Leads, Production Deployment dpl_3v2H5pk4v5muknJvpyKjXZpJHEXX, Production Idempotent Replay, Production Acceptance Snapshot: 685 Survey Leads

### Community 304 - "Watch Drawer Browse And Presentation"
Cohesion: 0.25
Nodes (7): Acceptance, Boundaries, Group Meaning And Privacy, Product, Progress And Tree, Settings And Management, Watch Drawer Browse And Presentation

### Community 305 - "YouTube Adapter Notes"
Cohesion: 0.25
Nodes (7): Ownership, Playback Phases, Required Acceptance, Selector Maintenance, Supported Surface, Synchronization Policy, YouTube Adapter Notes

### Community 306 - "findKatamariPlayerFromReactNode"
Cohesion: 0.48
Nodes (7): findAncestor(), findKatamariPlayer(), findKatamariPlayerFromReactNode(), isKatamariPlayer(), isRecord(), readKatamariPlayer(), readObjectProperty()

### Community 307 - "overlay-layout.ts"
Cohesion: 0.48
Nodes (5): DEFAULT_MINI_PANEL_BOTTOM_RESERVE_PX, getOverlayChromePlacement(), normalizePixelValue(), OverlayChromePlacement, shouldShowCameraStack()

### Community 308 - "AniDachi Apple Touch Icon"
Cohesion: 0.43
Nodes (7): AniDachi Apple Touch Icon, Flame Hair Gradient, Friendly Closed-Eye Expression, iOS Home Screen Asset, Mascot Face, Web App Brand Asset, White Face Shape

### Community 309 - "AniDachi Web Logo Asset"
Cohesion: 0.52
Nodes (7): Smiling Anime Face Mascot, AniDachi Web Logo Asset, Circular Logo Badge, Closed Eye Smile Expression, Friendly Anime Brand Identity, Red Orange Gradient Hair Silhouette, White Face Shape

### Community 310 - "public.watch_sessions"
Cohesion: 0.62
Nodes (6): public.user_tracked_titles, public.watch_progress_checkpoints, public.watch_session_participants, public.watch_sessions, public.rooms, public.users

### Community 311 - "Operating contract (mandatory)"
Cohesion: 0.29
Nodes (7): Evidence hierarchy, Measurement commands, Operating contract (mandatory), Publishing gate (new or major SEO URL), Ranking / indexation safety, URL stability gate (mandatory), Winner queue (while freeze is active)

### Community 312 - "Project Status Board"
Cohesion: 0.29
Nodes (7): AniDachi SEO audit (Executor — awaiting Planner confirm), Full SEO Analysis (2026-08-02), Keyword Enrichment Analysis (2026-08-02), Keyword Enrichment Implementation (2026-08-03) — Executor, Project Status Board, SEO agent critical fixes (Executor — awaiting Planner confirm), SEO Trust & Authority

### Community 313 - "Site, Extension, Auth, and Database Integration Notes"
Cohesion: 0.29
Nodes (7): Authenticated Room Flow, Custom Website Auth Model, Extension PKCE Login Flow, Server-derived Participant Identity, Shared Protocol Monorepo, Site, Extension, Auth, and Database Integration Notes, Web and Worker Source of Truth Split

### Community 314 - "Voice UX Simplification V2"
Cohesion: 0.29
Nodes (7): Account-Scoped Last Explicit Voice Preference, Dictate Reactions Removal, Manual Voice Staging Acceptance Pending, Room Media Startup Defaults, Room-Scoped Voice State, V-Only Push to Talk, Voice UX Simplification V2

### Community 315 - "Participant Audio Controls"
Cohesion: 0.29
Nodes (7): Header Microphone Control, Hotkey and Player Isolation, Participant Audio Controls, Participant With Rendered Video, Surface Handoff, UI Contract, Voice Settings Panel

### Community 316 - "September 4 Microphone-Independent Main Control Correction"
Cohesion: 0.33
Nodes (7): Manual Acceptance and Draft PR Gate, September 4 Microphone-Independent Main Control Correction, Automated Verification and Staging Artifact Evidence, Extension-Local Presentation Boundary, Microphone-Independent Launcher Visibility, Extension-Only Staging Rollout, Automated Coverage and Manual Staging Acceptance Boundary

### Community 317 - "Watch History Catalog And Progress Implementation Plan"
Cohesion: 0.29
Nodes (5): Final Acceptance Matrix, Global Constraints, Out Of Scope Follow-Ups, Rollback Rules, Watch History Catalog And Progress Implementation Plan

### Community 318 - "sync.ts"
Cohesion: 0.33
Nodes (6): getExpectedHostTime(), getPlaybackDrift(), SYNC_CATCH_UP_DRIFT_SECONDS, SYNC_IGNORE_DRIFT_SECONDS, SYNC_SEEK_DRIFT_SECONDS, SyncCorrection

### Community 319 - "current-resource-panel.tsx"
Cohesion: 0.47
Nodes (4): CurrentResourcePanel(), CurrentResourcePanelProps, formatProgressClock(), HistoryObservation

### Community 320 - "appendDiagnosticEntry"
Cohesion: 0.40
Nodes (6): appendDiagnosticEntry(), isDiagnosticEntry(), pruneDiagnosticEntries(), readDiagnosticEntries(), sanitizeDiagnosticData(), sanitizeDiagnosticEntry()

### Community 321 - "diagnostic-log.test.ts"
Cohesion: 0.33
Nodes (3): clearDiagnosticEntries(), DiagnosticMessage, handleDiagnosticMessage()

### Community 322 - "background-invite-notification-wiring.test.ts"
Cohesion: 0.40
Nodes (4): boundary, flush(), handlers, start()

### Community 323 - "AniDachi App Icon"
Cohesion: 0.40
Nodes (6): AniDachi App Icon, Browser Tab Identity, Red Orange Circular Badge, Stylized Anime Mascot Face, Transparent PNG Icon Asset, White Face Silhouette

### Community 324 - "skills.ts"
Cohesion: 0.33
Nodes (4): SKILL_CONTENT, SKILL_DETECTION_PROMPT, SKILL_NAMES, SkillName

### Community 325 - "public.room_invites"
Cohesion: 0.47
Nodes (5): public.room_invite_recipients, public.room_invites, public.friend_groups, public.rooms, public.users

### Community 326 - "public.list_recent_people_evidence"
Cohesion: 0.33
Nodes (4): public.list_recent_people_evidence(), public.friendships, public.recent_people_hidden, public.watch_progress_checkpoints

### Community 328 - "public.claim_active_room_session_v1"
Cohesion: 0.47
Nodes (5): public.claim_active_room_session_v1(), public.create_room_with_active_session_v1(), public.room_members, public.rooms, public.active_room_sessions

### Community 329 - "anidachi_private.tick_inbox_push_scheduler"
Cohesion: 0.40
Nodes (5): anidachi_private.inbox_push_scheduler, anidachi_private.tick_inbox_push_scheduler(), net.http_request_queue, net._http_response, public.account_inbox_push_outbox

### Community 331 - "watch_history_v3_episode_previews.test.sql"
Cohesion: 0.40
Nodes (3): bounded_preview, pg_temp.preview_tail(), preview_pages

### Community 333 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 334 - "YouTube keyword bank + templates (Keyword Planner US)"
Cohesion: 0.33
Nodes (6): Alternatives / listicle differentiation minimum, Keyword Planner gate (before locking URLs), YouTube anti-cannibalization map (owned queries), YouTube conversion checklist (required), YouTube keyword bank + templates (Keyword Planner US), YouTube measurement note

### Community 335 - "High-level Task Breakdown (historical)"
Cohesion: 0.33
Nodes (6): Completed (prior watch-page batches — historical), Completed (YouTube SEO batch 2 — 2026-07-26), High-level Task Breakdown (historical), Next batch (10 new watch pages), SEO agent critical fixes (Planner → Executor 2026-07-26), YouTube conversion polish (Executor — awaiting Planner confirm)

### Community 336 - "Sitewide CTA → Plan-Picker Survey (Planner Notes)"
Cohesion: 0.33
Nodes (6): High-level Task Breakdown (implementation plan), Key Challenges and Analysis, Manual test checklist (post-implementation), Sitewide CTA → Plan-Picker Survey (Planner Notes), What exists today (relevant CTA surfaces found), What you asked for

### Community 337 - "Control plane: apps/web and Supabase"
Cohesion: 0.47
Nodes (6): Authenticated Web-to-Worker room-token join, Client plane: WXT extension and provider adapters, Control plane: apps/web and Supabase, Live plane: apps/api, Durable Objects, and WebSockets, packages/protocol shared TypeScript and Zod contracts, Watch progress browse ownership

### Community 338 - "AniDachi Project Knowledge Map"
Cohesion: 0.47
Nodes (6): AniDachi Project Knowledge Map, Graph-First, Source-Verified Navigation, P2P Block 6 Graph Navigation, Pull Request Graph and Source Evidence, Scoped Local Graphs, Shared Graph Artifact Allowlist

### Community 339 - "Тарифы AniDachi для MVP"
Cohesion: 0.33
Nodes (5): Как это работает, Позиционирование, Пример с подпиской, Таблица тарифов, Тарифы AniDachi для MVP

### Community 340 - "Extension UX"
Cohesion: 0.33
Nodes (6): Extension UX, Friend And Group Actions, Main Invite Surface, Notification Permission, Plan-Limit States, Popup Navigation

### Community 341 - "Always-Visible Compact Participant Pills"
Cohesion: 0.33
Nodes (6): Persistent Compact Participant Rail Integration, Audio-Adjustment Expansion Latch, Listener-Local Audio Controls and Compact Mute Marker, Always-Visible Compact Participant Pills, Participant Rail Eligibility and Displayable-Video Deduplication, Smart Participant Pills

### Community 342 - "Final Staging Runtime Acceptance"
Cohesion: 0.33
Nodes (6): Final Staging Runtime Acceptance, Fresh Preview Deployment dpl_AnAzpf8XTHUcCrYMz19TDkQ2y3rq, Runtime Deployment Acceptance Gate, Recovered Submission Idempotency, Staging Survey Lead Count: 685, Three Observed Failed Survey Submissions Recovered

### Community 343 - "Exact Assignment Release"
Cohesion: 0.33
Nodes (6): Durable-First Guest Departure, Durable Active-Room Assignment, Durable Release First, Live Detach Second, Exact Assignment Release, Exact Session Invariants, Public Exact Departure Contract

### Community 344 - "Testing Strategy"
Cohesion: 0.33
Nodes (6): Cross-Surface Acceptance, Database And Web Integration, Extension, Protocol, Staging, Testing Strategy

### Community 345 - "Five-Object CRM Reconciliation"
Cohesion: 0.33
Nodes (5): CRM Recovery Deployment Order, ETag Concurrency Control, Five-Object CRM Reconciliation, CRM Recovery Rollback Strategy, updateKreatliCrmBlobText()

### Community 346 - "smoke-worker.mjs"
Cohesion: 0.40
Nodes (3): baseUrl, fetchJson(), url()

### Community 347 - "apps/api Agent Instructions"
Cohesion: 0.40
Nodes (4): apps/api Agent Instructions, Rules, Source Of Truth, Verification

### Community 348 - "apps/extension Agent Instructions"
Cohesion: 0.40
Nodes (4): apps/extension Agent Instructions, Rules, Source Of Truth, Verification

### Community 349 - "apps/web Agent Instructions"
Cohesion: 0.40
Nodes (4): apps/web Agent Instructions, Rules, Source Of Truth, Verification

### Community 351 - "CTA and conversion map (internal)"
Cohesion: 0.40
Nodes (4): CTA and conversion map (internal), Placements, Surfaces, Template IDs (`page_template` in events)

### Community 352 - "privacy.ts"
Cohesion: 0.60
Nodes (3): YOUTUBE_SHORTS_PRIVACY, YouTubePrivacyStatus, youtubeUploadStepLabel()

### Community 353 - "watch_history_v2_migration_concurrency_contract.mjs"
Cohesion: 0.50
Nodes (3): psqlArgs, query(), waitForLock()

### Community 354 - "20260525_anidachi_auth.sql"
Cohesion: 0.80
Nodes (4): public.refresh_tokens, public.room_members, public.rooms, public.users

### Community 355 - "20260620_billing_entitlements.sql"
Cohesion: 0.50
Nodes (4): public.billing_customers, public.stripe_events, public.subscriptions, public.users

### Community 356 - "20260621_social_profiles_friends_recent.sql"
Cohesion: 0.60
Nodes (4): public.friendships, public.profiles, public.recent_people_hidden, public.users

### Community 357 - "public.friend_groups"
Cohesion: 0.60
Nodes (4): public.friend_group_members, public.friend_groups, public, public.users

### Community 358 - "public.list_recent_people_evidence_v2"
Cohesion: 0.40
Nodes (4): public.list_recent_people_evidence_v2(), public.friendships, public.recent_people_evidence, public.recent_people_hidden

### Community 359 - "auth_artifact_cleanup.test.sql"
Cohesion: 0.40
Nodes (3): auth_cleanup_results, pg_temp.explain_json(), cron.job

### Community 360 - "Participant Audio Control Surface Handoff"
Cohesion: 0.50
Nodes (5): Participant Audio Control Surface Handoff, Hotkey and Player Interaction Isolation, Listener-Local Participant Volume and Mute, Side Voice Rail Audio Control, Video Bubble Contour Audio Control

### Community 361 - "V2 Staging Acceptance Matrix"
Cohesion: 0.40
Nodes (5): Mode and Privacy, Recovery and Load, Room-Scoped Restore, UI and Indicators, V2 Staging Acceptance Matrix

### Community 362 - "Architectural Boundaries"
Cohesion: 0.40
Nodes (5): `apps/api`, `apps/extension`, `apps/web`, Architectural Boundaries, `packages/protocol`

### Community 363 - "Failure Behavior"
Cohesion: 0.40
Nodes (5): Cache Corruption Or Contract Mismatch, Failure Behavior, Offline Extension, Partial Server Failure, Stale And Duplicate Requests

### Community 364 - "Q: Оценить отзыв о Wave 2 Watch History v2"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Оценить отзыв о Wave 2 Watch History v2, Source Nodes

### Community 365 - "Q: Trace Watch History v2 extension background ownership, outbox/storage, provider progress adapters, overlay and popup writers for Wave 3."
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Trace Watch History v2 extension background ownership, outbox/storage, provider progress adapters, overlay and popup writers for Wave 3., Source Nodes

### Community 366 - "Q: так что ты сделал и зачем и что еще осталось, хорошо подумай и тщательно пройдить по пройденым шагам и по будущим"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: так что ты сделал и зачем и что еще осталось, хорошо подумай и тщательно пройдить по пройденым шагам и по будущим, Source Nodes

### Community 367 - "Q: Trace website and extension JWT verification, refresh rotation, session caching, logout, and database authority for Task 7."
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Trace website and extension JWT verification, refresh rotation, session caching, logout, and database authority for Task 7., Source Nodes

### Community 368 - "Q: Trace Watch History v2 from extension local playback projection and outbox through HTTP API and bounded title RPC to website and Popup; identify whether a visible title returns all episode rows."
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: Trace Watch History v2 from extension local playback projection and outbox through HTTP API and bounded title RPC to website and Popup; identify whether a visible title returns all episode rows., Source Nodes

### Community 369 - "waitlist-stats/route.ts"
Cohesion: 0.50
Nodes (4): Hero(), Repaired Hero To Private Blob Path Query, store.ts, waitlist-stats/route.ts

### Community 370 - "Q: How should per-participant volume be moved into a side pill for video bubbles while preventing player hover interference?"
Cohesion: 0.40
Nodes (4): Answer, Outcome, Q: How should per-participant volume be moved into a side pill for video bubbles while preventing player hover interference?, Source Nodes

### Community 371 - "Video-Bubble Side Pill"
Cohesion: 0.40
Nodes (5): ParticipantAudioInlineControl, Per-Participant Volume Side-Pill Query, Provider Player-Mode Validation, Unchanged Audio Ownership and Transport, Video-Bubble Side Pill

### Community 372 - "Stale Active Room Assignment After Guest Leave"
Cohesion: 0.40
Nodes (5): ACTIVE_ROOM_CONFLICT, Background-Owned Exact Departure, Guest Leave Active Room Conflict Query, Local-Only Explicit Leave Teardown, Stale Active Room Assignment After Guest Leave

### Community 373 - "Reaction Misclassified as Control Traffic"
Cohesion: 0.40
Nodes (5): Bounded Total Room Frames, Dedicated Reaction Rate Class, Rapid Reaction Delivery Query, Reaction Misclassified as Control Traffic, WebSocket Close After Rate Rejections

### Community 374 - "packages/protocol Agent Instructions"
Cohesion: 0.40
Nodes (4): packages/protocol Agent Instructions, Rules, Source Of Truth, Verification

### Community 377 - "20260602_extension_auth.sql"
Cohesion: 0.67
Nodes (3): public.devices, public.extension_auth_codes, public.users

### Community 380 - "public.watch_catalog_read_v3"
Cohesion: 0.50
Nodes (3): public.watch_catalog_read_v3(), public.watch_episode_progress, public.watch_catalog_snapshots

### Community 382 - "vercel.json"
Cohesion: 0.50
Nodes (3): regions, $schema, sfo1

### Community 383 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 384 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 385 - "graphify reference: incremental update and cluster-only"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 386 - "Experimental Features"
Cohesion: 0.50
Nodes (3): Experimental Features, Hold Fire Super Reaction, P2P Media Transport

### Community 387 - "Current Reality Check"
Cohesion: 0.50
Nodes (4): Already Good Enough To Build On, Current Reality Check, Not Done Yet, Partial Or Misleading

### Community 388 - "Privacy Security And Abuse Controls"
Cohesion: 0.50
Nodes (4): Abuse Controls, Chrome Web Store, Privacy Security And Abuse Controls, Supabase

### Community 389 - "Audio Speech Activity Classification"
Cohesion: 0.50
Nodes (4): Audio Speech Activity Classification, Audio Transport Flow Classification, Microphone and Camera Independence, Microphone Publication State

### Community 390 - "Voice Activity and Flow Model"
Cohesion: 0.50
Nodes (4): Sampling Cost, Speech Classification, Transport Flow Classification, Voice Activity and Flow Model

### Community 391 - "Target Contract And Storage Map"
Cohesion: 0.50
Nodes (4): Durable storage, Runtime ownership, Shared protocol, Target Contract And Storage Map

### Community 392 - "Production Closeout Complete"
Cohesion: 0.50
Nodes (4): Production Promotion PR 240, Full Production Redeploy dpl_DCt6ocJBbEJ848rfaC38W5bhbdyg, Main SHA 8cc5e4e6641ca55f0b62a320e8726de67900ce34, Production Closeout Complete

### Community 393 - "Surface Behavior"
Cohesion: 0.50
Nodes (4): Extension Popup, Popup Social Information Architecture, Surface Behavior, Web Account Dashboard

### Community 394 - "launch-chrome.sh"
Cohesion: 0.83
Nodes (3): port_in_use(), print_port_conflict_help(), launch-chrome.sh script

### Community 403 - "Staging Smoke Workflow"
Cohesion: 0.67
Nodes (3): Staging Password Gate, Staging Smoke Workflow, Staging Web Smoke Script

### Community 404 - "Current Integration Foundation"
Cohesion: 0.67
Nodes (3): Current Integration Foundation, Room Scoped ICE Authorization, Site Extension Integration Notes

### Community 405 - "Single Active Room Global Constraints"
Cohesion: 0.67
Nodes (3): Exact-session Cleanup Rule, Single Active Room Global Constraints, Migration-first Additive Boundary

### Community 406 - "Persistent Exact Operation Generation"
Cohesion: 0.67
Nodes (3): Authoritative Room Snapshot Handoff, Persistent Exact Operation Generation, Late Admission Compensation

### Community 407 - "Panel Access and Focus Overrides"
Cohesion: 0.67
Nodes (3): Main Control Always Visible, Main Control Auto Hide, Panel Access and Focus Overrides

### Community 408 - "Build Extension Workflow"
Cohesion: 0.67
Nodes (3): Build Extension Workflow, Extension Channel, Release Ref Validation

### Community 409 - "Camera Interaction Shell"
Cohesion: 0.67
Nodes (3): Camera Interaction Shell, Overlay Interaction Boundary, Player Hover Interference Prevention

## Ambiguous Edges - Review These
- `Codex-Hosted Semantic Extraction` → `Automated Verification and Staging Artifact Evidence`  [AMBIGUOUS]
  docs/superpowers/plans/2026-07-30-interface-visibility-settings.md · relation: implements

## Knowledge Gaps
- **3448 isolated node(s):** `CompareCellValue`, `CompareColumn`, `GuideLinkItem`, `LegacyRoomSessionRecord`, `PageSessionStorageLike` (+3443 more)
  These have ≤1 connection - possible missing edges or undocumented components. (Counts symbols only; 4551 node(s) total have ≤1 connection when file, concept and rationale nodes are included.)
- **248 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **What is the exact relationship between `Codex-Hosted Semantic Extraction` and `Automated Verification and Staging Artifact Evidence`?**
  _Edge tagged AMBIGUOUS (relation: implements) - confidence is low._
- **Why does `react` connect `react` to `voice-audio-preferences.ts`, `.next/**`, `privileged-overlay-wiring.test.tsx`, `seo-guide-blocks.tsx`, `primary-checkout-cta.tsx`, `overlay-app.tsx`, `reaction-pop.tsx`, `protocol/src/index.ts`, `popup-app.tsx`, `contact-messages.ts`, `panel-account-title.tsx`, `overlay-layout-ghost-preview.tsx`, `use-camera-interaction-lock.ts`, `popup-watch-history.tsx`, `crm-client.tsx`, `[slug]/page.tsx`, `overlay-interface-settings.tsx`, `overlay-room-rail.tsx`, `watch-library-client.tsx`, `Participant`, `room-media-defaults.ts`, `popup-people-model.ts`, `popup-watch-browse.test.tsx`, `chrome-extension-demo-async-overlay.tsx`, `friends-client.tsx`, `playback-sync-controller.ts`, `reaction-shortcuts.ts`, `ghost-cam.ts`, `overlay-layout-editor.tsx`, `overlay-layout-editor.test.tsx`, `react-dom/client`, `account-inbox-cache.ts`, `getSession`, `invites-client.tsx`, `[roomId]/page.tsx`, `popup-watch-history.test.tsx`, `feature-requests.ts`?**
  _High betweenness centrality (0.099) - this node is a cross-community bridge._
- **Why does `.next/**` connect `.next/**` to `getGuideLinks`, `getResolvedSiteOrigin`, `seo-guide-blocks.tsx`, `primary-checkout-cta.tsx`, `react`, `tasks`, `pricing-copy.ts`, `watch-history-v3-routes.ts`, `sitemap-discovery.ts`, `[slug]/page.tsx`, `verifyKreatliCrmSession`, `watch-anime-together/page.tsx`, `about/page.tsx`, `youtube/storage.ts`, `connect/page.tsx`, `pseo-new-guides.tsx`, `account/feature-requests/page.tsx`, `getSession`, `invites-client.tsx`, `best-anime-to-watch-with-friends/page.tsx`, `best-anime-movies-to-watch-with-friends/page.tsx`, `best-short-anime-to-watch-with-friends/page.tsx`, `[roomId]/page.tsx`?**
  _High betweenness centrality (0.065) - this node is a cross-community bridge._
- **Why does `vitest` connect `vitest` to `overlay-app.tsx`, `auth-client.ts`, `room-invite-notifications.ts`, `protocol/src/index.ts`, `room-session.ts`, `room-session-storage.ts`, `room-client.ts`, `popup-app.tsx`, `watch-history.ts`, `account.ts`, `src/types.ts`, `api/src/index.ts`, `VideoAdapter`, `popup-watch-history.tsx`, `social-client.ts`, `overlay-interface-settings.tsx`, `crunchyroll/progress.ts`, `overlay-room-rail.tsx`, `room-departure-retry.ts`, `RoomState`, `room-media-defaults.ts`, `popup-people-model.ts`, `privileged-overlay-intent.ts`, `watch-history-client.ts`, `playback-sync-controller.ts`, `room-source.ts`, `room-persistence.ts`, `reaction-shortcuts.ts`, `ghost-cam.ts`, `watch-history-storage.ts`, `room-departure.ts`, `participant-disconnect.ts`, `room-hibernation-runtime.ts`, `watch-history-controller.ts`, `extension-codes.ts`, `overlay-layout-editor.tsx`, `WatchSourceDescriptor`, `youtube/player-chrome.ts`, `p2p-media.test.ts`, `room-socket-attachment.ts`, `bridge-client.ts`, `react-dom/client`, `account-inbox-cache.ts`, `YouTubeVideoAdapter`, `api/src/room-lifecycle.ts`, `room-source-persistence.ts`, `src/account-inbox-client.ts`, `overlay-layout-model.ts`, `crunchyroll/player-chrome.ts`, `popup-watch-history.test.tsx`, `ice-servers.ts`, `debug-log.ts`, `voice-audio-preferences.ts`, `catalog.ts`, `privileged-overlay-wiring.test.tsx`, `hotkeys.ts`, `reaction-pop.tsx`, `node:fs`, `artwork-select.ts`, `user-identity.ts`, `p2p-ice.ts`, `use-camera-interaction-lock.ts`, `voice-activity.ts`, `overlay-layout-runtime.ts`, `extension/test/watch-history-browse.test.ts`, `RecentP2PSignalBuffer`, `Participant`, `popup-watch-browse.test.tsx`, `background-privileged-room-route.test.ts`, `youtube/progress.ts`, `room-invite-target-status.ts`, `overlay-mount.ts`, `release-channel-build.test.ts`, `ghost-cam-size.ts`, `overlay-layout-editor.test.tsx`, `social-snapshot-cache.ts`, `popup-styles.ts`, `overlay-voice-session.ts`, `room-metering.ts`, `overlay-layout-engine.test.ts`, `room-invite-notification-runtime.test.ts`, `panel-account-title.tsx`, `overlay-layout-ghost-preview.tsx`, `overlay-media-session.ts`, `extension/src/watch-history-browse.ts`, `room-quota-display.ts`, `room-tab-lock.ts`, `overlay-layout.ts`, `diagnostic-log.test.ts`, `background-invite-notification-wiring.test.ts`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Are the 13 inferred relationships involving `OverlayApp()` (e.g. with `fallbackCopy()` and `handleOnline()`) actually correct?**
  _`OverlayApp()` has 13 INFERRED edges - model-reasoned connections that need verification._
- **What connects `CompareCellValue`, `CompareColumn`, `GuideLinkItem` to the rest of the system?**
  _3448 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `.next/**` be split into smaller, more focused modules?**
  _Cohesion score 0.010434248887524936 - nodes in this community are weakly interconnected._
Token accounting: Codex agent usage is unavailable; zero fields are placeholders, not measured zero cost.

## Watch Episode Catalog API (2026-09-06)

- Scoped update: 9 code files and 2 canonical documents; 45 nodes and 101 edges added.
- Authenticated `GET /api/watch-history/v3/browse/catalog` reads accepted owner/generation-bound snapshots and at most 50 personal episode rows.
- Separate main/Specials totals, current-region evidence, cursor/revision checks and final personal-page checks preserve catalog and history boundaries.
- Exact extension-bearer GET reaches normal route authentication through the staging gate. Server-only staging authorization is separate from the local grid UI and production promotion.
- Existing document concepts were confirmed by a fresh full read and retained; unchanged communities and absent ignored query-memory sources were preserved.
- Integrity: zero missing or dangling endpoints, self-loops, duplicates or collapsed edges.
