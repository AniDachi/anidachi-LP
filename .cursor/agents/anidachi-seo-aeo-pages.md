...
- **Commercial / high intent** (e.g. “watch Crunchyroll with friends”, “youtube watch party”, “anime watch party extension”, product vs competitor): prioritize **pillars** (`/watch-anime-together`, `/watch-crunchyroll-together`, `/watch-youtube-together`, `/anime-watch-party-toolkit`), **compare** URLs, and “how-to” guides where the reader is deciding what to use. Lead with **clear above-the-fold value** and path to **`/extension`** (“what it is → why AniDachi → install”). `/pricing` is Plus/Pro upgrade only. Use **`aboveFoldCta`**. `SeoPageLayout` already places one install card after the answer and one at the end of the article. That is the cap: **two install cards**. Do **not** add another `PrimaryCheckoutCta` in page children, and do not use `midContentSlot` for a third card.
...
2. **Problem → mechanism → `/extension`**: `aboveFoldCta` only. Layout adds the end-of-article card. Do not add a third `PrimaryCheckoutCta`.
...
6. **Hard links**: pillar `/watch-youtube-together` in the answer + `/extension` after install steps + `/pricing` only when the sentence is about Plus/Pro + one sibling spoke. Related via `getGuideLinks({ includeTags: ["pillar-watch-youtube"], ... })` only (avoid broad `watch-party` OR that pulls anime cluster).
...
6. **Hard links**: pillar `/watch-crunchyroll-together` in the answer + `/extension` after install steps + `/pricing` only when the sentence is about Plus/Pro + one sibling spoke. Related via `getGuideLinks({ includeTags: ["pillar-watch-crunchyroll"], ... })` **only** — ban OR-ing bare `watch-party` / `how-to-core` / `compare` on CR spokes (wrong-cluster pollution). Soft-link Anime hub in body when useful; do not nest CR under Anime in crumbs.
...
9. **Primary how-to parity**: `/guides/how-to-watch-crunchyroll-with-friends` must ship `HowToJsonLd` + `aboveFoldCta` + `/extension` install wording (mirror `/guides/how-to-watch-youtube-with-friends`). Do not add a mid install card.
...
- **One primary action** per page: **`/extension`** until Chrome Web Store is live; `/pricing` is the upgrade path. Stay truthful to home/install.
- **Two install cards maximum**: `aboveFoldCta` plus the layout’s end-of-article card. Do not add `PrimaryCheckoutCta` inside page children.
...
| `pillar` (`conversionTemplate` on `/watch-party-app`) | `app/watch-party-app/page.tsx` |
