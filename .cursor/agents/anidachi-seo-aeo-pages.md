...
2. **Problem → mechanism → `/extension`**: fold CTA (`aboveFoldCta`) plus an inline **`PrimaryCheckoutCta`** (`placement="content_mid"`) after the problem section on commercial intents (alternatives, free, Discord pain, host, switcher listicles).
...
1. **Answer-first** H2 with strong lead that names AniDachi when commercial.
2. **Problem → mechanism → `/extension`**: fold CTA (`aboveFoldCta`) plus inline **`PrimaryCheckoutCta`** (`placement="content_mid"`) after the problem section on commercial intents (alternatives, free, Discord pain, host, sync, primary how-to, switcher listicles).
...
9. **Primary how-to parity**: `/guides/how-to-watch-crunchyroll-with-friends` must ship `HowToJsonLd` + `aboveFoldCta` + mid CTA + `/extension` install wording (mirror `/guides/how-to-watch-youtube-with-friends`).
