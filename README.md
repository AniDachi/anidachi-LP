...
- `NEXT_PUBLIC_SITE_URL` — canonical origin for metadata, sitemap, robots, JSON-LD (production: `https://www.anidachi.app`; on Vercel previews, `VERCEL_URL` is used when this is unset)
...
In Codex, use `$graphify .` for a full assistant-backed graph and
`$graphify . --update` after changing docs or other semantic inputs. This path
uses Codex subagents and needs no separate provider key. `pnpm graph:update:code`
is intentionally AST-only. The headless `pnpm graph:extract` command is for
CLI/CI usage and needs a Graphify backend when docs, PDFs, or images are included.
