...
async function runScenarios() {
  ...
  const iceWrongRoom = await fetch(`${HTTP_BASE}/rooms/other-room/ice-servers`, {
    headers: { Authorization: `Bearer ${iceToken}` },
  });
  record(
    "ice-servers rejects token for a different room (401)",
    iceWrongRoom.status === 401,
    `status=${iceWrongRoom.status}`,
  );
  const legacyIce = await fetch(
