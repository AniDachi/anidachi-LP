...
const NAME = "Anime Junkie (Early Access)";
const AMOUNT_USD = 38;
...
async function ensurePrice(stripe: Stripe) {
  ...
  const product =
    ...
    (await stripe.products.create({
      name: NAME,
      description:
        "Advanced room controls + founder support. Works with your own Crunchyroll login.",
    }));
...
async function main() {
  ...
  process.stdout.write(
    `Created/verified ${mode} Anime Junkie price:\n${price.id}\n`
  );
}
