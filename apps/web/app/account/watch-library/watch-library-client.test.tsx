...
function accessFixture(
  state: "allowed" | "plan_required" = "allowed",
  generation = 1,
  owner = OWNER_ID,
) {
  return {
...
function capacityFixture(crunchyroll = 200, owner = OWNER_ID, generation = 1) {
  return {
    ...
    serverTime: NOW,
    providers: {
      youtube: { used: 12, limit: 100 },
      crunchyroll: { used: crunchyroll, limit: 200 },
    },
  };
}
...
it("website detail pages merge by canonical episode identity and retain continuation", () => {
  const merged = mergeWatchHistoryTitleEpisodePage(
    itemFixture(),
    detailFixture(),
  );
  const episodes = merged.seasons.flatMap((season) => season.episodes);
  assert.deepEqual(
    episodes.map((value) => value.episodeKey),
    ["episode-one", "episode-two", "episode-three"],
  );
  assert.equal(
    episodes.find((value) => value.episodeKey === "episode-two")?.currentTime,
    120,
  );
  assert.deepEqual(merged.episodePage, {
    complete: false,
    nextCursor: "cursor-two",
  });
...
it("website detail pages retain represented seasons omitted by bounded catalog metadata", () => {
  ...
  assert.deepEqual(
    merged.seasons.map((season) => season.seasonKey),
    ["season-zero", "season-one"],
  );
  assert.equal(merged.seasons[0]?.seasonTitle, "Canonical Season Zero");
  assert.deepEqual(
    merged.seasons[0]?.episodes.map((value) => value.episodeKey),
    ["episode-zero", "episode-three"],
  );
...
it("website optimistic episode deletion preserves the last canonical aggregate", () => {
  ...
  assert.deepEqual(
    next.items[0]?.seasons[0]?.aggregate,
    item.seasons[0]!.aggregate,
  );
});

it("website renders complete, observed-only, and zero-available title states honestly", () => {
  const render = (item: WatchHistoryItem) =>
    renderToStaticMarkup(
      React.createElement(WatchLibraryClient, {
  ...
  assert.equal(
    getWatchHistoryAggregateLabel(exact),
    "5 / 13 episodes · 38.46%",
  );
...
function installServer(
  ...
  globalThis.fetch = async (input, init) => {
    ...
    const override = await options.intercept?.(path, init);
    if (override) return override;
    if (
      path.includes("/editor?") &&
      new URLSearchParams(path.split("?")[1]).get("titleKey") !==
        editor.titleKey
    )
    ...
    if (path.endsWith("/editor") && init?.method === "POST") {
      ...
      editor = {
        ...
        episodes: editor.episodes.map((ep) => {
          const change = body.changes.find(
            (value) => value.episodeKey === ep.episodeKey,
          );
          return change
            ? {
                ...ep,
                watched: change.watched,
                currentTime: change.watched ? ep.duration : 0,
                progress: change.watched ? 1 : 0,
              }
            : ep;
        }),
      };
    ...
    if (path.endsWith("/access"))
      return Response.json(accessFixture(options.plan));
    ...
    if (path.endsWith("/delete")) {
      const target = (
        JSON.parse(String(init?.body)) as { target: WatchHistoryDeleteScope }
      ).target;
      const generation =
        history.meta.accountGeneration + (target.scope === "all" ? 1 : 0);
      history = {
        ...history,
...
async function openTitle(container: HTMLElement) {
  ...
  await waitFor(() =>
    assert.ok(container.querySelector('[aria-label="Episodes"]')),
  );
}
async function chooseSeason(container: HTMLElement, value: string) {
  const select = container.querySelector(
    'select[aria-label="Season or specials"], .wh-season-row select',
...
function episodeButton(container: HTMLElement, text: string) {
  return [...container.querySelectorAll<HTMLButtonElement>(".wh-episode")].find(
...
async function selectSeason(container: HTMLElement) {
  await act(async () => {
    container
      .querySelector<HTMLInputElement>(".wh-select-season input")!
      .click();
  });
}
async function markFirstEpisode(container: HTMLElement) {
  await click(
    [...container.querySelectorAll<HTMLButtonElement>(".wh-episode")].find(
...
it("SSR includes Free's saved covers with a recording notice and no open editor", () => {
  const html = renderToStaticMarkup(
    <WatchLibraryClient
      initialHistory={historyFixture()}
      initialPreferences={preferencesFixture}
      initialAccess="plan_required"
    />,
  );
  assert.match(html, /Manage Series One/);
  assert.match(html, /Plus or Pro unlocks recording/);
  assert.doesNotMatch(html, /wh-inspector/);
  assert.match(html, /Clear all history/);
  assert.doesNotMatch(
    html,
    /YouTube history:|aria-label="Track YouTube history"/,
  );
...
it("SSR refuses unavailable access and an access change during the read", async () => {
  ...
  await assert.rejects(
    loadWatchLibraryData(OWNER_ID, {
      access: async () =>
        accessFixture(++checks === 1 ? "allowed" : "plan_required"),
      preferences: async () => preferencesFixture,
...
it("storage counts and full warnings follow the platform without refetching or counting visible cards", async () => {
  ...
  try {
    const storage = () =>
      view.container.querySelector('[aria-label="History storage"]')
        ?.textContent ?? "";
...
it("bulk clearing stays in Library options and requires confirmation on Free", async () => {
  ...
  try {
    const options = view.container.querySelector<HTMLDetailsElement>(
      ".wh-page-actions details",
    )!;
...
it("platform switching completes pagination and keeps the progress filter", async () => {
  ...
  try {
    const status = view.container.querySelector<HTMLSelectElement>(
    ...
    await waitFor(() =>
      assert.ok(
        view.container.querySelector('[aria-label="Manage Video One"]'),
      ),
    );
    assert.equal(
      view.container.querySelector('[aria-label="Manage Series One"]'),
      null,
    );
    ...
    assert.equal(
      server.calls.filter((call) => call.path.includes("cursor=next-page"))
        .length,
      1,
    );
    await click(buttonByLabel(view.container, "Crunchyroll"));
...
it("drafts survive season changes and Save sends one atomic owner-bound command", async () => {
  ...
  try {
    ...
    assert.match(
      episodeButton(view.container, "Episode 1").className,
      /wh-watched/,
    );
    await click(buttonByLabel(view.container, "Save 2 changes"));
    await waitFor(() =>
      assert.match(view.container.textContent ?? "", /Progress saved/),
    );
    const writes = server.calls.filter(
      (call) => call.path.endsWith("/editor") && call.body,
    );
    assert.equal(writes.length, 1);
    assert.equal(writes[0].headers.get(WATCH_HISTORY_OWNER_HEADER), OWNER_ID);
    assert.deepEqual(writes[0].body?.changes, [
      { episodeKey: "episode-one", watched: true },
      { episodeKey: "episode-three", watched: true },
    ]);
    assert.equal(
      server.getEditor().episodes.filter((ep) => ep.watched).length,
      2,
    );
  } finally {
    await unmount(view.root);
...
it("selection never changes progress; explicit actions create a draft and Undo restores partial progress", async () => {
  ...
  try {
    ...
    assert.equal(
      buttonByLabel(view.container, "Save 0 changes").disabled,
      true,
    );
    ...
    assert.equal(
      view.container
        .querySelector('[aria-label="Episode progress"]')
...
it("season selection is indeterminate, scoped to the current season and excludes future episodes", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(
      view.container.querySelector(".wh-inspector")!.textContent!,
      /Reset all title progress/,
    );
    await click(episodeButton(view.container, "Episode 1"));
    assert.equal(
      view.container.querySelector<HTMLInputElement>(".wh-select-season input")!
        .indeterminate,
      true,
    );
    ...
    assert.equal(
      episodeButton(view.container, "Future episode").getAttribute(
        "aria-checked",
      ),
      "false",
    );
    ...
    const write = server.calls.find(
      (call) => call.path.endsWith("/editor") && call.body,
    );
    assert.deepEqual(write?.body?.changes, [
      { episodeKey: "episode-one", watched: false },
    ]);
  } finally {
    await unmount(view.root);
  }
...
it("Shift selects a range without changing progress; clearing selection is not clearing history", async () => {
  ...
  try {
    ...
    await click(episodeButton(view.container, "Episode 1"));
    await act(async () => {
      episodeButton(view.container, "Episode 2").dispatchEvent(
...
it("single films use the same top controls and keep an unknown duration unknown", async () => {
  const history = historyFixture();
  history.items[0].itemKind = "movie";
  const editor = editorFixture();
  editor.episodes = [
    { ...editor.episodes[0], duration: 0, currentTime: 60, progress: 0 },
  ];
  const server = installServer({ history, editor });
  const view = await renderClient(history);
  try {
...
it("unavailable entries with old progress may be selected for clearing, never for completion", async () => {
  ...
  const server = installServer({ editor });
  const view = await renderClient();
  try {
    await openTitle(view.container);
    await click(buttonByText(view.container, "Edit"));
    ...
    assert.equal(buttonByText(view.container, "Mark watched").disabled, true);
    assert.equal(
      buttonByText(view.container, "Clear progress").disabled,
      false,
    );
    await click(buttonByText(view.container, "Clear progress"));
    await click(buttonByLabel(view.container, "Save 1 change"));
    assert.deepEqual(
      server.calls.find((call) => call.path.endsWith("/editor") && call.body)
        ?.body?.changes,
      [{ episodeKey: "future", watched: false }],
    );
  } finally {
    await unmount(view.root);
  }
...
it("season mark excludes future episodes; specials remain a named dropdown section", async () => {
  ...
  try {
    ...
    await click(buttonByText(view.container, "Edit"));
    assert.equal(
      episodeButton(view.container, "Future episode").disabled,
      true,
    );
    await selectSeason(view.container);
    await click(buttonByText(view.container, "Mark watched"));
    ...
    assert.equal(
      server.getEditor().episodes.find((ep) => ep.episodeKey === "future")!
        .watched,
      false,
    );
  } finally {
...
it("Free opens series and resumes its own saved position without edit or room creation", async () => {
  ...
  testWindow.location.assign = (url) => {
  ...
  try {
    await openTitle(view.container);
    assert.equal(buttonByText(view.container, "Edit").disabled, true);
    await click(buttonByText(view.container, "Resume"));
    await waitFor(() => assert.equal(launched.length, 1));
    const intent = JSON.parse(
      new URLSearchParams(new URL(launched[0]).hash.slice(1)).get(
        "anidachiResume",
      )!,
    );
    assert.equal(intent.currentTime, 60);
    assert.equal(intent.accountGeneration, 1);
    assert.equal(launched[0].includes("anidachiRoom"), false);
    assert.equal(
      server.calls.some((call) => call.body),
      false,
    );
  } finally {
    testWindow.location.assign = oldAssign;
    await unmount(view.root);
  }
});
for (const plan of ["allowed", "plan_required"] as const)
  for (const state of ["new", "started", "watched"] as const)
    it(`${plan} opens a ${state} episode despite an unavailable catalog hint`, async () => {
      const editor = editorFixture();
      const selected = editor.episodes[0];
      ...
      const server = installServer({ plan, editor });
      const oldAssign = testWindow.location.assign;
      const launched: string[] = [];
      testWindow.location.assign = (url) => {
        launched.push(String(url));
      ...
      try {
        ...
        assert.equal(
          intent.currentTime,
          state === "watched" ? 0 : selected.currentTime,
        );
        ...
        const nextIntent = JSON.parse(
          new URLSearchParams(new URL(launched[1]).hash.slice(1)).get(
            "anidachiResume",
          )!,
        );
...
it("a conflict preserves the draft and requires explicit review of latest progress", async () => {
  ...
  const server = installServer({
    intercept: (path, init) =>
      path.endsWith("/editor") && init?.method === "POST" && conflicts++ === 0
        ? Response.json(
            {
              error: "Progress changed elsewhere",
              code: "HISTORY_EDIT_CONFLICT",
            },
            { status: 409 },
          )
        : undefined,
  ...
  try {
    ...
    await waitFor(() =>
      assert.ok(
        buttonByText(view.container, "Load latest & review my changes"),
      ),
    );
    ...
    assert.equal(
      buttonByLabel(view.container, "Save 1 change").disabled,
      false,
    );
    assert.doesNotMatch(
      view.container.querySelector(".wh-selection-feedback")?.textContent ?? "",
      /Undo/,
    );
    assert.equal(
      server.calls.filter((call) => call.path.endsWith("/editor")).length,
      1,
    );
  } finally {
    await unmount(view.root);
...
it("network retry keeps the exact mutation id and payload", async () => {
  ...
  try {
    ...
    await click(buttonByLabel(view.container, "Save 1 change"));
    await waitFor(() =>
      assert.match(view.container.textContent ?? "", /Connection lost/),
    );
    await click(buttonByText(view.container, "Retry"));
    await waitFor(() =>
      assert.match(view.container.textContent ?? "", /Progress saved/),
    );
    const writes = server.calls.filter((call) => call.path.endsWith("/editor"));
...
it("changing title asks Save/Discard/Stay and does not silently lose the draft", async () => {
  ...
  installServer({ history });
  const view = await renderClient(history);
  try {
    await openTitle(view.container);
    await click(buttonByText(view.container, "Edit"));
    await markFirstEpisode(view.container);
    await click(buttonByLabel(view.container, "Manage Series Two"));
    assert.ok(buttonByText(view.container, "Stay here"));
    ...
    await waitFor(() =>
      assert.ok(
        view.container.querySelector(
          'dialog[aria-label="Series Two progress"]',
        ),
      ),
    );
  } finally {
...
it("deletion refreshes server capacity and retains privacy actions on Free", async () => {
  ...
  try {
    await openTitle(view.container);
    await click(buttonByText(view.container, "Remove from history"));
    await waitFor(() =>
      assert.match(view.container.textContent ?? "", /199 \/ 200/),
    );
    assert.doesNotMatch(view.container.textContent ?? "", /Series One/);
    assert.equal(
      server.calls.filter((call) => call.path.endsWith("/delete")).length,
      1,
    );
    assert.equal(
      buttonByText(view.container, "Clear all history").disabled,
      false,
    );
  } finally {
    await unmount(view.root);
...
it("old root responses cannot resurrect a deleted title", async () => {
  ...
  installServer({
    intercept: (path) =>
      path.includes("?limit=24") && ++reads === 1
        ? new Promise((resolve) => {
            resolveOld = resolve;
          })
        : undefined,
...
it("an old editor response cannot reveal data after switching account", async () => {
  ...
  try {
    ...
    await waitFor(() => assert.ok(resolveOld));
    const history = historyFixture();
    history.meta.ownerUserId = "22222222-2222-4222-8222-222222222222";
    history.items[0].title = "Another account";
    const preferences = structuredClone(preferencesFixture);
    preferences.meta.ownerUserId = history.meta.ownerUserId;
    await act(async () =>
      view.root.render(
        <WatchLibraryClient
    ...
    assert.doesNotMatch(
      view.container.textContent ?? "",
      /Episode 1|Series One/,
    );
    assert.match(view.container.textContent ?? "", /Another account/);
  } finally {
    await unmount(view.root);
  }
});
  ...
  for (const authority of [false, true]) {
    ...
    try {
      ...
      if (authority) {
        ...
        await waitFor(() => assert.ok(buttonByText(view.container, "Retry")));
...
it("poster failure before hydration has a fallback and a new URL can load", async () => {
  ...
  history.items[0].artworkUrl = "https://www.crunchyroll.com/old.jpg";
  installServer({
    intercept: (path) =>
      path.includes("?limit=24") ? Response.json(history) : undefined,
  });
  const element = (
    <WatchLibraryClient
      initialHistory={history}
  ...
  try {
    ...
    assert.ok(container.querySelector(".wh-artwork-fallback"));
    history = structuredClone(history);
    history.items[0].artworkUrl = "https://www.crunchyroll.com/new.jpg";
    await click(buttonByLabel(container, "Refresh history"));
    await waitFor(() =>
      assert.equal(
        container.querySelector("img")?.getAttribute("src"),
        history.items[0].artworkUrl,
      ),
    );
  } finally {
    await unmount(root);
...
it("sign-out waits for the editor decision before revoking the session", async () => {
  ...
  try {
    ...
    await click(buttonByText(view.container, "Edit"));
    await markFirstEpisode(view.container);
    const request = new testWindow.CustomEvent("anidachi:before-sign-out", {
      cancelable: true,
      detail: async () => {
        signedOut = true;
      },
    ...
    await act(async () => {
      assert.equal(testWindow.dispatchEvent(request), false);
    });
    assert.equal(signedOut, false);
    await click(buttonByText(view.container, "Stay"));
    assert.equal(signedOut, false);
    await act(async () => {
      testWindow.dispatchEvent(
        new testWindow.CustomEvent("anidachi:before-sign-out", {
          cancelable: true,
          detail: async () => {
            signedOut = true;
          },
...
it("joining from notifications waits for the history editor decision", async () => {
  ...
  try {
    ...
    const request = () =>
      testWindow.dispatchEvent(
        new testWindow.CustomEvent("anidachi:before-account-navigation", {
          cancelable: true,
          detail: async () => {
            joined = true;
          },
