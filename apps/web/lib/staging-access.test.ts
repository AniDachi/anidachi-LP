...
test("staging gate bypasses extension token endpoints and allowed bearer API calls", () => {
  ...
  assert.equal(
    canBypassStagingGate({
      pathname: "/api/account/inbox/seen",
      method: "POST",
      authorization: "Bearer token",
    }),
    true,
  ...
  assert.equal(
    canBypassStagingGate({
      pathname: "/api/devices/push-subscription",
      method: "POST",
      authorization: "Bearer token",
    }),
    true,
  ...
  assert.equal(
    canBypassStagingGate({
      pathname: "/api/devices/device_1/push-subscription",
      method: "DELETE",
      authorization: "Bearer token",
    }),
    true,
...
test("staging gate lets an extension bearer reach only the supported Watch History v2 methods", () => {
  ...
  for (const [pathname, method] of allowedRequests) {
    assert.equal(
      ...
      true,
      `${method} ${pathname} must reach the route bearer verifier`,
    );
    assert.equal(
      canBypassStagingGate({ pathname, method }),
      false,
      `${method} ${pathname} must remain password-gated without a bearer`,
    );
  }
