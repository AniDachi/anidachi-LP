...
export const PRICING_PLUS_VS_PRO_ANSWER =
  `Plus (${PRICING_PLUS_SHORT}) removes the Free daily host limit, raises the room to ${plusPolicy.maxParticipants} people and ${plusPolicy.maxMicrophones} mics, and lets you record and edit personal progress on Crunchyroll and YouTube. Saved history and Resume remain available on Free. Pro (${PRICING_PRO_SHORT}) is the same features with up to ${proPolicy.maxParticipants} people and ${proPolicy.maxMicrophones} mics, plus priority support and the option to negotiate custom pricing for 8+ person groups. Friends can join any host on Free. ${ASYNC_COMING_SOON} — it is not part of Plus or Pro today.`;
