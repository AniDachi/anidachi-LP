...
test("plan entitlements match the approved MVP pricing matrix", () => {
	...
	assert.equal(free.account.historyEnabled, false);
	assert.equal("historyRetentionDays" in free.account, false);
	...
	assert.equal(plus.account.historyEnabled, true);
	assert.equal("historyRetentionDays" in plus.account, false);
	...
	assert.equal(pro.account.historyEnabled, true);
	assert.equal("historyRetentionDays" in pro.account, false);
});
