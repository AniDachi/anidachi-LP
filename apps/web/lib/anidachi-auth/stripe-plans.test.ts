...
test("active subscriptions require an unexpired known period; cancellation flag is not early expiry", () => {
	const now = new Date("2026-09-08T12:00:00Z");
	for (const currentPeriodEnd of [null, "invalid"])
		assert.throws(
			() =>
				effectivePlanForSubscription(
					{ planCode: "pro", status: "active", currentPeriodEnd },
					now,
				),
			/unavailable/,
		);
		...
		effectivePlanForSubscription(
			{
				planCode: "plus",
				status: "active",
				currentPeriodEnd: "2026-09-08T12:00:01Z",
			},
			now,
		),
		"plus",
