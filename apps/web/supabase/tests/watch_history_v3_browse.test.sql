...
create function pg_temp.watch(uid uuid, ep text default 'E1', source_gen int default 1, obs timestamptz default clock_timestamp(), title_id text default 'S', room_gen int default 1) returns jsonb language plpgsql as $$
declare e jsonb; a jsonb;
begin
e:=jsonb_build_object('schemaVersion',3,'clientEventId',gen_random_uuid(),'clientSessionKey','browse-'||uid||'-'||ep,'accountGeneration',1,
