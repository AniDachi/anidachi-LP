...
-- The original single-active-room migration was already applied to staging.
-- Harden its admission RPCs with a forward-only replacement so every
-- environment follows the same immutable migration history.
...
create or replace function public.claim_active_room_session_v1(
  p_user_id uuid,
  p_room_id text,
  p_role text,
  p_participant_session_id text
