...
describe("overlay layout model v2", () => {
	it("uses the V2-only local storage key", () => {
		expect(OVERLAY_LAYOUT_STORAGE_KEY_V2).toBe(
			"local:overlayLayoutPreferencesV2",
		);
	});
	...
	it("normalizes malformed fields without mutating the input", () => {
		...
		expect(normalized.video).toMatchObject({ leaderSide: "left", sizeStep: 4 });
	...
	it("supports a one-column chat at the right edge of the grid", () => {
		...
		expect(belowMinimum.chat).toMatchObject({
			position: { x: 11, y: 4 },
			width: 1,
		});
	});
	...
	it("clamps message counts to the supported product range", () => {
		expect(
			normalizeOverlayLayoutDefinition({ chat: { maxMessages: -10 } }).chat
				.maxMessages,
		).toBe(3);
		expect(
			normalizeOverlayLayoutDefinition({ chat: { maxMessages: 999 } }).chat
				.maxMessages,
		).toBe(64);
	});
	...
	it("normalizes chat message transparency without allowing full invisibility", () => {
		expect(
			normalizeOverlayLayoutDefinition({ chat: { messageTransparency: -10 } })
				.chat.messageTransparency,
		).toBe(0);
		expect(
			normalizeOverlayLayoutDefinition({ chat: { messageTransparency: 65 } })
				.chat.messageTransparency,
		).toBe(65);
		expect(
			normalizeOverlayLayoutDefinition({ chat: { messageTransparency: 120 } })
				.chat.messageTransparency,
		).toBe(95);
	});

	it("preserves the extra-large chat text scale", () => {
		expect(
			normalizeOverlayLayoutDefinition({ chat: { textScale: "xlarge" } }).chat
				.textScale,
		).toBe("xlarge");
	});
