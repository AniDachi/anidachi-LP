...
describe("InterfaceSettingsPanel", () => {
  ...
  it("renders only the approved interface visibility choices", async () => {
    ...
    expect(getButton(view.container, "Auto hide").getAttribute("aria-checked")).toBe("true");
    expect(getButton(view.container, "Always visible", 0).getAttribute("aria-checked")).toBe(
      "false",
    );
    expect(getButton(view.container, "Smart").getAttribute("aria-checked")).toBe("true");
    expect(getButton(view.container, "Always visible", 1).getAttribute("aria-checked")).toBe(
      "false",
    );
  ...
  it("plays the preview once and lets the user replay it", async () => {
    vi.useFakeTimers();
    const view = await renderPanel();
    const preview = getPreview(view.container);
    ...
    await advance(INTERFACE_PREVIEW_STEP_MS * 3);
    expect(preview.getAttribute("data-preview-moment")).toBe("interaction");
    await advance(INTERFACE_PREVIEW_STEP_MS * 3);
    expect(preview.getAttribute("data-preview-moment")).toBe("interaction");

    await click(getButton(view.container, "Replay interface preview"));
    expect(preview.getAttribute("data-preview-moment")).toBe("idle");
  ...
  it("disables choices while loading and exposes saving and error status", async () => {
    ...
    expect([...view.container.querySelectorAll('[role="radio"]')].every(isDisabled)).toBe(true);
  ...
  it("keeps preview silhouettes decorative", async () => {
    ...
    expect(
      [...view.container.querySelectorAll(".interface-settings-silhouette")].every(
...
async function renderPanel(overrides: PanelOverrides = {}): Promise<RenderedPanel> {
  ...
  return {
    ...
    rerender: async (preferences) => {
      await act(async () => {
        root.render(<InterfaceSettingsPanel {...props} preferences={preferences} />);
      });
    },
    root,
  };
...
async function keyboard(button: HTMLButtonElement, init: KeyboardEventInit): Promise<void> {
  await act(async () => {
...
function getRadioGroup(container: HTMLElement, name: string): HTMLElement | null {
  return container.querySelector(`[role="radiogroup"][aria-label="${name}"]`);
}

function getButton(container: HTMLElement, name: string, index = 0): HTMLButtonElement {
