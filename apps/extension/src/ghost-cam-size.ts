...
export function getResponsiveGhostCamSizePx(
  ...
  if (!isUsefulDimension(containerWidthPx) || !isUsefulDimension(containerHeightPx)) {
    return baseSizePx;
  }
  ...
  const playerScale =
    clamp01((shortSidePx - 540) / 420) * 0.72 + clamp01((longSidePx - 1280) / 640) * 0.28;
  ...
  const stackShare = getCameraStackWidthShare(safeCameraCount);
  const maxByStackWidthPx =
    safeCameraCount === 1
      ? Number.POSITIVE_INFINITY
      : Math.floor((containerWidthPx * stackShare - gapPx * (safeCameraCount - 1)) / safeCameraCount);
...
function getCameraStackWidthShare(cameraCount: number): number {
  if (cameraCount >= 4) {
    return 0.42;
  }
  if (cameraCount === 3) {
