...
const CHAT_TEXT_METRICS = {
	...
	xlarge: { fontSizePx: 17, lineHeightPx: 21 },
} as const;
...
export function resolveVideoLayout(
	...
	const occupiedCount = clampInteger(
		cameraCount,
		0,
		OVERLAY_LAYOUT_CAMERA_SLOT_CAPACITY,
	);
	const storedSizeStep = clampInteger(
		video.sizeStep,
		0,
		GHOST_CAM_SIZE_MAX_STEP,
	) as OverlayLayoutCameraSizeStep;
	const sizeStepCap = clampInteger(
		maximumSizeStep,
		0,
		GHOST_CAM_SIZE_MAX_STEP,
	) as OverlayLayoutCameraSizeStep;
...
export function resolveOverlayLayout(
	...
	for (
		let sizeStep = normalizedDefinition.video.sizeStep;
		sizeStep >= 0;
		sizeStep -= 1
	) {
	...
	return fallbackVideo
		? createCameraPriorityFallback(
				normalizedDefinition,
				safeRect,
				fallbackVideo,
			)
		: createMinimumFallback(
				normalizedDefinition,
				normalizedViewport,
				cameraCount,
				safeRect,
...
function createChatLayout(
	...
	return {
		...
		rect: {
			height:
				CHAT_PADDING_Y_PX * 2 + effectiveMaxMessages * rowHeightPx + rowGaps,
			width: Math.max(
				CHAT_PADDING_X_PX * 2,
...
function getPreferredContactAxis(
	...
	const horizontalDistance =
		videoAnchor.x < chatStartX
			? chatStartX - videoAnchor.x
			: videoAnchor.x > chatEndX
				? videoAnchor.x - chatEndX
				: 0;
	const verticalDistance = Math.abs(videoAnchor.y - definition.chat.position.y);
...
function findFreeChatRect(
	...
	const candidates = Array.from(
		{
			length:
				(OVERLAY_LAYOUT_GRID_COLUMNS - chatWidthColumns + 1) *
				OVERLAY_LAYOUT_GRID_ROWS,
		},
		(_, index) => {
			const x = index % (OVERLAY_LAYOUT_GRID_COLUMNS - chatWidthColumns + 1);
			const y = Math.floor(
				index / (OVERLAY_LAYOUT_GRID_COLUMNS - chatWidthColumns + 1),
			);
			return {
				distance:
					Math.abs(x - requestedPosition.x) + Math.abs(y - requestedPosition.y),
				x,
				y,
			};
		},
...
function findNearestVideoContactRect(
	...
	const candidates = [
		{
			...
			rect: {
				...chatRect,
				x: clampedX,
				y: videoBounds.y - LAYOUT_OBJECT_GAP_PX - chatRect.height,
			},
		},
		{
			axis: "vertical" as const,
			rect: {
				...chatRect,
				x: clampedX,
				y: videoBounds.y + videoBounds.height + LAYOUT_OBJECT_GAP_PX,
			},
		},
		{
			...
			rect: {
				...chatRect,
				x: videoBounds.x - LAYOUT_OBJECT_GAP_PX - chatRect.width,
				y: clampedY,
			},
		},
		{
			...
			rect: {
				...chatRect,
				x: videoBounds.x + videoBounds.width + LAYOUT_OBJECT_GAP_PX,
				y: clampedY,
			},
		},
	]
		.filter(
			(candidate) =>
				contactAxis === undefined || candidate.axis === contactAxis,
		)
		.map((rect, index) => ({
			distance:
	...
	return candidates.find(({ rect }) =>
		isAvailableChatRect(rect, safeRect, blockedRects),
	)?.rect;
}
...
function isAvailableChatRect(
	...
	return (
		isWithinSafeRect(rect, safeRect) &&
...
function clampAxisPosition(
	...
	return Math.max(
		safeStart,
		Math.min(position, safeStart + safeSize - objectSize),
	);
}
...
function clampRectToSafeRect(rect: PixelRect, safeRect: PixelRect): PixelRect {
	...
	return {
		...
		x: Math.max(
			safeRect.x,
			Math.min(rect.x, safeRect.x + safeRect.width - width),
		),
		y: Math.max(
			safeRect.y,
			Math.min(rect.y, safeRect.y + safeRect.height - height),
		),
	};
}
...
export function resolveOverlayLayoutSafeRect(
	...
	const right = Math.max(
		left,
		width - finiteNonNegative(viewport.safeInsets.right),
	);
	const bottom = Math.max(
		top,
		height - finiteNonNegative(viewport.safeInsets.bottom),
	);
