...
const KNOWN_CRUNCHYROLL_SEASONS_BY_WATCH_ID: Record<
	string,
	CrunchyrollSeasonMetadata
> = {
	G31UVXQPG: season(1),
	GPWU8KEQG: season(1),
	G2XUN0J2D: season(1),
...
export function inferCrunchyrollSeasonFromSourceUrl(
	...
	return watchId
		? (KNOWN_CRUNCHYROLL_SEASONS_BY_WATCH_ID[watchId] ?? null)
		: null;
}
...
export function normalizeSeasonTitle(
	...
	if (
		!cleaned ||
		cleaned.toLowerCase() === "crunchyroll" ||
		isPlaceholderSeasonTitle(cleaned)
	) {
...
function getCrunchyrollWatchId(
	sourceUrl: string | null | undefined,
): string | null {
