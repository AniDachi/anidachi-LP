...
export { selectCrunchyrollPosterTall } from "./artwork-select";

export async function loadCrunchyrollSeriesPoster(
	seriesId: string,
): Promise<string | null> {
...
export async function loadCrunchyrollPosterArtwork({
	...
	if (isCrunchyrollPage()) {
		const bridgePoster = await loadCrunchyrollPosterFromMainWorld({
			contentId,
			seriesId,
		});
		if (bridgePoster) {
	...
	return selectCrunchyrollPosterTall(
		await loadCrunchyrollCmsObject(relatedSeriesId, token),
	);
}
