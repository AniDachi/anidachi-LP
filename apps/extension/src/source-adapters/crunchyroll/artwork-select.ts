...
export function selectCrunchyrollPosterTall(value: unknown): string | null {
	...
	posters.sort(
		(first, second) =>
			...
			Math.abs(first.width - POSTER_TARGET_WIDTH) -
				Math.abs(second.width - POSTER_TARGET_WIDTH),
	);
...
function collectCrunchyrollArtworkImages(
	value: unknown,
): CrunchyrollArtworkCandidate[] {
...
function readArtworkImages(
	...
	return flattened
		.filter(
			(image): image is { source: string; width: number; height: number } => {
...
function flattenUnknownArray(value: unknown): unknown[] {
	...
	return value.flatMap((item) =>
		Array.isArray(item) ? flattenUnknownArray(item) : [item],
	);
}
...
function findRelatedSeriesId(value: unknown, depth: number): string | null {
	...
	const directSeriesId =
		readNonEmptyString(record.series_id) ?? readNonEmptyString(record.seriesId);
	if (directSeriesId) {
		return directSeriesId;
	}

	const series = asRecord(record.series);
	const nestedSeriesId =
		readNonEmptyString(series?.id) ?? readNonEmptyString(series?.series_id);
	if (nestedSeriesId) {
