...
function startCrunchyrollStudy(): () => void {
		...
		for (const eventName of EVENT_NAMES) {
			const listener = () => {
				if (
					...
					Math.round(performance.now()) % 3 !== 0
				) {
					return;
				}
			...
			disposers.push(() =>
				video.removeEventListener(eventName, listener, true),
			);
		}
	};
	...
	const clickListener = (event: MouseEvent) => {
		const element =
			event.target instanceof Element
				? event.target.closest<HTMLElement>(
						"[data-testid], button, [role='button']",
					)
				: null;
		if (
			!element?.closest(
		...
		record({
			...
			currentTime:
				video instanceof HTMLVideoElement ? finite(video.currentTime, 0) : 0,
			duration:
				video instanceof HTMLVideoElement ? finiteOrNull(video.duration) : null,
			event: "control-click",
	...
	window.addEventListener(
		"anidachi-crunchyroll-study-request",
		requestListener,
	);
	disposers.push(() =>
	...
	disposers.push(() =>
		document.removeEventListener("fullscreenchange", fullscreenListener, true),
	);
	disposers.push(() =>
...
function getCrunchyrollStudySnapshot() {
	...
	const bitmovinContainer = document.querySelector<HTMLElement>(
	...
	return {
		containers: {
			...
			videoPlayerWrapper: elementSnapshot(
...
function getVisibleControls(): StudyElement[] {
	return Array.from(
		...
		.filter((item) => {
			const key =
				`${item.testId ?? ""} ${item.aria ?? ""} ${item.role ?? ""}`.toLowerCase();
...
function readBuffered(video: HTMLVideoElement): Array<[number, number]> {
	...
	for (let index = 0; index < video.buffered.length; index += 1) {
		ranges.push([
			round(video.buffered.start(index)),
			round(video.buffered.end(index)),
		]);
	}
	return ranges;
}
