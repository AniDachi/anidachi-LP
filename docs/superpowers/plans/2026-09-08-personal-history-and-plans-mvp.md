...
- D04: на этапе этого плана ручные отметки были отложены; последующее согласованное
  расширение описано в [плане редактора кабинета](2026-09-10-account-library-editor.md).
  Текущее удаление сохраняется, ручные отметки не создают фиктивные playback sessions.
...
### Free quota countdown follow-up, 2026-09-15

После исчерпания Free-квоты хост видит карточку с обратным отсчетом
`HH:MM:SS`, переходом к тарифам и пояснением, что можно войти в чужую комнату.
Гости получают сообщение об окончании времени хоста; его расход не становится
их личным расходом. Карточка восстанавливается при открытии шторки.

`GET /api/me/room-quota` проверяет bearer/cookie-сессию и заголовок владельца,
заново определяет тариф и возвращает `serverTime`, `remainingSeconds`, `resetAt`
либо `quota: null` для платного доступа. Ответ `private, no-store`; данные клиента
не определяют день или остаток. В staging исключение из парольного экрана
ограничено этим GET с bearer; проверка самой сессии остается в обработчике.

Отсчет привязан к времени приема запроса сервером и `performance.now()` от начала
запроса. Задержка запроса учитывается; небольшой ранний ноль из-за сети приводит
только к повторной серверной проверке. Ответ, переживший сон/скачок часов во время
запроса, отбрасывается целиком. `Date.now()` используется
только для обнаружения сна/скачка часов, после которого нужен новый запрос.
Открытие, возвращение на вкладку, восстановление сети и достижение нуля также
обновляют данные; параллельные запросы объединяются, минимальный интервал 5 секунд,
таймаут 10 секунд. Посекундного опроса сервера нет. Истекший токен обновляется
один раз с проверкой владельца до и после; ответ другого аккаунта отбрасывается.

При нуле сначала проверяем сервер. Без связи показываем Retry, а не выданные
минуты. Подтвержденное завершение квоты не отменяется из-за задержки финальной
записи расхода в том же UTC-дне. Новый день или подтвержденный платный тариф
показывает восстановление доступа; комнату пользователь создает сам через
обычную серверную проверку. Политика учета и существующие live-контракты не меняются.

Доставка: staging API и совместимый ZIP, CI/проверка артефакта, затем обычная
promotion PR и приватный production ZIP. Миграции, новые секреты и публикация
ZIP на сайте не требуются. Откат: предыдущие Web/ZIP; новый GET аддитивен и старым
клиентам не нужен. Ручная проверка установленного ZIP отмечается отдельно от
автоматических тестов и изолированного браузерного просмотра карточки.
...
## Дополнение от 2026-09-09: вместимость истории

Пользователь согласовал 100 видео YouTube и 200 тайтлов Crunchyroll на аккаунт.
При заполнении блокируется только новый тайтл, сохраненные продолжают
обновляться у платных пользователей. Автоматической очистки нет; место
освобождается ручным удалением. Новый контракт, реализация и проверки
зафиксированы в [watch-history-capacity.md](../../watch-history-capacity.md).
Это дополняет D01, не возвращая старые тарифные квоты и сроки хранения.

## Дополнение от 2026-09-13: разрешение записи в расширении

Перед автоматическим сбором истории пользователь явно разрешает запись для
своего аккаунта в текущем браузере. Без решения и после отказа сбор, сохранение
новых наблюдений и отправка очереди запрещены; чтение сохраненной истории,
Resume и совместный просмотр доступны. Plus/Pro и отдельное разрешение YouTube
остаются дополнительными условиями. Отзыв действует и в уже открытых вкладках;
ранее отправленные запросы могут завершиться. Серверная модель истории и
ручное редактирование на сайте не меняются. Подробности и приемка:
[Chrome compliance record](../../releases/personal-history-mvp/2026-09-13-chrome-compliance.md).
