...
- [x] Add fail-first protocol/service/SQL tests: solo/shared; matching episode
  ...
  ```
- [x] Run failures against the existing implementation and record output.
- [x] Extend bounded RPC selection using the existing filtered matching relation;
...
- [x] Build/validate previews from canonical rows and selected matching evidence.
  Preserve the final generation check; update strict schemas and old-caller tests.
- [x] Run focused protocol/web and actual disposable SQL tests, record size/query
...
- [x] Fail-first: warm both modes and old-title detail, accept a real new progress
  ...
  ```
- [x] Implement one bounded persistent read cache. Separate stale-but-displayable
  progress changes from hard invalidations; preserve late-response protection.
- [x] Test same-owner token refresh, logout/login scope, new account, generation,
...
- [x] Fail-first: render a title with exact-query preview while its detail request
  ...
  ```
- [x] Seed episode presentation from validated matching previews, prefer newer
...
- [x] Reconcile newly observed solo titles immediately; shared additions require
  verified membership or an explicitly pending observation, never guessed groups.
- [x] Verify stable order/disclosures, terminal progress, bounded pagination and
  read-only offline display through component tests and the installed MV3 artifact.
- [x] Run protocol/web/extension checks/tests, narrow artifact build/validation
  and independent review. Keep all claims scoped to actual local evidence;
  record needed staging activation separately. Graphify/doc maintenance accompanies
  the final local documentation/graph checkpoint.

Local runtime completed at `4c4d559c`. Full checks/tests and installed MV3 evidence,
the two closed final-review findings, known build warnings and the explicit
database-first staging boundary are recorded in
`docs/watch-history-local-read-verification.md`. Nothing was remotely activated
or copied into the user's established tester folders.
