...
6. As recorded in the roadmap: no `participantSessionId`. Generations and the Hibernation core are implemented, but forced wake tests, DO alarm room end, and precise quota metering are still pending.
...
- [~] 6.2 (Task 6) Hibernation migration per roadmap: `acceptWebSocket`, `webSocketMessage/Close/Error`, versioned attachments under Cloudflare's current 16,384-byte serialized attachment limit, rebuild from `getWebSockets()`, SQLite-backed room snapshot + `serverSeq` replay buffer, and `setWebSocketAutoResponse` for raw `ping`/`pong` are implemented. Note: `web_socket_auto_reply_to_close` is already active via compat date — rely on clean CLOSED transitions. Remaining: forced hibernation wake tests in the Workers runtime and staging idle-session acceptance.
