...
- [ ] 2026-06-13: Block 2 PR-1 (web side) implemented on `codex/room-lifecycle-quota`: migration `20260612_room_lifecycle_quota.sql` (lifecycle columns, partial unique idempotency index, `usage_daily` + `increment_host_usage`), pure quota math in `apps/web/lib/room-quota.ts` (12 node:test cases), orchestration in `lib/anidachi-auth/room-usage.ts`, idempotent `createRoom`, quota-aware create/connect with capped host token TTL, `lobby -> live` promotion, new host-only `POST /api/rooms/:roomId/end`, `last_active_at` bumps on connect/join. Verified: `pnpm check` and `pnpm test` green (33 web tests). NOT yet done: Supabase migration must be applied before deploy; staging acceptance pending; extension-side 2.6 UX and `clientRequestId` sending land in PR-2. Checkboxes 2.1–2.5 stay open until staging verification.
