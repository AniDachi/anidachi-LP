...
- [x] 1.5 Real-WebRTC two-browser harness shipped: `tests/e2e/p2p-media-harness.mjs` bundles the *actual* `P2PMediaController` + `RoomClient` (esbuild) into a page, boots the real Worker (`wrangler dev`), and drives two Chromium contexts with a fake camera. Asserts both peers have received a room snapshot with both cameras enabled before measuring media SLOs, then verifies **decoded** video TTFM via `getStats` (S3/S4), camera-flow health state, and that reload/short-network-loss recovery restores decoded video without recreating the room (S5). ICE restart counters are logged as diagnostics because cooldown can legitimately suppress an extra restart when media already resumes. The harness also has an explicit relay diagnostic mode (`HARNESS_FORCE_RELAY=true`) that compiles the real media engine with `iceTransportPolicy: "relay"` and asserts that selected candidate pairs use relay candidates. Relay mode can use explicit `HARNESS_ICE_SERVERS_JSON` or fetch short-lived TURN servers through the Worker `/ice-servers` path with `HARNESS_USE_WORKER_ICE_SERVERS=true`. Worker-backed relay mode requires Cloudflare TURN configuration with `turns:443`; the Worker and extension now cache last-valid relay credentials so transient credential-API failures do not fall back to STUN-only. Default direct-first runs still usually select `host/host` on one machine, so relay mode and staging multi-network acceptance are required before claiming real-world network coverage. Signaling-layer coverage (token reject, join/snapshot, relay+serverSeq, replay, ghost, keepalive, cap, takeover, clean-leave, ICE-auth) lives in `scripts/room-signaling-harness.mjs`.
...
- [x] 5.11 Opus DTX/FEC negotiation hardening: local offers and answers are created explicitly, then narrowly patched for the negotiated audio/Opus payload before `setLocalDescription()`. Existing Opus fmtp params are preserved while `useinbandfec=1` and `usedtx=1` are guaranteed; missing Opus fmtp lines are inserted after the matching `rtpmap`. The patch does not touch remote SDP or non-Opus/video fmtp. Verified: extension check/test (182 tests) and real-WebRTC harness 12/12.
- [x] 5.12 Local camera track-ended recovery hardening: `P2PMediaController` no longer re-acquires a dead local camera track immediately. Local camera recovery now uses an explicit `off | starting | live | recovering | unavailable` state, delayed exponential backoff, a rapid-failure sliding window, cancellable recovery/stability timers, and deduped public `CAMERA_ON/OFF` publishing. During automatic recovery the room-visible camera status stays on, so remote peers do not see false `CAMERA_OFF -> CAMERA_ON` flapping or unnecessary video-expectation churn; after repeated rapid failures the controller gives up with a single public off status and a camera-specific user message. Verified: extension check/test (192 tests), staging extension build, and staging artifact validation. Remaining proof is still two-machine/two-network manual acceptance with camera contention/unavailable scenarios.
- [x] 5.13 Single-client media session stability: `useGhostCam` no longer tears down and recreates the P2P media session when the same authenticated participant is refreshed as a new object by identity refreshes, room snapshots, panel open, focus, or storage events. The session lifecycle now keys on stable room/participant/generation primitives, while mutable callbacks and participant data are read through refs. This prevents local camera/mic flicker before a second participant joins. Verified: new hook regression test, extension check, and extension test suite (193 tests).
...
- [ ] 7.2b Tighten Worker CORS to the exact safe web/extension/provider-origin model after a focused origin audit. Do not guess here: content-script requests may carry provider-page origins, so CORS tightening must be validated against the real extension request path.
...
- [x] 2026-06-27: Tightened remote-video health and harness semantics on branch `codex/p2p-media-recovery-harness`: decoded-frame movement is now authoritative when `framesDecoded` exists, while bytes-only flow remains a fallback for browsers without decoded-frame counters. The real-WebRTC harness mirrors overlay camera signaling (`CAMERA_ON`), waits for both peers to observe both active cameras in the room snapshot before media SLO timing, exposes remote audio/video flow state, asserts expected camera flow, and treats ICE restart counters as diagnostics while requiring decoded video to resume after reload and short network loss. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (178 tests), `pnpm --filter @anidachi/api check`, `pnpm --filter @anidachi/api test`, `pnpm --filter @anidachi/api test:runtime`, `pnpm harness:rooms` (35/35), and `npm --prefix tests/e2e run harness:p2p` twice (12/12 + 12/12). Remaining proof is still forced TURN and real multi-network acceptance.
- [x] 2026-06-27: Added Opus DTX/FEC SDP hardening on branch `codex/opus-dtx-fec`: local offer/answer generation now guarantees `useinbandfec=1` and `usedtx=1` only on negotiated audio/Opus fmtp before `setLocalDescription()`, while preserving existing fmtp params and leaving remote/non-Opus SDP untouched. SDP summaries now expose `audioOpusDtx` in addition to `audioOpusInbandFec`. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (182 tests), and `npm --prefix tests/e2e run harness:p2p` (12/12). Remaining proof is still forced TURN and real multi-network acceptance.
- [x] 2026-06-27: Added Cloudflare STUN to the extension local fallback on branch `codex/cloudflare-stun-fallback`: `getDefaultP2PIceServers()` now includes the documented Cloudflare STUN pair before Google STUN, and the existing ICE prioritization/dedupe tests protect the Worker-plus-local merge path from duplicate STUN URLs. Remaining proof is still forced TURN and real multi-network acceptance.
- [x] 2026-06-27: Added `iceCandidatePoolSize: 2`, a regression test proving normal configs stay `iceTransportPolicy: "all"`, and compact selected-candidate-pair telemetry on branch `codex/p2p-ice-direct-telemetry`. The telemetry records direct/relay and candidate type/protocol only; it does not log raw candidates, IPs, URLs, or participant ids. Remaining proof is still forced TURN and real multi-network acceptance.
- [x] 2026-06-28: Removed legacy LiveKit runtime path on branch `codex/remove-livekit-legacy-media`: `useGhostCam` is P2P-only, the extension no longer imports `livekit-client` or reads the old `WXT_MEDIA_TRANSPORT` / `local:experiment:mediaTransport` switch, Worker `/livekit/token` and `apps/api/src/livekit-token.ts` are removed, `LIVEKIT_URL` is removed from `wrangler.toml`, local `infra/livekit` is deleted, active docs now describe P2P+TURN as the only current media path, and an API regression test asserts the old token endpoint stays unavailable. CORS tightening remains split into 7.2b because extension content-script origins need a separate exact-origin audit.
- [x] 2026-06-28: Tightened the extension media lifecycle on local branch `codex/fix-overlay-session-video-latency`: push-to-talk no longer treats a visible transient `window.blur` as user intent to stop while the V key is still held; local microphone `track.ended` recovery keeps voice intent active with bounded re-acquire/backoff; camera recovery keeps public `CAMERA_ON` stable and avoids downgrading the video transceiver to `recvonly` while a wanted camera is recovering, so recovery can use `replaceTrack` instead of forcing extra SDP churn. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (204 tests), `pnpm build:extension:staging`, `pnpm validate:extension:staging`, and `npm --prefix tests/e2e run harness:p2p` (12/12). Remaining proof: reload the updated staging artifact in both browsers and repeat real two-network/VPN/manual media acceptance.
- [x] 2026-06-29: Root-caused the remaining push-to-talk drop on local branch `codex/fix-overlay-session-video-latency`: `OverlayApp` had an unmount cleanup effect whose dependency changes could run cleanup during ordinary rerenders and call `stopLiveVoiceTalk()` while the user was still holding V. Replaced it with a ref-backed unmount-only cleanup hook, added a regression test proving callback identity changes do not stop PTT, rebuilt the staging extension artifact, and verified `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (205 tests), `pnpm build:extension:staging`, `pnpm validate:extension:staging`, and `npm --prefix tests/e2e run harness:p2p` (12/12). Remaining proof: manual hold-V test in the real staging artifact on two real clients/networks.
- [x] 2026-06-29: Fixed intermittent overlay sign-in adoption on local branch `codex/fix-overlay-session-video-latency`: entering a Crunchyroll episode could mount or re-render the overlay before the website session had been silently exchanged into extension tokens, leaving the panel stuck on "Not signed in" until reload. Added a bounded silent-adoption retry helper used by initial load and panel-open identity refresh, with cancellation for hidden pages and explicit sign-out suppression. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (207 tests), `pnpm build:extension:staging`, `pnpm validate:extension:staging`, and `npm --prefix tests/e2e run harness:p2p` (12/12). Remaining proof: manual episode-switch test in the rebuilt staging artifact.
- [x] 2026-06-29: Fixed false remote microphone indicators on local branch `codex/fix-overlay-session-video-latency`: inbound WebRTC audio stats can now confirm or clear active-speaker state only while that remote participant is expected to be talking after `voice-start`. After `voice-stop`, RTP/DTX/comfort-packet movement is forced back to quiet and cannot relight the peer's mic badge. Added regression coverage for moving inbound stats after `voice-stop`. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (208 tests), `pnpm build:extension:staging`, `pnpm validate:extension:staging`, and `npm --prefix tests/e2e run harness:p2p` (12/12). Remaining proof: manual two-device/VPN PTT acceptance with the rebuilt staging artifact.
- [x] 2026-07-03: Fixed the real-log PTT/camera coupling failure on local branch `codex/fix-overlay-session-video-latency`: P2P participant selection now keeps voice-only peers in the mesh, `voice-start`/`voice-stop` signals can establish/clear audio expectation without requiring `cameraEnabled`, duplicate stop/start control events are ignored safely, and the diagnostic bundle exports a merged top-level page+background timeline so future bug reports show the full media/auth/signaling sequence. The noisy local `stun.cloudflare.com:53` fallback was removed after repeated Chrome 701 timeouts in exported logs. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (235 tests), `pnpm build:extension:staging`, and `pnpm validate:extension:staging`. Remaining proof: manual two-client/VPN PTT acceptance with camera off/on and saved full logs.
- [x] 2026-07-03: Fixed same-room reconnect camera blink on branch `codex/restore-extension-wip-20260703`: P2P media lifetime now follows an active room session (`roomId` + participant + media seats + non-idle status) instead of transient `connected && snapshotReady`, so reconnecting the room WebSocket no longer publishes `CAMERA_OFF` or recreates the camera controller. `useGhostCam` cleanup now disconnects only its own controller instance. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (243 tests), `pnpm build:extension:staging`, `pnpm validate:extension:staging`, and `npm --prefix tests/e2e run harness:p2p` (12/12). Remaining proof: manual two-client/VPN acceptance with the rebuilt staging artifact.
- [x] 2026-07-07: Fixed the real-log two-client camera subscriber race on branch `codex/room-rail`: `useGhostCam` now accepts answer/ICE/renegotiate signals from an already-created P2P peer during the linger window, even if a transient room snapshot briefly shows that peer as `cameraEnabled=false`. Chat-only participants are still not added to the WebRTC mesh solely because another participant publishes camera. `useGhostCam` now also logs inactive-media signal drops with camera/voice state for future diagnostics. Verified: `pnpm --filter @anidachi/extension check`, `pnpm --filter @anidachi/extension test` (250 tests), `pnpm build:extension:staging`, `pnpm validate:extension:staging`, and `npm --prefix tests/e2e run harness:p2p` (12/12). Remaining proof: reload the refreshed staging extension folder on both real clients and repeat the two-PC camera acceptance with saved full logs if it still fails.
...
- [x] 2026-07-16: Made local camera publishing opt-in on branch `codex/overlay-layout-runtime-v2`: creating, joining, and restoring a room now starts with camera off; a same-room WebSocket reconnect preserves the user's explicit current choice; leaving, ending, signing out, or changing account resets camera intent to off. The join transition is applied only after the persisted session is confirmed current, so a stale async join cannot disable media in an active room. Verified: extension check/test (465 tests), staging extension build, and staging artifact validation. Remaining proof: manual create/join/restore and same-room reconnect acceptance in the loaded staging artifact.
- [x] 2026-07-16: Fixed one-way camera publishing and dropped-SDP recovery on branch `codex/overlay-layout-runtime-v2`: a participant who enables camera now establishes a WebRTC peer with every joined media-seat receiver even when their cameras are off, and incoming signaling is authorized by stable media-seat membership instead of racing replicated `cameraEnabled` state. Per-peer media mutations are serialized, sender parameters are applied only when they change, and deferred negotiation drains after media sync. If a signaling reconnect finds a stale local offer, both clients now discard the old peer through the existing `bye` control signal and negotiate one clean connection with the current tracks instead of rolling back one side into a mixed media state. Verified: extension check/test (488 tests), room harness 39/39, isolated real-Chromium one-way harness 2/2 (`camera-off host receives guest video`, 154 ms locally), and repeated full real-Chromium harness runs 17/17. The final measured dropped-offer and dropped-answer recoveries were 307 ms and 308 ms. Remaining proof: manual two-client staging acceptance with the rebuilt artifact.
- [~] 2026-07-27: Implemented the client-only voice-controls slice on branch `codex/voice-controls-plan`: Push to talk and explicit Open mic now share one privacy-safe microphone publication lifecycle; camera and microphone intent remain independent; remote volume/mute is listener-local and account-scoped; speech activity is separate from RTP-flow health; and live P2P voice no longer changes source-player volume. Safe diagnostics expose mode, publication intent/state, expected audio publishers, speech/flow classifications, and effective per-participant output without device labels or user content. During strict real-WebRTC verification, a rapid answerer-side camera off/on exposed a lost renegotiation request inside the existing cooldown; the request is now coalesced and retried after the cooldown instead of dropping the latest transceiver direction. Verified: extension check plus 861/861 extension tests, root check/test (6/6 Turbo tasks), and the real-Chromium P2P harness (26/26), including late join, signaling reconnect, camera/microphone independence, warm microphone reuse, local mute/volume persistence, remote track replacement, and immediate camera recovery. The staging artifact built and validated, Graphify refreshed, and both canonical test folders match it exactly. Remaining: manual two-client/different-network and forced-relay acceptance.
...
- [~] 2026-08-31: Reworked guest departure on `codex/redesign-room-departure`
  ...
  after that commit. Passive tab close is unchanged and retains its separate
  60-second Worker alarm callback. Normal extension leave no longer invokes
  active-room recovery automatically, while `already_departed` is an idempotent
  success and the emergency role-specific Leave/End action remains separately
  confirmed. The staging gate now lets bearer-authenticated internal POST
  callbacks reach route-level service authentication without opening other
  staging routes. Fresh local proof: protocol 140/140, API 166/166 plus runtime
  36/36, Web 382 passed/3 skipped, extension 1457/1457, room harness 39/39,
