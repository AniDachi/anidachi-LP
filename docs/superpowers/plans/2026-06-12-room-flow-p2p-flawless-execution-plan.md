...
- [~] 1.5 Scenarios: DONE at the signaling layer — token reject (missing/invalid), two-party join + snapshot + PARTICIPANT_JOINED, token-derived identity, targeted P2P signal relay with serverSeq, missed-signal replay on reconnect (S5), stale duplicate-socket force-close + no-ghost (S8), PING/PONG keepalive. REMAINING (Playwright + Chromium fake media slice): real getUserMedia video both ways with TTFM via `getStats` (S3/S6), `context.setOffline` recovery timing, force-relay (`WXT_P2P_FORCE_RELAY=true`), idle keepalive. Tracked as a follow-up that needs a browser runner.
