...
- [x] 2026-06-27: Added Cloudflare STUN to the extension's local unauthenticated ICE fallback on branch `codex/cloudflare-stun-fallback`: `getDefaultP2PIceServers()` added Cloudflare STUN before Google STUN, while existing ICE prioritization/deduping keeps endpoint STUN before TURN and avoids duplicate URLs when Worker `/ice-servers` already returned Cloudflare STUN. Note: the fallback was later narrowed to `stun.cloudflare.com:3478` only after real logs showed repeated Chrome 701 timeouts on `:53`. Remaining proof is still the real TURN relay and two-network staging acceptance.
...
- [x] 2026-06-28: Tightened extension camera/mic recovery on local branch `codex/fix-overlay-session-video-latency`: PTT keeps voice intent through visible transient `window.blur` while V is still held, mic `track.ended` recovery uses bounded re-acquire instead of publishing immediate `voice-stop`, and camera recovery keeps public `CAMERA_ON` plus video `sendrecv` stable instead of downgrading to `recvonly` during wanted recovery. Verified: extension check/test (204 tests), staging extension build + validation, and real-WebRTC harness 12/12. Remaining proof: updated staging artifact manual acceptance on two real networks/VPN.
- [x] 2026-06-29: Fixed the remaining local PTT drop on local branch `codex/fix-overlay-session-video-latency`: the overlay's unmount cleanup was dependency-driven and could call `stopLiveVoiceTalk()` on ordinary rerenders when unrelated callbacks changed identity. It is now ref-backed and unmount-only, with regression coverage proving callback identity changes do not stop PTT. Verified: extension check/test (205 tests), staging extension build + validation, and real-WebRTC harness 12/12. Remaining proof: manual hold-V acceptance in the updated staging artifact on two real clients/networks.
- [x] 2026-06-29: Fixed intermittent overlay auth adoption on local branch `codex/fix-overlay-session-video-latency`: initial/panel-open identity refresh now retries silent website-session pickup for a short bounded window instead of treating the first non-interactive miss as final, while still cancelling after explicit sign-out or hidden-page teardown. Verified: extension check/test (207 tests), staging extension build + validation, and real-WebRTC harness 12/12. Remaining proof: manual Crunchyroll episode-switch acceptance in the rebuilt staging artifact.
- [x] 2026-06-29: Fixed false remote microphone indicators on local branch `codex/fix-overlay-session-video-latency`: WebRTC inbound audio stats no longer publish remote active-speaker state by themselves. They only confirm/clear activity while the remote peer is expected to be talking after `voice-start`; after `voice-stop`, any remaining RTP/DTX/comfort-packet movement is forced quiet and cannot relight the peer's mic badge. Verified: extension check/test (208 tests), staging extension build + validation, and real-WebRTC harness 12/12. Remaining proof: manual two-device/VPN PTT acceptance.
- [x] 2026-07-03: Decoupled push-to-talk audio from camera visibility on local branch `codex/fix-overlay-session-video-latency`: voice-only participants now enter or remain in the P2P media mesh without requiring `cameraEnabled`, incoming `voice-start`/`voice-stop` bypass the camera-only media filter while still requiring active room membership, and duplicate PTT stop/start signals are idempotent. Diagnostic exports now include a unified top-level timeline from page debug plus background diagnostics, while preserving split diagnostic/page entries for deeper inspection. The local unauthenticated STUN fallback also drops `stun.cloudflare.com:53` after real logs showed repeated Chrome 701 timeouts. Verified: extension check/test (235 tests), staging extension build + validation. Remaining proof: reload the updated staging artifact on both real clients/VPN and export full logs if PTT/video still regresses.
...
- [~] 2026-08-31: Guest departure recovery on `codex/fix-rapid-reaction-delivery` now distinguishes explicit intent from passive stale-tab cleanup. The missing staging bearer allowlist entry for `POST /api/rooms/:roomId/depart` is covered and fixed; an explicit same-room leave can recover a missing/stale tab record through the authenticated server-owned assignment, while tab close remains exact-session-only. The active-room conflict UI replaces `Open active room` with a confirmed role-specific emergency Leave/End action. Local root, protocol, Web, extension, API/runtime, room-harness, real-WebRTC, and staging-artifact gates are green. Remaining proof: deploy Web plus the rebuilt extension to staging and repeat ordinary guest leave, lost-record recovery, host recovery, and stale-tab takeover in two profiles.
...
- [x] Reconnect uses a stable, bounded `participantSessionId`; exact-session
  matching protects takeover, departure, and stale cleanup paths.
- [x] Worker socket replacement and pending disconnect handling are bound to
  participant session, not only user id.
