...
**Дата:** 2026-09-15; обновлено 2026-09-18. **Статус:** этапы 1–3 приняты на staging через PR #349 (`bcd00c7d`), #350 (`5a7aa839`) и #351 (`876b9e94`). Владелец согласовал продолжение блока email; выполняется отдельный этап 4A — безопасное сохранение контакта. Этапы 4B/4C и остальные этапы еще не начаты. Main остается `4b4ff883`; реальные письма, изменение контактов и публикация ZIP при проверках не выполняются.
...
- [ ] Независимое ревью, commit `fix(web): save install leads with atomic CRM mutations`, отдельный PR и приемка staging; точные SHA/deployment receipts записать в PR.
