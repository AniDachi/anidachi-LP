...
- [x] Повторить `lib/extension-install-compatibility.test.ts`: прежняя кнопка, disabled state, отсутствие presence и возврат к комнате сохранены. Базовые 6/6 passed; новые проверки до правки — 3 passed / 9 failed, после — 18/18 вместе с compatibility. Web typecheck passed.
- [x] Полный web suite: 633 passed / 6 fixture-dependent skipped / 0 failed. Next production build, web typecheck и `pnpm dev:check` (web/docs) passed.
